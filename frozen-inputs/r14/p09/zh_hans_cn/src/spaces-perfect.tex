...
(\ref{equation-epsilon}) 中的态射 $\epsilon$ 是带环位点的平坦态射。特别地，
函子
...
\noindent
设 $X$ 为概形，记号同 (\ref{equation-epsilon})。回忆，由“下降”命题
...
\ref{lemma-check-quasi-coherence-on-covering} 以及概形情形得出；概形情形即
“概形的导出范畴”引理
...
\ref{lemma-check-quasi-coherence-on-covering} 可知，结论归结为概形情形；后者即
“概形的导出范畴”引理
...
$E = R\lim E_n$，见引理 \ref{lemma-nice-K-injective}。于是由“单射对象”引理
...
因此可把引理 \ref{lemma-application-nice-K-injective} 应用于函子
...
选取引理 \ref{lemma-quasi-coherence-direct-image} 中的整数 $N$。
...
$$
可交换（引理 \ref{lemma-quasi-coherence-pullback} 和
“概形的导出范畴”引理
\ref{perfect-lemma-quasi-coherence-pullback}）。这是因为引理
\ref{lemma-derived-quasi-coherent-small-etale-site} 中的等价
...
$$
也可交换（引理 \ref{lemma-quasi-coherence-direct-image} 和
“概形的导出范畴”引理
\ref{perfect-lemma-quasi-coherence-direct-image}）。这也由上面所示的位点交换图得出，
因为引理 \ref{lemma-derived-quasi-coherent-small-etale-site} 的证明表明，函子
$R\epsilon_*$ 给出等价
...
引理 \ref{lemma-derived-quasi-coherent-small-etale-site} 和评注
\ref{remark-match-total-direct-images} 实现。
\end{proof}
...
\ref{lemma-derived-quasi-coherent-small-etale-site} 和评注
\ref{remark-match-total-direct-images} 实现。
\end{proof}
...
\ref{lemma-closed-proper-over-base} 中的等价条件成立，我们称
{\it $T$ 在 $Y$ 上固有}。
\end{definition}
...
故由引理 \ref{lemma-functoriality-closed-proper-over-base} 第 (2) 项可得，
...
\ref{definition-proper-over-base}。由于 $\mathcal{F}$ 的概形论支撑是闭子空间，
其底闭子集为 $\text{Supp}(\mathcal{F})$，由定义
\ref{definition-proper-over-base} 可见 (1)、(2) 等价。显然 (2) 蕴含 (3)。
反过来，若 (3) 成立，则
$\text{Supp}(\mathcal{F}) \subset |Z|$，因而例如由引理
\ref{lemma-closed-closed-proper-over-base}，$\text{Supp}(\mathcal{F})$
在 $Y$ 上固有。
\end{proof}
...
于是本引理由引理
\ref{lemma-base-change-closed-proper-over-base} 得出。
\end{proof}
...
有限型且拟凝聚，断言本身便由引理
\ref{lemma-closed-closed-proper-over-base} 和
\ref{lemma-union-closed-proper-over-base} 直接得出。关于拟凝聚性，参见
...
\begin{proof}
由引理 \ref{lemma-module-support-proper-over-base}，存在闭浸入
...
由假设和引理 \ref{lemma-support-proper-over-base-pushforward}，
...
亦可由引理 \ref{lemma-quasi-coherence-direct-image} 推出。
\end{proof}
...
\begin{proof}
证明与引理 \ref{lemma-direct-image-coherent} 的证明相同。
考察正合函子 $Rf_*$ 对特异三角
$\tau_{\leq a}E \to E \to \tau_{\geq a + 1}E \to \tau_{\leq a}E[1]$
的作用，也可由引理 \ref{lemma-direct-image-coherent} 推出此结论。
\end{proof}
...
\ref{lemma-coherent-internal-hom} 和
\ref{lemma-direct-image-coherent-bdd-below} 推出。
\end{proof}
...
$$
由引理 \ref{lemma-make-more-elementary-distinguished-squares}
...
\begin{proof}
证明与引理 \ref{lemma-induction-principle} 的证明完全相同。
...
\begin{remark}
\label{remark-how-to}
在引理 \ref{lemma-induction-principle-separated} 中，应如何选择集合
$\mathcal{B}$？以下是一些例子：
\begin{enumerate}
...
\ref{lemma-induction-principle-separated} 重现引理
...
\begin{proof}
这可由引理 \ref{lemma-induction-principle-separated} 推出；不过，我们将直接
重做引理 \ref{lemma-induction-principle} 的证明，给出一个直接论证。
我们将使用滤过
$$
\emptyset = U_{n + 1} \subset
U_n \subset U_{n - 1} \subset \ldots \subset U_1 = X
...
(\ref{equation-stalk-restriction}) 和 (\ref{equation-stalk-j-shriek})，
...
$$
我们已经在引理 \ref{lemma-exact-sequence-lower-shriek} 中看到，
$$
...
$$
该列来自引理 \ref{lemma-exact-sequence-j-star}；再利用
...
\begin{proof}
使用引理 \ref{lemma-exact-sequence-lower-shriek} 的特异三角，
...
\ref{injectives-theorem-K-injective-embedding-grothendieck}。
在引理 \ref{lemma-exact-sequence-j-star} 的证明中，我们得到复形的短正合列
$$
...
\ref{lemma-exact-sequence-j-star} 得出。完全同样地，(1)(b) 由引理
\ref{lemma-exact-sequence-lower-shriek} 得出。

\medskip\noindent
设 $F$ 为 $D(\mathcal{O}_V)$ 中的对象，且其上同调层支撑在 $j^{-1}T$ 上。
由引理 \ref{lemma-restrict-direct-image-open}，有
$(Rj_*F)|_U = Rj_{W, *}(F|_W) = 0$，因为按假设 $F|_W = 0$。
同理，由引理 \ref{lemma-restrict-lower-shriek}，
$(j_!F)|_U = j_{W!}(F|_W) = 0$。
...
\ref{lemma-restrict-lower-shriek} 以及
...
置 $F' = \sigma^*F$。引理
\ref{lemma-exact-sequence-lower-shriek} 和
\ref{lemma-exact-sequence-j-star} 的特异三角表明
...
\ref{lemma-restrict-direct-image-open}，限制与总正像相容，故
$(Rj_{V, *}B)|_U$ 以及
...
\ref{lemma-pushforward-with-support-in-open} 得到 $B' = 0$。

\medskip\noindent
态射 $F \to E$ 的存在性由 Mayer--Vietoris 的 $\Hom$ 序列得出，参见引理
\ref{lemma-mayer-vietoris-hom}。
\end{proof}
...
\medskip\noindent
本节研究函子 $RQ_X$。在
\ref{section-better-coherator} 节中，我们将研究包含函子
...
由于仿射态射是可表的，利用评注
\ref{remark-match-total-direct-images} 的相容性，可将问题归结为概形情形。
概形情形是“概形的导出范畴”引理
...
以及引理
\ref{lemma-derived-quasi-coherent-small-etale-site} 得出。
\end{proof}
...
更确切地说，我们将使用引理
\ref{lemma-induction-principle-enlarge}。
...
$$
是拟同构（引理
\ref{lemma-pushforward-with-support-in-open}）。
由假设有同构
...
\ref{lemma-affine-coherator} 和
\ref{lemma-flat-pushforward-coherator}，并利用我们的假设，有
$$
...
于是引理 \ref{lemma-affine-pushforward} 保证
引理 \ref{lemma-argument-proves} 的假设成立。
因此结论成立。
\end{proof}
...
\begin{proof}
注意，由命题
\ref{proposition-quasi-compact-affine-diagonal}，图中的水平箭头
...
根据评注 \ref{remark-how-to}，归纳原理引理
\ref{lemma-induction-principle-separated} 的条件 (1)、(2) 和 (3)(a)
成立，因此还需证明 (3)(b) 和 (4)。
...
\ref{spaces-morphisms-lemma-affine-permanence}）。
因此，由引理 \ref{lemma-affine-pushforward}，$P(U)$ 成立。
...
\ref{lemma-affine-injective-colimit-direct-sum-pushforwards-artin} 中证明。
\end{proof}
...
$\mathcal{F}_i = (Z_i \to X)_*\mathcal{G}_i$，
如引理 \ref{lemma-injective-colimit-direct-sum-pushforwards-artin} 所述。
...
\ref{lemma-quasi-coherence-direct-image} 中所述。
考虑如下拟凝聚层复形的短正合序列
$$
...
$$
这是 $X$ 上的复形。由
由引理 \ref{lemma-quasi-coherence-direct-image}，我们看到
...
\ref{lemma-injective-pushforward} 所给出的必要消灭性推出。 % P09-ERR-0427
\end{proof}

\begin{proposition}
\label{proposition-Noetherian}
设 $S$ 为概形，$X$ 为 $S$ 上的诺特代数空间。
则函子（\ref{equation-compare}）
$$
D(\QCoh(\mathcal{O}_X))
\longrightarrow
D_\QCoh(\mathcal{O}_X)
...
\begin{proof}
这立即由引理 \ref{lemma-Noetherian-pushforward} 和
\ref{lemma-argument-proves} 得到。
\end{proof}
...
\end{enumerate}
这里 $\epsilon$ 如（\ref{equation-epsilon}）中所述。
\end{lemma}
...
由引理 \ref{lemma-epsilon-flat} 可知 $E$ 是
$D^-(\mathcal{O}_X)$ 中的对象。
...
态射（引理 \ref{lemma-epsilon-flat}），有
$$
...
\ref{lemma-descend-pseudo-coherent} 和
...
（\ref{equation-epsilon}）中所述。
在此情形中，由引理 \ref{lemma-descend-pseudo-coherent}，
$E$ 是 $m$-拟凝聚的，当且仅当 $E_0$ 也是如此。
同样，由引理 \ref{lemma-descend-finite-type}，$H^i(E_0)$
是有限型的（即凝聚的），当且仅当 $H^i(E)$ 也是如此。
最后，由引理 \ref{lemma-epsilon-flat}，$H^i(E_0) = 0$
当且仅当 $H^i(E) = 0$。
...
$D_\QCoh(\mathcal{O}_{X_0})$ 中的对象，且
$\epsilon : X_\etale \to (X_0)_{Zar}$ 如
（\ref{equation-epsilon}）中所述。对于每个拟凝聚模
...
$$
因为 $\epsilon$ 平坦（引理 \ref{lemma-epsilon-flat}）。
...
\begin{proof}
回忆 $\epsilon$ 是平坦的（引理 \ref{lemma-epsilon-flat}），
...
其中使用引理 \ref{lemma-check-quasi-coherence-on-covering} 的判别准则、
引理 \ref{lemma-descend-pseudo-coherent} 和
\ref{lemma-descend-perfect} 的判别准则，以及引理
\ref{lemma-descend-RHom} 的结果。
\end{proof}
...
由引理 \ref{lemma-derived-quasi-coherent-small-etale-site}，
可将 $K, L, M$ 写成 $\epsilon^*K_0, \epsilon^*L_0, \epsilon^*M_0$，
其中某些 $K_0, L_0, M_0$ 属于 $D_\QCoh(\mathcal{O}_X)$。
于是由引理 \ref{lemma-descend-RHom} 可见，情形 (1) 和 (3)
...
\ref{definition-approximation-holds} 中任意满足下列条件的三元组
$(T, E, m)$：
\begin{enumerate}
\item $E$ 是 $(m - r)$-拟凝聚的；且
...
\ref{lemma-pushforward-with-support-in-open}，有 $Rj_*E|_V = E$，
...
$(T, E, m)$ 为定义 \ref{definition-approximation-holds} 中的三元组。
若
\begin{enumerate}
...
由引理 \ref{lemma-pushforward-perfect}，$Rj_*P$ 是
$D(\mathcal{O}_X)$ 中的完美对象。另一方面，由引理
\ref{lemma-pushforward-with-support-in-open}，$Rj_*P = j_!P$。
例如由（\ref{equation-stalk-j-shriek}）可见，$j_!P$ 支撑在 $T$ 上。
...
若定义 \ref{definition-approximation-holds} 中的三元组
...
令 $\epsilon : X_\etale \to X_{0, Zar}$ 为（\ref{equation-epsilon}）
中的态射。由引理 \ref{lemma-derived-quasi-coherent-small-etale-site}，
可写成 $E = \epsilon^*E_0$，其中某个对象 $E_0$ 属于
$D_\QCoh(\mathcal{O}_{X_0})$。
由引理 \ref{lemma-descend-pseudo-coherent}，$E_0$ 是
$m$-拟凝聚的。比较上同调层的茎（见引理
\ref{lemma-epsilon-flat} 的证明），可见 $H^i(E_0)$ 支撑在 $T_0$ 上
（对 $i \geq m - r + 1$）。由《概形的导出范畴》引理
\ref{perfect-lemma-approximation-affine}，存在逼近 $P_0 \to E_0$，
它对应于三元组 $(T_0, E_0, m)$。
由引理 \ref{lemma-descend-perfect}，可见
...
设 $r_U$ 为定义 \ref{definition-approximation} 中适合于对子
...
\ref{lemma-approximation-affine} 保证存在逼近
$P \to E|_V$，它是三元组 $(T', E|_V, m)$ 在 $V$ 上的逼近。
应用引理 \ref{lemma-open} 可见 $(T', E, m)$ 可被逼近。
这样的逼近也是 $(T, E, m)$ 的逼近。
...
$U \times_X V$ 使用引理
\ref{lemma-derived-quasi-coherent-small-etale-site} 的等价
（以及评注 \ref{remark-match-total-direct-images} 的相容性）。
...
$$
可提升为 $Q \to E|_V$。由引理 \ref{lemma-glue}，得到
...
\begin{proof}
这由引理 \ref{lemma-induction-principle} 的归纳原理以及引理
\ref{lemma-induction-step} 和 \ref{lemma-approximation-affine} 推出。
\end{proof}
...
\begin{proof}
我们将用引理 \ref{lemma-induction-principle-enlarge} 的归纳原理证明此事。
因此立即化归到如下情形：有初等特异方形
...
E|_{W \times_X V}
$$
可提升为 $Q \to E|_V$。由引理 \ref{lemma-glue}，得到态射
...
引理 \ref{lemma-lift-map-from-perfect-complex-with-support} 的证明表明
$$
...
\begin{proof}
这是引理 \ref{lemma-lift-map-from-perfect-complex-with-support} 的特殊情形。
\end{proof}
...
\begin{proof}
我们将使用引理 \ref{lemma-induction-principle} 的归纳原理证明此事。

\medskip\noindent
若 $X$ 仿射，则 $\mathcal{O}_X$ 是完美生成元。这由引理
\ref{lemma-derived-quasi-coherent-small-etale-site} 和
《概形的导出范畴》引理
...
是 $K$（见引理 \ref{lemma-pushforward-perfect} 和
\ref{lemma-pushforward-with-support-in-open}）。
...
由引理 \ref{lemma-pushforward-with-support-in-open}，有
$K' =  f_! K$，因而
$$
...
（还使用引理 \ref{lemma-derived-quasi-coherent-small-etale-site}），
这些群的消失蕴含 $E|_V$ 同构于
...
（见定理 \ref{theorem-bondal-van-den-Bergh}）。
则下列条件等价：
\begin{enumerate}
...
这一事实形式地推出。若 $f$ 仿射（引理 \ref{lemma-affine-morphism}），
或 $f$ 是开浸入，或 $f$ 是这类态射的复合，则这些条件成立。
\end{remark}

\noindent
下述结果是定理 \ref{theorem-bondal-van-den-Bergh} 的一个加强，
...
\ref{lemma-lift-map-from-perfect-complex-with-support}，可选取
...
其在 $V$ 上的限制是 $K$（见引理 \ref{lemma-pushforward-perfect} 和
\ref{lemma-pushforward-with-support-in-open}）。
...
的推前与底层概形上的推前一致；见评注
\ref{remark-match-total-direct-images}。
...
与直和可交换，并且由引理
\ref{lemma-quasi-coherence-pushforward-direct-sums}，$H^0(X, -)$ 在
...
与直和可交换；见引理
\ref{lemma-quasi-coherence-pushforward-direct-sums}。
于是，限制到 $U$ 与 $Rj_*$ 的伴随性蕴含 $K|_U$ 是
$D_\QCoh(\mathcal{O}_U)$ 中的完美对象。因此化归到 $X$ 仿射的情形，
特别地，它是拟紧且拟分离的概形。通过引理
\ref{lemma-derived-quasi-coherent-small-etale-site} 和
\ref{lemma-descend-perfect}，化归到概形的情形，即《概形的导出范畴》命题
...
\ref{theorem-bondal-van-den-Bergh}，至少存在一个这样的对象。
结合引理 \ref{lemma-quasi-coherence-direct-sums}、命题
\ref{proposition-compact-is-perfect} 以及《导出范畴》命题
\ref{derived-proposition-generator-versus-classical-generator}，
可见 $G$ 是 $D_{perf}(\mathcal{O}_X)$ 的经典生成元。
\end{remark}

\noindent
下述结果是命题 \ref{proposition-compact-is-perfect} 的加强。
设 $T \subset |X|$ 为闭子集，其中 $X$ 是代数空间。
...
由命题 \ref{proposition-compact-is-perfect}，完美对象给出
...
\ref{lemma-generator-with-support}。% P09-ERR-0439
...
\ref{lemma-generator-with-support}，至少存在一个这样的对象。
结合 $D_{\QCoh, T}(\mathcal{O}_X)$ 有直和这一事实、引理
\ref{lemma-compact-is-perfect-with-support} 以及《导出范畴》命题
\ref{derived-proposition-generator-versus-classical-generator}，
...
\ref{lemma-quasi-coherence-internal-hom}。显然，$K$ 支撑在 $T$ 上，
...
\ref{lemma-generator-with-support}。如《导出范畴》引理
...
注意，由引理 \ref{lemma-quasi-coherence-tensor-product} 和
\ref{lemma-pseudo-coherent}（后者用于说明完美复形具有拟凝聚上同调层），
...
$$
都为零。事实上，由所引引理和引理
\ref{lemma-application-nice-K-injective}，有
$$
...
由定理 \ref{theorem-bondal-van-den-Bergh} 和 K-内射消解的存在性，
这样的对象存在。我们将证明，取
$$
...
\end{equation}
由引理 \ref{lemma-ext-from-perfect-into-bounded-QCoh}，
...
$$
由于 $K^\bullet$ 完美，由命题
\ref{proposition-compact-is-perfect}，它给出
$D(\mathcal{O}_X)$ 中的紧对象。% P09-ERR-0442
结合 (\ref{equation-E-is-OK})，由《微分分次代数》引理
...
\ref{lemma-tensor-with-QCoh-complex}，我们得到
$$
- \otimes_E^\mathbf{L} K^\bullet :
...
设 $X$ 为拟紧且拟分离的代数空间。
设 $T \subset |X|$ 为闭子集，且 $|X| \setminus T$ 拟紧。
定理 \ref{theorem-DQCoh-is-Ddga} 的类似结果对
$D_{\QCoh, T}(\mathcal{O}_X)$ 成立。
这由与该定理证明完全相同的论证得出，其中使用引理
\ref{lemma-generator-with-support}、\ref{lemma-compact-is-perfect-with-support}
以及引理 \ref{lemma-tensor-with-QCoh-complex} 的带支撑变式。
...
由定理 \ref{theorem-DQCoh-is-Ddga} 证明中的构造，
...
\ref{definition-approximation-holds}、\ref{definition-approximation}
和定理 \ref{theorem-approximation}。
归纳地假设已有
$$
...
于是选取 $a \leq b$，即引理
\ref{lemma-ext-from-perfect-into-bounded-QCoh} 中用于完美对象 $K_n$
的整数。
选取逼近 $K_{n + 1} \to K$，即
...
\begin{proof}
本引理的证明与引理
\ref{lemma-pseudo-coherent-hocolim} 的证明完全相同，
...
\noindent
在第 \ref{section-coherator} 节中，我们构造并研究了右伴随 $RQ_X$；
它所伴随的典范函子是
...
\begin{proof}[第一种证明]
我们将使用引理 \ref{lemma-induction-principle} 中的归纳原理证明此事。
如果 $D(\QCoh(\mathcal{O}_X)) \to D_\QCoh(\mathcal{O}_X)$ 是等价，
则本引理成立，因为第 \ref{section-coherator} 节的函子 $RQ_X$
是函子 $D(\QCoh(\mathcal{O}_X)) \to D(\mathcal{O}_X)$ 的右伴随。
特别地，本引理对仿射代数空间成立；见引理
\ref{lemma-affine-coherator}。因此，只须证明如下命题：
...
\ref{lemma-exact-sequence-j-star} 的特异三角
$$
...
$$
它位于 $D(\mathcal{O}_X)$ 中。
注意，由引理 \ref{lemma-quasi-coherence-direct-image}，
...
$$
因为由引理 \ref{lemma-quasi-coherence-pullback}，
...
并且取直和与包含函子可交换（引理
\ref{lemma-quasi-coherence-direct-sums}）。
另一方面，$D_\QCoh(\mathcal{O}_X)$ 是紧生成的，因为它有完美生成元；
见定理 \ref{theorem-bondal-van-den-Bergh}，% P09-ERR-0451
并且由命题 \ref{proposition-compact-is-perfect}，完美对象是紧的。
\end{proof}
...
$S$ 上代数空间的初等特异方形。
假设 $X$、$U$、$V$ 拟紧且拟分离。
由引理 \ref{lemma-better-coherator}，函子
...
这是把正合函子 $DQ_X$ 应用于引理
\ref{lemma-exact-sequence-j-star} 的特异三角，
并使用三次引理 \ref{lemma-pushforward-better-coherator} 得出的。
\end{remark}
...
设 $S$ 为概形。
设 $X$ 为 $S$ 上拟紧且拟分离的代数空间。
引理 \ref{lemma-better-coherator} 的函子 $DQ_X$
...
见引理 \ref{lemma-affine-coherator}。
...
由引理 \ref{lemma-quasi-coherence-direct-image} 在
...
最后，由评注 \ref{remark-explain-consequence} 的特异三角得出结论。
\end{proof}
...
并且由引理 \ref{lemma-boundedness-better-coherator}，
其负次数上同调层为零。
\end{example}
...
而由引理 \ref{lemma-quasi-coherence-pushforward-direct-sums}，
...
也成立。假设 (3)，并选取一个如定义
\ref{definition-tor-independent} 中的几何点图。
...
当 $X$、$Y$、$B$ 是概形时，定义
\ref{definition-tor-independent} 与《概形的导出范畴》定义
\ref{perfect-definition-tor-independent} 一致。

\medskip\noindent
设 $\overline{x}, \overline{b}, \overline{y}$ 如定义
\ref{definition-tor-independent} 中那样，位于点
$x, y, b$ 之上。回忆
...
$$
是同构；见引理 \ref{lemma-affine-morphism}
（并使用引理 \ref{lemma-quasi-coherence-pullback}、
\ref{lemma-quasi-coherence-tensor-product} 和
\ref{lemma-quasi-coherence-direct-image} 来说明对象
$Rf'_*L(g')^*E$ 和 $Lg^*Rf_*E$ 具有拟凝聚上同调层）。
注意，$g'$ 也是仿射的（《空间的态射》引理
\ref{spaces-morphisms-lemma-base-change-affine}）。
由引理 \ref{lemma-affine-morphism-pull-push}，该映射变为映射
$$
...
因此由引理 \ref{lemma-cohomology-base-change}，只须证明
...
\ref{lemma-tor-independent} 和《代数进阶》引理
...
$$
这是引理 \ref{lemma-affine-morphism-pull-push} 的结论。
在两种情形中，由引理 \ref{lemma-quasi-coherence-internal-hom}，
...
$$
它在两种情形中都是同构；引理
\ref{lemma-internal-hom-evaluate-tensor-isomorphism}。% P09-ERR-0461
为说明该引理适用于情形 (2)，注意
$g'_*\mathcal{O}_{X'} = Rg'_*\mathcal{O}_{X'} =
Lf^*g_*\mathcal{O}_X$，其中第二个等号由引理
\ref{lemma-compare-base-change} 给出。% P09-ERR-0462
使用《概形的导出范畴》引理
\ref{perfect-lemma-tor-dimension-affine}、引理
\ref{lemma-descend-tor-amplitude} 和《位点上的上同调》引理
...
$$
是同构。% P09-ERR-0463
注意，由引理 \ref{lemma-affine-morphism-pull-push}，左端等于
$R\Gamma(X', L(g')^*K)$。因此结论由引理
\ref{lemma-compare-base-change} 得出。
\end{proof}

\begin{remark}
\label{remark-multiplication-map}
沿用引理 \ref{lemma-affine-morphism-and-hom-out-of-perfect} 的记号。% P09-ERR-0460
下图交换：
$$
\xymatrix{
...
在第 \ref{section-cohomology-and-base-change-perfect} 节中，
...
\begin{enumerate}
\item 引理 \ref{lemma-single-complex-base-change-condition} 的等价条件成立；并且
\item $f$ 拟紧且拟分离，
\end{enumerate}
...
在这种情形中，将使用引理 \ref{lemma-induction-principle} 的归纳原理，
...
\ref{lemma-derived-quasi-coherent-small-etale-site}，$K$ 和 $K'$
来自底层概形导出范畴中的对象；而《概形的导出范畴》引理
\ref{perfect-lemma-single-complex-base-change-condition} 的条件
对这些复形成立，这是引理
\ref{lemma-single-complex-base-change-condition} 的结论。
因此（利用评注
\ref{remark-match-total-direct-images} 中说明的相容性），
...
（引理 \ref{lemma-unbounded-relative-mayer-vietoris}），
得到交换图
$$
\xymatrix{
...
\ref{lemma-quasi-coherence-tensor-product} 和
\ref{lemma-quasi-coherence-internal-hom}，以及《位点上的上同调》引理
...
(\ref{equation-bc}) 的源和靶，即可检验映射
(\ref{equation-bc}) 是同构；见《代数进阶》引理
...
为证明映射 (\ref{equation-bc}) 是同构，在情形 (2) 中，
...
$$
是同构。注意，在右端对 $\mathcal{G}^\bullet$ {f 不}使用导出拉回。
为证明这一点，应用引理 \ref{lemma-single-complex-base-change} 和
\ref{lemma-single-complex-base-change-condition-inherited}，可见只须证明典范映射
$$
L(g')^*\mathcal{G}^\bullet \to (g')^*\mathcal{G}^\bullet
$$
满足引理 \ref{lemma-single-complex-base-change-condition} 的等价条件。
...
$$
满足引理 \ref{lemma-single-complex-base-change-condition} 的等价条件。
这一点已在引理 \ref{lemma-base-change-tensor} 的证明中得到证明。
\end{proof}
...
\begin{proof}
由引理 \ref{lemma-direct-image-coherent}，$Rf_*E$ 是
$D^b_{\textit{Coh}}(\mathcal{O}_Y)$ 的对象。因此 $Rf_*E$ 拟凝聚
（引理 \ref{lemma-identify-pseudo-coherent-noetherian}）。
所以只须证明 $Rf_*E$ 具有有限 Tor 维数；见《位点上的上同调》引理
\ref{sites-cohomology-lemma-perfect}。
由引理 \ref{lemma-tor-qc-qs}，只须检验
...
由假设 (1)、(2) 和引理 \ref{lemma-union-closed-proper-over-base}，
...
$$
这由引理 \ref{lemma-cohomology-base-change} 和《位点上的上同调》引理
...
\begin{proof}
由引理 \ref{lemma-perfect-direct-image}，对象 $K$ 完美。
...
因此，由引理 \ref{lemma-tensor-perfect}，$K$ 是完美的。
\end{proof}
...
\noindent
引理 \ref{lemma-compute-ext}（或文献中的类似结果）
...
\ref{lemma-cohomology-base-change} 得出。

\begin{lemma}
\label{lemma-compute-tensor-perfect}
假设和记号如引理 \ref{lemma-tensor-perfect}。那么存在函子性同构
$$
...
第三个等号由引理 \ref{lemma-cohomology-base-change} 得出。
...
假设和记号如引理 \ref{lemma-ext-perfect}。% P09-ERR-0479
那么存在函子性同构
$$
H^i(B, K \otimes^\mathbf{L}_{\mathcal{O}_B} \mathcal{F})
\longrightarrow
...
\begin{proof}
如引理 \ref{lemma-ext-perfect} 的证明中那样，令
$E^\vee$ 为对偶完美复形，并回忆
...
\ref{lemma-compute-tensor-perfect} 应用于 $E^\vee$ 和
$\mathcal{G}^\bullet$ 得出。
关于边界映射的陈述含义如下：给定短正合列
$0 \to \mathcal{F}_1 \to \mathcal{F}_2 \to \mathcal{F}_3 \to 0$，
这些同构组成交换图
...
$(X, E, -m - 1 + a)$ 的完美复形逼近
（这由定理 \ref{theorem-approximation} 保证存在）。
于是诱导映射
$$
\Ext^i_{\mathcal{O}_X}(E,
\mathcal{G}^\bullet \otimes_{\mathcal{O}_X} f^*\mathcal{F})
...
\ref{lemma-compute-ext-perfect}。% P09-ERR-0482
关于边界的陈述已在引理
\ref{lemma-compute-ext-perfect} 的证明中说明。

\medskip\noindent
(B) 的证明。如 (A) 的证明中那样，可以假设 $X$ 是诺特的。
由引理 \ref{lemma-identify-pseudo-coherent-noetherian}，$E$ 拟凝聚。
由引理 \ref{lemma-pseudo-coherent-hocolim}，可以写成
...
$$
它在 $Y$ 上完美；见引理 \ref{lemma-tensor-perfect}。% P09-ERR-0483
由引理 \ref{lemma-compute-ext-perfect} 及其证明，以及上一段的论证
...
\ref{lemma-pseudo-coherent-hocolim}。同伦余极限的映射性质给出
...
拟凝聚复形 $L$（即引理 \ref{lemma-compute-ext} 的 (B) 部分中的复形）
...
在情形 \ref{situation-descent} 中，令 $E_0$ 和 $K_0$ 为
...
\ref{lemma-induction-principle} 的归纳原理证明 $P$ 对每个 $U_0$ 成立。
该引理的条件 (2) 立即由导出范畴中 Hom 的 Mayer--Vietoris 性质得出；
见引理 \ref{lemma-mayer-vietoris-hom}。
因此只需在 $X_0$ 仿射时证明本引理。

\medskip\noindent
若 $X_0$ 仿射，则结果由概形情形得出；见《概形的导出范畴》引理
\ref{perfect-lemma-descend-homomorphisms}。
为此，使用引理 \ref{lemma-derived-quasi-coherent-small-etale-site}
的等价，并使用引理
\ref{lemma-descend-pseudo-coherent}、
\ref{lemma-descend-tor-amplitude} 和
\ref{lemma-descend-perfect} 中说明的性质对应。
\end{proof}

\begin{lemma}
\label{lemma-perfect-on-limit}
在情形 \ref{situation-descent} 中，$D(\mathcal{O}_X)$ 的完美对象范畴
...
$U_i = X_i \times_{X_0} U_0$。我们将用引理
\ref{lemma-induction-principle} 的归纳原理证明 $P$ 对每个 $U_0$ 成立。
首先，由引理 \ref{lemma-descend-homomorphisms}，已经知道该函子全忠实。
因此只需证明本质满性。
...
$$
应用引理 \ref{lemma-glue}，得到对象 $E_i$（位于 $X_i$ 上）和映射
...
为此，使用引理
\ref{lemma-derived-quasi-coherent-small-etale-site} 的等价，
并使用引理 \ref{lemma-descend-perfect} 中说明的性质对应。
\end{proof}
...
\ref{section-cohomology-base-change} 和
\ref{section-producing-perfect} 节的讨论。
备注 \ref{remark-explain-perfect-direct-image} 给出了一个易于把握的特例。
...
\begin{proof}
关于基变换的陈述是引理 \ref{lemma-base-change-tensor}。
因此只需证明 $K$ 是完美对象。若 $Y$ 诺特，则这由引理
\ref{lemma-tensor-perfect} 得出。
我们将用诺特逼近把问题归约到这种情形。
建议读者跳过本证明的其余部分。
...
且支撑在 $R$ 上适当。由引理
\ref{lemma-base-change-tensor-perfect}，存在由有限投射
...
关于基变换的陈述是引理 \ref{lemma-base-change-tensor}。
因此只需证明 $K$ 是拟凝聚对象。
这将由引理 \ref{lemma-base-change-tensor-perfect}
...
$$
在次数 $\geq m$ 的上同调层上是同构。
由引理 \ref{lemma-base-change-tensor-perfect}，源是完美复形。
因此 $K$ 如所需地为 $m$-拟凝聚。
\end{proof}
...
\begin{proof}
这是引理 \ref{lemma-base-change-tensor-perfect} 的特例，分别取
...
\begin{proof}
这是引理 \ref{lemma-base-change-tensor-pseudo-coherent} 的特例，
...
（定理 \ref{theorem-bondal-van-den-Bergh}），只需对完美 $E$ 验证。
由引理 \ref{lemma-Rlim-quasi-coherent}，
...
$$
，这由引理 \ref{lemma-cohomology-base-change} 得出。
由引理 \ref{lemma-flat-proper-perfect-direct-image-general} 可知
...
\begin{proof}
关于基变换的陈述是引理 \ref{lemma-base-change-RHom}。
因此只需证明 $K$ 是完美对象。若 $Y$ 诺特，则这由引理
\ref{lemma-ext-perfect} 得出。
我们将用诺特逼近把问题归约到这种情形。
建议读者跳过本证明的其余部分。
...
见《空间的极限》引理
\ref{spaces-limits-lemma-eventually-proper-support}。
最后，由引理 \ref{lemma-descend-perfect}，增大 $i$ 后，
可以假设存在完美对象 $E_i$（属于 $D(\mathcal{O}_{X_i})$），
它到 $X$ 的拉回是 $E$。把引理
\ref{lemma-compute-ext-perfect} 应用于
...
\ref{lemma-descend-pseudo-coherent}），从而可以应用概形情形的结果；
...
\ref{lemma-open-where-cohomology-in-degree-i-rank-r} 立即得出。
\end{proof}
...
$Y$ 是仿射概形。
由上同调与基变换
（引理 \ref{lemma-flat-proper-perfect-direct-image-general}），复形
$E = Rf_*\mathcal{O}_X$
完美，并且它的构造与任意基变换可交换。
特别地，对 $y \in Y$ 有
$H^0(E \otimes^\mathbf{L} \kappa(y)) =
H^0(X_y, \mathcal{O}_{X_y}) = \kappa(y)$。
因此，用引理 \ref{lemma-jump-loci} 的记号，
$\beta_0(y) \leq 1$ 对所有 $y \in Y$ 成立。应用引理
\ref{lemma-locally-closed-where-H0-locally-free}，取
$a = 0$ 和 $r = 1$。得到泛闭子概形
...
\begin{proof}
由引理 \ref{lemma-proper-flat-h0}，只需证明
...
\ref{lemma-unbounded-mayer-vietoris}。
\end{proof}
...
\ref{theorem-DQCoh-is-Ddga} 及其证明，存在三角范畴等价
...
这由引理 \ref{lemma-countable-cohomology} 得出。
\end{proof}
...
\begin{proof}
由引理 \ref{lemma-cohomology-base-change}，
$$
R\Gamma(U, Lf^*K) = R\Gamma(X, Rf_*\mathcal{O}_U \otimes^\mathbf{L} K)
$$
对 $K$ 函子性成立。注意，$R\Gamma(X, -)$ 与同伦余极限可交换，
因为由引理 \ref{lemma-quasi-coherence-pushforward-direct-sums}，
...
\ref{spaces-limits-lemma-descend-flat}。
由引理 \ref{lemma-compare-base-change}，对象
...
为此应用引理 \ref{lemma-countable}。所以只需证明
$R^if_*\mathcal{O}_U$ 在平展于 $X$ 的仿射概形上的截面集合可数。
这由引理 \ref{lemma-countable-cohomology} 应用于结构层得出。
\end{proof}
...
这是引理 \ref{lemma-resolution-property-ample-relative} 的特殊情形。
\end{proof}
...
\noindent
本节将说明，第 \ref{section-generating} 节所构造的拟紧拟分离概形之
...
这由引理 \ref{lemma-ext-from-perfect-into-bounded-QCoh} 得出。
\end{proof}
...
回顾注 \ref{remark-classical-generator}：$G$ 是
...
\medskip\noindent
我们将利用引理 \ref{lemma-induction-principle} 的归纳原理，
证明等价条件 (2)--(7) 推出 (1)。
...
由引理 \ref{lemma-direct-summand-of-a-restriction} 和定理
\ref{theorem-bondal-van-den-Bergh}，存在完美复形 $Q$（在 $X$ 上），
...
$V$ 后是 $K$（见引理 \ref{lemma-pushforward-perfect} 和
\ref{lemma-pushforward-with-support-in-open}）。
...
因而两次使用引理 \ref{lemma-bounded-truncation} 可得
$$
...
（由引理 \ref{lemma-quasi-coherence-direct-image}），所以
...
（限制到 $U$ 和 $V$ 后检验典范映射是同构）。
因而两次使用引理 \ref{lemma-bounded-below-truncation} 可得
$$
\Hom_{D(\mathcal{O}_U)}(Q|_U [-i], E|_U) =
\Hom_{D(\mathcal{O}_X)}(Q[-i], R (U \to X)_* (E|_U)) = 0
$$
