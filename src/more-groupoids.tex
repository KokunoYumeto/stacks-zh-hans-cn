...
\end{equation}
其中 $[U/R]$ 是商叠，见 Groupoids in Spaces, Remark
\ref{spaces-groupoids-remark-fundamental-square}。
...
\noindent
我们继续沿用 Groupoids, Section
\ref{groupoids-section-notation} 中引入的约定与记号。
...
\noindent
为便于本章引用，我们简要重述 Groupoids, Lemmas
\ref{groupoids-lemma-diagram} 与
...
见 Schemes, Lemma \ref{schemes-lemma-section-immersion}。
考虑图
$$
\xymatrix{
...
警告：使用 Lemma \ref{more-groupoids-lemma-local-source} 时必须谨慎。
...
尽管有 Remark \ref{more-groupoids-remark-local-source-warning} 中的警告，
在某些情形下仍可使用 Lemma \ref{more-groupoids-lemma-local-source}，
...
见 Morphisms, Definition
\ref{morphisms-definition-relative-dimension-d} 以及
Morphisms, Lemmas
\ref{morphisms-lemma-openness-bounded-dimension-fibres} 和
\ref{morphisms-lemma-dimension-fibre-after-base-change}。
\item $\mathcal{Q} = \mathcal{R} =$“局部有限型”，且
$\mathcal{P} =$“局部拟有限”。
这是上一项中 $d = 0$ 的情形，见 Morphisms, Lemma
\ref{morphisms-lemma-locally-quasi-finite-rel-dimension-0}。
\item $\mathcal{Q} = \mathcal{R} =$“局部有限型”，且
$\mathcal{P} =$“非分歧”。
见 Morphisms, Lemmas
\ref{morphisms-lemma-unramified-characterize} 和
...
$\mathcal{P} =$“平坦”。见 More on Morphisms, Theorem
\ref{more-morphisms-theorem-openness-flatness} 和
Lemma \ref{more-morphisms-lemma-flat-locus-base-change}。
\item[(5)] $\mathcal{Q} =$“局部有限表示”，
$\mathcal{R} =$“平坦且局部有限表示”，且
$\mathcal{P}=$“Cohen--Macaulay”。见 More on Morphisms, Definition
\ref{more-morphisms-definition-CM} 以及 More on Morphisms, Lemmas
\ref{more-morphisms-lemma-base-change-CM} 和
\ref{more-morphisms-lemma-flat-finite-presentation-CM-open}。
\item[(6)] $\mathcal{Q} =$“局部有限表示”，
$\mathcal{R} =$“平坦且局部有限表示”，且
$\mathcal{P}=$“syntomic”。使用 Morphisms, Lemma
\ref{morphisms-lemma-set-points-where-fibres-lci}
（该轨迹自动为开集）。
\item[(7)] $\mathcal{Q} =$“局部有限表示”，
$\mathcal{R} =$“平坦且局部有限表示”，且
$\mathcal{P}=$“光滑”。见 Morphisms, Lemma
\ref{morphisms-lemma-set-points-where-fibres-smooth}
（该轨迹自动为开集）。
\item[(8)] $\mathcal{Q} =$“局部有限表示”，
$\mathcal{R} =$“平坦且局部有限表示”，且
$\mathcal{P}=$“\'etale”。见 Morphisms, Lemma
...
$\mathcal{P}$。则 $W$ 是 $R$-不变的，见 Groupoids, Definition
\ref{groupoids-definition-invariant-open}。
\end{lemma}

\begin{proof}
开集 $W \subset U$ 的存在性与性质见 Descent, Lemma
\ref{descent-lemma-largest-open-of-the-base}。
在图 (\ref{more-groupoids-equation-diagram}) 中，令 $W_1 \subset R$
为最大的开子概形，使态射
$\text{pr}_1 : R \times_{s, U, t} R \to R$
在其上具有性质 $\mathcal{P}$。由上述 Descent, Lemma
\ref{descent-lemma-largest-open-of-the-base} 以及
$\{s : R \to U\}$ 和 $\{t : R \to U\}$
...
$R$-不变的（见 Groupoids, Definition
\ref{groupoids-definition-invariant-open}）。
\end{lemma}

\begin{proof}
开集 $W \subset U$ 的存在性与性质见 Descent, Lemma
\ref{descent-lemma-largest-open-of-the-base}。态射
$$
G \times_{U, t} R \longrightarrow R \times_{s, U} G, \quad
(g, r) \longmapsto (r, r^{-1} \circ g \circ r)
$$
是 $R$ 上的同构（其中 $\circ$ 表示群胚中的合成）。
因此，由上述 Descent, Lemma
\ref{descent-lemma-largest-open-of-the-base} 所证明的
...
这是下面更一般且更精确的 Lemma \ref{more-groupoids-lemma-two-fibres}
的特殊情形；取 $r' = i(r)$ 即可看出。
...
由该图的性质以及 Schemes, Lemma
\ref{schemes-lemma-points-fibre-product}，存在
...
等价；见 Groupoids in Spaces, Section
\ref{spaces-groupoids-section-stacks} 以及 Section
\ref{spaces-groupoids-section-quotient-stack-restrict}。
...
\begin{proof}
在 Lemma \ref{more-groupoids-lemma-local-source} 中取
$g = \text{id}$、
$\mathcal{Q} =$“局部有限表示”、
$\mathcal{R} =$“平坦且局部有限表示”，以及
$\mathcal{P}=$“Cohen--Macaulay”；见 Remark
\ref{more-groupoids-remark-local-source-apply}。由此得到开集 $U' \subset U$，
...
这证明了 (3)；(4) 同理。(5) 是 (1)、(2) 以及 Groupoids, Section
\ref{groupoids-section-restrict-groupoid} 中关于限制的讨论
的形式推论。
\end{proof}
...
\begin{proof}
这由 Lemma \ref{more-groupoids-lemma-double-restrict} 中图的性质形式地得出。
...
因此，本节可视为 Groupoids, Section
\ref{groupoids-section-properties-group-schemes-field} 的对应版本。
...
稳定子群概形 $G \to U$ 分离。由 Groupoids, Lemma
\ref{groupoids-lemma-diagonal}，态射
$j = (t, s) : R \to U \times_S U$ 分离。
由于 $U$ 是域的谱，概形 $U \times_S U$ 仿射
（见 Schemes, Section \ref{schemes-section-fibre-products}
中纤维积的构造）。因此 $R$ 分离，见 Schemes, Lemma
...
\begin{proof}
这是 Lemma \ref{more-groupoids-lemma-two-fibres} 的 (1)、(2) 两项的特殊情形。
\end{proof}
...
因而仿射、满且平坦。由 Morphisms, Lemma
\ref{morphisms-lemma-scheme-over-field-universally-open}，
态射 $s, t : R \to U$ 与 $U' \to U$ 均普遍开。
由于 $R$ 非空且 $U$ 是域的谱，态射 $s, t : R \to U$
满且平坦。最后应用 Morphisms, Lemmas
...
是经过 $e$ 的不可约分支。由 Varieties, Lemma
...
$Z \times_k k'$ 的任意不可约分支。由 Varieties, Lemma
\ref{varieties-lemma-orbit-irreducible-components}，
...
Morphisms, Lemma
\ref{morphisms-lemma-dimension-fibre-after-base-change}，
有 $\dim_r(R) = \dim_{r'}(R)$。由 Morphisms, Lemma
...
回忆 $k'_s$ 和 $k'_t$ 都是域，见 Varieties, Lemma
\ref{varieties-lemma-integral-closure-ground-field}。
由 Varieties, Proposition
\ref{varieties-proposition-unique-base-field}，
...
Groupoids, Lemma
\ref{groupoids-lemma-group-scheme-finite-type-field}。
\end{proof}
...
$R \to U$ 的微分层自由。因此光滑性由 Varieties, Lemma
\ref{varieties-lemma-char-zero-differentials-free-smooth} 得出。
\end{proof}
...
\begin{proof}
由引理 \ref{more-groupoids-lemma-sheaf-differentials}，
层 $\Omega_{R/U}$ 自由。因此引理由 Varieties, Lemma
\ref{varieties-lemma-char-p-differentials-free-smooth} 得出。
\end{proof}
...
定义 $S$ 上群胚概形的态射，见 Groupoids, Definition
\ref{groupoids-definition-groupoid}。特别地，下列图交换：
$$
\vcenter{
\xymatrix{
...
\noindent
下一个引理是 Groupoids, Lemma
\ref{groupoids-lemma-open-subgroup-closed-over-field} 的推广。
...
记号和假设同情形 \ref{more-groupoids-situation-morphism-groupoids-on-field}。
设 $Z \subset R_2$ 为约化闭子概形（见 Schemes, Definition
\ref{schemes-definition-reduced-induced-scheme}），其底拓扑空间是
...
$$
由 Varieties, Lemma
\ref{varieties-lemma-closure-image-product-map}，
左侧竖直箭头之像的闭包在集合论意义下是
...
$a : R_1 \to R_2$ 的概形论像（见 Morphisms, Definition
\ref{morphisms-definition-scheme-theoretic-image}）。则
$$
(U, Z, s_2|_Z, t_2|_Z, c_2|_Z)
$$
是 $S$ 上的群胚概形。
...
$$
由 Varieties, Lemma
\ref{varieties-lemma-scheme-theoretic-image-product-map}，
$a \times a$ 的概形论像是
...
的有限型点，见 Morphisms, Definition
\ref{morphisms-definition-finite-type-point}。采用情形
...
Morphisms, Lemma
\ref{morphisms-lemma-dimension-fibre-at-a-point} 给出。
...
由 Groupoids, Lemma
\ref{groupoids-lemma-restrict-stabilizer}，有 $G'' = G_u$。
...
Algebra, Lemma \ref{algebra-lemma-CM-dim-formula}。注意
...
由素理想回避，见 Algebra, Lemma \ref{algebra-lemma-silly}，
...
可见 $t(f)$ 在其上不是零因子；例如见 Algebra, Lemma
\ref{algebra-lemma-reformulate-CM}。因此由 Algebra, Lemma
\ref{algebra-lemma-grothendieck-general}，可知
$s : A_{\mathfrak m} \to B_{\mathfrak q}/t(f)B_{\mathfrak q}$
平坦，其纤维环为
$B_{\mathfrak q}/(s(\mathfrak m)B_{\mathfrak q} + t(f)B_{\mathfrak q})$；
由 Algebra, Lemma \ref{algebra-lemma-reformulate-CM}，
...
而由 Algebra, Lemma \ref{algebra-lemma-reformulate-CM}，
...
Cohen--Macaulay 且局部有限表现的。设 $u \in U$ 是概形 $U$
的有限型点，见 Morphisms, Definition
\ref{morphisms-definition-finite-type-point}。
...
（使用 Morphisms, Lemma
\ref{morphisms-lemma-base-change-finite-presentation} 及
...
注意 $u$ 也是 $U'$ 的有限型点（例如可使用 Morphisms, Lemma
\ref{morphisms-lemma-identify-finite-type-points}
对有限型点的刻画看出这一点）。设
$g' : U'' \to U'$ 和 $(U'', R'', s'', t'', c'')$
...
（使用 More on Morphisms, Lemma
\ref{more-morphisms-lemma-CM-local-source-and-target}
...
\noindent
本节开始把 More on Morphisms, Section
\ref{more-morphisms-section-etale-localization}
中的平展（\'etale）局部化技术应用于群胚概形。
这方面更深入的材料见 More on Groupoids in Spaces,
...
即 Morphisms of Spaces, Proposition
\ref{spaces-morphisms-proposition-locally-quasi-finite-separated-over-scheme}
及其推论 Morphisms of Spaces, Lemma
...
见 Morphisms, Lemma \ref{morphisms-lemma-finite-fibre}。
因此 $f \circ s : R \to S$ 在每个 $r_i$ 处拟有限
（Morphisms, Lemma
\ref{morphisms-lemma-composition-quasi-finite}）。
所以 $r_i$ 在纤维 $R_p$ 中是孤立点，见 Morphisms, Lemma
...
（Morphisms, Lemmas \ref{morphisms-lemma-base-change-flat} 和
\ref{morphisms-lemma-base-change-finite-presentation}），
因而为开映射（Morphisms, Lemma
\ref{morphisms-lemma-fppf-open}）。
每个 $r_i$ 在 $R_p$ 中孤立这一事实蕴含每个
$u_j = t(r_i)$ 在 $U_p$ 中孤立。再次使用 Morphisms, Lemma
\ref{morphisms-lemma-quasi-finite-at-point-characterize}，
...
且只有有限多个点（例如，这可由远为一般的 Morphisms, Lemma
...
见 Groupoids, Lemma
\ref{groupoids-lemma-restrict-groupoid-relation} 及其证明。
\end{proof}
...
态射
$s^{-1}(Z_{r - 1}) \setminus s^{-1}(Z_r) \to Z_{r - 1} \setminus Z_r$
有限局部自由且秩为 $r$，因为由 Divisors, Lemma
\ref{divisors-lemma-locally-free-rank-r-pullback}，
...
$U = Z_0 \supset Z_1 \supset Z_2 \supset \ldots$
由有限表现的闭浸入 $Z_k \to U$ 给出，见 Divisors, Lemma
\ref{divisors-lemma-locally-free-rank-r-pullback}。
...
$Z_k \setminus Z_{k + 1}$（Properties, Lemma
\ref{properties-lemma-generic-point-in-constructible}）。
...
$\tilde W \to W$ 整且满；由 Limits, Proposition
\ref{limits-proposition-affine}，这蕴含 $W$ 仿射。
然而，每个轨道与 $U'$ 相交这一假设蕴含
$h : \tilde R \to U$ 满。整满态射为商映射
（Topology, Lemma
\ref{topology-lemma-closed-morphism-quotient-topology} 及
Morphisms, Lemma
...
$R$-轨道的所有点处均不消失
（Algebra, Lemma \ref{algebra-lemma-silly}）。考虑
$$
...
确实，在 Groupoids, Lemma
\ref{groupoids-lemma-determinant-trick} 中已经看到，
...
$r \leadsto r'$，满足 $s(r') = u'$，见 Schemes, Lemma
\ref{schemes-lemma-quasi-compact-closed}。
令 $u'' = t(r')$。注意 $u'' \not = u'$，因为有限态射的纤维中
...
\begin{proof}
这可由 Morphisms, Lemma
\ref{morphisms-lemma-get-affine} 得出，但我们也给出直接证明。
令 $A' = \Gamma(V, \mathcal{O}_V)$。则
$j' : V \to \Spec(A')$ 是拟紧开浸入，见 Properties, Lemma
\ref{properties-lemma-quasi-affine}。设 $f' \in A'$ 为 $f$ 的像。
则 $(j')^{-1}D(f') = j^{-1}D(f)$ 仿射。另一方面，
$j'(V) \cap V(f')$ 是 $\Spec(A')$ 的子概形，它同
$\Spec(A)$ 的闭子概形 $j(V) \cap V(f)$ 同构。
因此它在 $\Spec(A')$ 中闭，例如可见 Schemes, Lemma
...
$j^{-1}D(f)$ 的主开集，因而仿射。于是再次由 Properties, Lemma
\ref{properties-lemma-invert-f-affine}，可见 $D(g)$ 同构于
...
的一个仿射开集 $U'$，见 Varieties, Proposition
...
\medskip\noindent
若 $u \in W$，则由 Groupoids, Lemma
\ref{groupoids-lemma-find-invariant-affine} 已得结论
...
\noindent
ind-拟仿射态射定义于 More on Morphisms, Section
\ref{more-morphisms-section-ind-quasi-affine}。
本节是 Descent, Section
\ref{descent-section-quasi-affine} 对 ind-拟仿射态射的对应版本。
...
于是 $X$ 拟紧且拟分离，因而是谱空间，见 Properties, Lemma
\ref{properties-lemma-quasi-compact-quasi-separated-spectral}。
再由 Topology, Lemma
\ref{topology-lemma-make-spectral-space} 及 Sheaves, Lemma
\ref{sheaves-lemma-descend-opens} 可见该等式成立。
（事实上，若 $\mathcal{F}$ 为 Abel 层，该公式对高阶上同调群也成立；
见 Cohomology, Lemma \ref{cohomology-lemma-colimit}。）
...
\begin{proof}
注意 $(E, \mathcal{O}_X|_E)$ 是局部环化空间，所以 Schemes, Lemma
...
$\Spec(A) = \lim \Spec(A_i)$（Limits, Lemma
\ref{limits-lemma-topology-limit}）。由于
$E = \lim U_i$（由 Topology, Lemma
\ref{topology-lemma-make-spectral-space}），可见
...
$\{X_i \to S\}$ 的下降数据，见 Descent, Definition
...
因此 Descent, Lemma
\ref{descent-lemma-descending-types-morphisms} 适用；
...
由 Groupoids, Lemma
\ref{groupoids-lemma-cartesian-equivalent-descent-datum}，
二元组 $(V, \varphi)$ 对应于群胚概形的 Cartesian 态射
$(U', R', s', t', c') \to (U, R, s, t, c)$。
任取一点 $u' \in U'$。由 Groupoids, Lemmas
...
$\psi$ 的余圈条件，如 Descent, Definition
\ref{descent-definition-descent-datum-modules} 所述
（略去细节）。由 Descent, Proposition
\ref{descent-proposition-descent-module}，得到 $R$-代数 $R'$
及 $A$-代数同构
...
下的逆像相等。由 Descent, Lemma
\ref{descent-lemma-open-fpqc-covering}，得到开集
