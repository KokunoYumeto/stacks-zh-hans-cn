...
$T/S$ 具有函子性。米田引理（见 Categories,
Lemma \ref{categories-lemma-yoneda}）说明，$f$ 决定函子变换
...
\noindent
设 $(G, m)$ 是概形 $S$ 上的群概形。
由上述讨论（以及 Section \ref{groupoids-section-notation} 中的讨论），
可得到 $S$ 上的概形态射：
...
\begin{proof}
回顾由 Schemes, Lemma \ref{schemes-lemma-section-immersion}，
...
参见 Schemes, Lemma \ref{schemes-lemma-base-change-immersion}
（相应地，\ Schemes, Lemma
...
$$
左上方的水平箭头是同构，方块为笛卡尔图。
因此结论由 Morphisms, Lemma \ref{morphisms-lemma-base-change-flat} 得出。
\end{proof}
...
\begin{proof}
由 Morphisms, Lemma \ref{morphisms-lemma-base-change-differentials} 有
$$
...
Varieties, Lemma \ref{varieties-lemma-map-tangent-spaces} 诱导。
\end{lemma}
...
还可以得到更多结论；参见 Section \ref{groupoids-section-algebraic-group-schemes}。
...
由 Lemma \ref{groupoids-lemma-group-scheme-separated}，只需证明
$e : S \to G$ 是闭浸入。根据 Morphisms, Lemma
\ref{morphisms-lemma-algebraic-residue-field-extension-closed-point-fibre}，
$e : S \to G$ 的像是 $G$ 的闭点。
显然，$\mathcal{O}_G \to e_*\mathcal{O}_S$ 是满射，因为
$e_*\mathcal{O}_S$ 是支集在 $G$ 的单位元处、取值为 $k$ 的摩天层。
由 Schemes, Lemma \ref{schemes-lemma-characterize-closed-immersions}，
$e$ 是闭浸入。
\end{proof}
...
是忠实平坦的；见 Algebra, Lemma \ref{algebra-lemma-local-flat-ff}）。
...
警告：Lemma \ref{groupoids-lemma-group-scheme-field-geometrically-irreducible}
...
\begin{proof}
证明略。提示：利用 Varieties, Lemmas
\ref{varieties-lemma-perfect-reduced} 和
...
由 Lemma \ref{groupoids-lemma-group-scheme-over-field-translate-open}，映射
$$
...
由 Varieties, Lemma \ref{varieties-lemma-closure-of-product}，
...
由 Lemma \ref{groupoids-lemma-open-subgroup-closed-over-field}，
...
$$
的像中。根据 Lemma
\ref{groupoids-lemma-group-scheme-over-field-open-multiplication}，该像是开的。
...
约化且不可约（Varieties, Lemmas
\ref{varieties-lemma-geometrically-reduced-any-base-change} 和
...
由 Lemma \ref{groupoids-lemma-group-scheme-field-geometrically-irreducible}，
$Z \cap Z' = \emptyset$。由
Lemma \ref{groupoids-lemma-irreducible-group-scheme-over-field-qc}，$Z$ 拟紧。
于是可以选取拟紧开集 $U \subset G$，使得
$Z \subset U$ 且 $U \cap Z' = \emptyset$。
由 Lemma \ref{groupoids-lemma-group-scheme-over-field-translate-open}，
$Z \times_k U \to G$ 的像 $W$ 在 $G$ 中是开的。
另一方面，$W$ 是拟紧空间的像，因而拟紧。我们断言 $W$ 是闭的。

\medskip\noindent
证明该断言。由于 $W$ 拟紧而 $G$ 分离
（Lemma \ref{groupoids-lemma-group-scheme-over-field-separated}），根据
Schemes, Lemma \ref{schemes-lemma-quasi-compact-permanence}，
包含映射 $W \to G$ 是拟紧的。因此，$W$ 闭包中的点都是
$W$ 中某些点的特化（Morphisms, Lemma
...
为证明 $G^0$ 是闭子群概形，使用
Lemma \ref{groupoids-lemma-closed-subgroup-scheme} 的判据。
态射 $e : \Spec(k) \to G$ 分解通过 $G^0$，因为我们所取的
$G^0$ 正是 $G$ 中包含 $e$ 的连通分支。
由于 $i : G \to G$ 是固定 $e$ 的自同构，$i$ 把 $G^0$ 映入自身。
根据 Varieties, Lemma
\ref{varieties-lemma-geometrically-connected-criterion}，
...
由 Lemma \ref{groupoids-lemma-connected-group-scheme-over-field-irreducible}，
$G^0$ 不可约；由
Lemma \ref{groupoids-lemma-group-scheme-field-geometrically-irreducible}，
$G^0$ 几何不可约；再由
...
Limits, Lemma \ref{limits-lemma-topology-limit}。类似地，
\begin{align*}
|T \times_k X| & =
|\Spec(A \otimes_k B)| \\
& =
...
置于某个仿射开集中。强烈建议读者先阅读
Lemma \ref{groupoids-lemma-points-in-affine}。
...
\begin{proof}
令 $G^0 \subset G$ 为 Proposition
\ref{groupoids-proposition-connected-component} 中得到的闭子群概形。
前两段将问题约化到 $G = G^0$ 的情形。

\medskip\noindent
注意，$T$ 是有限拓扑空间的有向逆极限
（Limits, Lemma \ref{limits-lemma-topology-limit}），
因而作为拓扑空间是预有限的
（Topology, Definition \ref{topology-definition-profinite}）。
...
$$
空间 $\pi_0(W)$ 是预有限的
（Topology, Lemma \ref{topology-lemma-spectral-pi0} 以及
Properties, Lemma
...
由 Topology, Lemma \ref{topology-lemma-closed-map}，$\psi$ 是闭映射，
...
的延拓是同构。由对 $T$ 的假设及
Lemma \ref{groupoids-lemma-profinite-product-over-field}，有
...
由 Proposition \ref{groupoids-proposition-connected-component}，
...
故 $G'$ 是开子群概形。由
Lemma \ref{groupoids-lemma-open-subgroup-closed-over-field}，它也是闭子群概形。
\end{proof}
...
\begin{proof}
由 Lemma \ref{groupoids-lemma-group-scheme-module-differentials}，
...
我们已在 Lemma \ref{groupoids-lemma-group-scheme-characteristic-zero-smooth}
...
\noindent
下一个引理是 Lemma \ref{groupoids-lemma-compact-set-in-affine} 的特殊情形，
其证明稍为简单。
...
Proposition \ref{groupoids-proposition-connected-component} 所定义。
...
\begin{proof}
由 Varieties, Lemma \ref{varieties-lemma-ample-after-field-extension}，
可假设 $k$ 代数闭。令 $G^0 \subset G$ 为 Proposition
\ref{groupoids-proposition-connected-component} 中的 $G$ 的连通分支。
...
由 Lemma \ref{groupoids-lemma-reduced-subgroup-scheme-perfect}，
$H$ 是 $k$ 上的群概形；由
...
（Varieties, Lemma \ref{varieties-lemma-smooth-regular} 以及
More on Algebra, Lemma \ref{more-algebra-lemma-regular-local-UFD}）。
$G$ 的一个非空仿射开集的补集，是某个有效 Cartier 除子 $D$ 的支集；
这由 Divisors, Lemma
\ref{divisors-lemma-complement-open-affine-effective-cartier-divisor} 得到。
（由 Lemma \ref{groupoids-lemma-group-scheme-over-field-separated}，$G$ 分离。）
...
考虑闭子集\footnote{利用 Divisors, Section
\ref{divisors-section-norms} 的材料，可以把有效 Cartier 除子
...
这些分支都是有效 Cartier 除子（Divisors, Lemma
\ref{divisors-lemma-weil-divisor-is-cartier-UFD} 以及上引诸引理），
...
由 Varieties, Lemma \ref{varieties-lemma-rational-equivalence-for-Pic}，
...
$$
最后一个群值函子是可表示的；见
Example \ref{groupoids-example-general-linear-group}。并且最后一个箭头显然为单射。
因此，对每个 $n$ 都得到闭子群概形
$$
...
令 $G^0 \subset G$ 如 Proposition
\ref{groupoids-proposition-connected-component} 所定义。
由于 $G^0$ 几何不可约，可断定 $K$ 包含 $G^0_S$
（对任意非空开集 $U \subset G^0_{k'}$ 和任意域扩张 $k'/k$，
都有 $U \cdot U^{-1} = G^0_{k'}$；见
Lemma \ref{groupoids-lemma-irreducible-group-scheme-over-field-qc} 的证明）。
...
$$
所以由 Descent, Lemma \ref{descent-lemma-closed-immersion}
（以及 Descent, Lemma
...
Remark \ref{groupoids-remark-16-equivalent}。}。
\end{definition}
...
\begin{proof}
这由 Lemma \ref{groupoids-lemma-algebraic-quasi-projective} 以及
More on Morphisms, Lemma \ref{more-morphisms-lemma-projective} 得出。
\end{proof}
...
\begin{proof}
若 $k$ 完美，则该结论由
Lemma \ref{groupoids-lemma-group-scheme-characteristic-zero-smooth}（特征零）
以及 Lemma
...
以及 Lemma \ref{groupoids-lemma-abelian-variety-change-field} 转到完美闭包，
从而约化到上述情形。
\end{proof}
...
\begin{proof}
设 $k$ 为域，$A$ 为 $k$ 上的阿贝尔簇。
由 Lemma \ref{groupoids-lemma-abelian-variety-change-field}，可把 $k$
替换为其代数闭包。考虑态射
$$
...
\begin{proof}
把立方定理（More on Morphisms, Theorem
\ref{more-morphisms-theorem-of-the-cube}）应用于差
$$
\mathcal{M} =
m_{1, 2, 3}^*\mathcal{L} \otimes
m_1^*\mathcal{L} \otimes
...
拉回 Lemma \ref{groupoids-lemma-apply-cube} 中的关系，得到
$$
\mathcal{L} \otimes
\mathcal{L} \otimes
\mathcal{L} \otimes
...
\begin{proof}
由 Lemma \ref{groupoids-lemma-abelian-variety-projective}
（以及 More on Morphisms, Lemma
\ref{more-morphisms-lemma-projective}），$A$ 有一个充足可逆模
$\mathcal{L}$。因为 $[-1] : A \to A$ 是自同构，
$[-1]^*\mathcal{L}$ 也是充足可逆 $\mathcal{O}_A$-模。
因此 $\mathcal{N} = \mathcal{L} \otimes [-1]^*\mathcal{L}$ 充足；
见 Properties, Lemma
\ref{properties-lemma-ample-tensor-globally-generated}。
由于 $\mathcal{N} \cong [-1]^*\mathcal{N}$，由
Lemma \ref{groupoids-lemma-pullbacks-by-n} 有
...
Varieties, Lemma \ref{varieties-lemma-ample-curve} 矛盾。
（也可使用 Varieties, Lemma \ref{varieties-lemma-ample-positive}。）
因此 $[d]$ 的每个纤维维数都是 $0$，故例如由
Cohomology of Schemes, Lemma \ref{coherent-lemma-characterize-finite}，
$[d]$ 有限。此外，由 Lemma \ref{groupoids-lemma-abelian-variety-smooth}，
$A$ 在 $k$ 上光滑，因而根据
Algebra, Lemma \ref{algebra-lemma-CM-over-regular-flat}，
$[d] : A \to A$ 平坦（这里还使用了域上光滑的概形正则，以及
正则环是 Cohen--Macaulay 环；见
Varieties, Lemma \ref{varieties-lemma-smooth-regular} 和
Algebra, Lemma \ref{algebra-lemma-regular-ring-CM}）。
所以由 Morphisms, Lemma \ref{morphisms-lemma-finite-flat}，
$[d]$ 有限局部自由。

\medskip\noindent
最后计算次数。由 Varieties, Lemma
\ref{varieties-lemma-degree-finite-morphism-in-terms-degrees}，
$$
...
已知 $[d]$ 有限局部自由
（Lemma \ref{groupoids-lemma-degree-multiplication-by-d}），故它在 $e$ 处
平展（\'etale），等价于证明
$\text{d}[d] : T_{A/k, e} \to T_{A/k, e}$ 是单射；见
Varieties, Lemma \ref{varieties-lemma-injective-tangent-spaces-unramified}
以及 Morphisms, Lemma \ref{morphisms-lemma-flat-unramified-etale}。
由 Lemma \ref{groupoids-lemma-group-scheme-addition-tangent-vectors}，
...
Lemma \ref{groupoids-lemma-group-scheme-addition-tangent-vectors}，
$[p]$ 的导数作为映射 $T_{A/k, e} \to T_{A/k, e}$
是乘以 $p$，因而为零（比较 Lemma
\ref{groupoids-lemma-abelian-variety-multiplication-by-d-etale} 的证明）。
...
（More on Algebra, Definition \ref{more-algebra-definition-p-basis}
以及 Lemma \ref{more-algebra-lemma-p-basis}）。
由 Algebra, Lemma \ref{algebra-lemma-derivative-zero-pth-power}，
$[p]^*(t_i)$ 有一个 $p$ 次根。因此，
...
由 Lemma \ref{groupoids-lemma-degree-multiplication-by-d}，$[p]$ 的次数为
...
\begin{proof}
(1) 由 Lemma \ref{groupoids-lemma-abelian-variety-projective} 得出。
(2) 由 Lemma \ref{groupoids-lemma-abelian-variety-abelian} 得出。
(3) 由 Lemma \ref{groupoids-lemma-degree-multiplication-by-d} 得出。
若 $k$ 代数闭，则 $k$ 上簇的满态射在 $k$-有理点上诱导满射，
故由 (3) 得 (4)。
(5) 由 Lemma \ref{groupoids-lemma-degree-multiplication-by-d} 以及如下事实得出：
次数为 $N$ 的有限局部自由态射经基变换后，仍是次数为 $N$
的有限局部自由态射。(6) 由
Lemma \ref{groupoids-lemma-abelian-variety-multiplication-by-d-etale} 得出。
具体地，若 $n$ 在 $k$ 中可逆，则 $[n]$ 平展（\'etale），
因而 $A[n]$ 在 $k$ 上平展（\'etale）。
另一方面，若 $n$ 在 $k$ 中不可逆，则 $[n]$ 在 $e$ 处不平展
（\'etale），从而 $A[n]$ 在 $k$ 上的 $e$ 处不平展（\'etale）
（使用 Morphisms, Lemmas
\ref{morphisms-lemma-flat-unramified-etale} 和
...
使用 Lemma \ref{groupoids-lemma-abelian-variety-multiplication-by-p} 所给出的
...
便恢复 Definition \ref{groupoids-definition-abelian-variety}
（使用 Varieties, Lemma \ref{varieties-lemma-geometrically-integral}）。
最弱的选择是“固有、连通、几何约化”；例如参见
Morphisms, Lemma \ref{morphisms-lemma-locally-projective-proper} 和
Varieties, Lemma \ref{varieties-lemma-smooth-geometrically-normal}。
因此，设 $A$ 是 $k$ 上固有、连通且几何约化的群概形。
由 Lemmas \ref{groupoids-lemma-connected-group-scheme-over-field-irreducible} 和
\ref{groupoids-lemma-group-scheme-field-geometrically-irreducible}，
$A$ 几何不可约，因而是阿贝尔簇。
最后，若 $A/k$ 是阿贝尔簇，则由 Proposition
\ref{groupoids-proposition-review-abelian-varieties}，它在 $k$ 上射影且光滑，
...
设 $S$、$G \to S$、$X \to S$ 如
Definition \ref{groupoids-definition-action-group-scheme} 所述。
...
情形如 Definition \ref{groupoids-definition-free-action}。
作用 $a$ 自由，当且仅当
$$
...
\noindent
在 Cohomology on Sites, Definition
\ref{sites-cohomology-definition-torsor} 中，我们定义了一个位点上
群层的扭子。设 $\tau \in \{Zariski, \etale, smooth, syntomic, fppf\}$
为一种拓扑，而 $(G, m)$ 是 $S$ 上的群概形。
由于 $\tau$ 强于典范拓扑（见 Descent, Lemma
\ref{descent-lemma-fpqc-universal-effective-epimorphisms}），
$\underline{G}$（见 Sites, Definition
\ref{sites-definition-representable-sheaf}）是
...
情形如 Definition \ref{groupoids-definition-pseudo-torsor}。
\begin{enumerate}
...
（由 Descent, Lemma \ref{descent-lemma-affine} 得出）。
若 $G = \text{GL}_{n, S}$，则 (3) 的答案为肯定；
事实上，任意 $\text{GL}_{n, S}$-扭子都局部平凡
（由 Descent, Lemma
\ref{descent-lemma-finite-locally-free-descends} 得出）。
\end{remark}
...
$$
其中 $m(x) = x \otimes x$；见 Example
\ref{groupoids-example-multiplicative-group}。在齐次元上求值便立即可见。
设 $M$ 是分次 $A$-模。使用 Definition
\ref{groupoids-definition-equivariant-module} 中的 $\alpha$，即可得到
...
Example \ref{groupoids-example-Gm-on-affine} 所述。
\end{lemma}
...
$$
（比较 Example \ref{groupoids-example-Gm-on-affine} 中的计算）。
...
设 $A$ 为分次环，$X = \Spec(A)$ 带有 Example
\ref{groupoids-example-Gm-on-affine} 中的作用
...
Example \ref{groupoids-example-Gm-on-affine}。
\end{lemma}

\begin{proof}
可以对模 $M$ 重复 Lemma \ref{groupoids-lemma-complete-reducibility-Gm}
的论证。也可以考虑概形
...
Lemma \ref{groupoids-lemma-complete-reducibility-Gm} 即得结论。
...
$$
这与上文 Section \ref{groupoids-section-equivalence-relations} 中的约定相容。
...
Lemma \ref{groupoids-lemma-diagram} 得出。
\end{proof}
...
是恒等态射。
\end{enumerate}
可与 Lemma \ref{groupoids-lemma-diagram} 中的交换图比较。
\end{definition}
...
Definition \ref{groupoids-definition-groupoid-module} 中的交换图。
...
Lemma \ref{groupoids-lemma-pullback} 所定义的拉回的右伴随。
\end{lemma}
...
Schemes, Lemma \ref{schemes-lemma-push-forward-quasi-coherent}。
因此 $s_*t^*\mathcal{G}$ 拟凝聚。采用 Lemma \ref{groupoids-lemma-diagram}
中的记号，有
$$
t^*s_*t^*\mathcal{G} =
...
基变换与推前交换，见 Cohomology of Schemes, Lemma
\ref{coherent-lemma-flat-base-change-cohomology}。

\medskip\noindent
为了验证 $\alpha$ 满足 Definition \ref{groupoids-definition-groupoid-module}
...
见 Lemma \ref{groupoids-lemma-equivalence-groupoid}。存在群胚概形态射
$$
f :
...
Lemma \ref{groupoids-lemma-diagram-pull} 有笛卡尔方块
$$
\xymatrix{
...
$$
故 Lemma \ref{groupoids-lemma-pushforward} 适用于 $f$。
...
$(r_0, r_1) \mapsto r_0^{-1} \circ r_1$ 的拉回。
为此，只需取 Definition \ref{groupoids-definition-groupoid-module}
中实际的上循环关系，并沿态射
$(\text{pr}_0, c \circ (i, 1)) : R \times_{t, U, t} R \to R \times_{s, U, t} R$
将其拉回；该态射也出现在
Lemma \ref{groupoids-lemma-diagram-pull} 的交换图中。因此
...
在 Lemma \ref{groupoids-lemma-construct-quasi-coherent} 的情形下，记遗忘函子为
$$
...
\begin{proof}
我们将直接使用 Cohomology of Schemes, Lemma
\ref{coherent-lemma-coherent-Noetherian} 对凝聚模的刻画，
...
\begin{proof}
我们将不再特别说明而使用 Cohomology of Schemes, Lemma
\ref{coherent-lemma-coherent-Noetherian} 对局部 Noether 概形上凝聚模的刻画。
由 Cohomology of Schemes, Lemma
\ref{coherent-lemma-directed-colimit-coherent}，可将
$\mathcal{F} = \colim \mathcal{H}_i$ 写成凝聚子模
$\mathcal{H}_i \subset \mathcal{F}$ 的滤过余极限。
给定 $U$ 上拟凝聚层 $\mathcal{H}$，以
$(s_*t^*\mathcal{H}, \alpha)$ 表示
Lemma \ref{groupoids-lemma-construct-quasi-coherent} 给出的
$(U, R, s, t, c)$ 上拟凝聚层。考虑
$\QCoh(U, R, s, t, c)$ 中的伴随态射
$(\mathcal{F}, \beta) \to (s_*t^*\mathcal{F}, \alpha)$，
见 Remark \ref{groupoids-remark-adjunction-map}。令
$$
(\mathcal{F}_i, \beta_i) =
(\mathcal{F}, \beta)
...
Lemma \ref{groupoids-lemma-abelian} 的证明，限制到 $U$ 是
...
由 Remark \ref{groupoids-remark-adjunction-map} 中对伴随态射的描述，
该图同构于
$$
\xymatrix{
...
Lemma \ref{groupoids-lemma-push-pull}，$\mathcal{F}_i$ 凝聚。
最后，因为 $s$ 拟紧且拟分离，$s_*$ 与余极限交换
（见 Cohomology of Schemes, Lemma
\ref{coherent-lemma-colimit-cohomology}）。故
...
$$
Definition \ref{groupoids-definition-groupoid-module} 中图的交换性意味着
$$
...
（可能为空的）集合。由 Properties, Lemma
\ref{properties-lemma-set-of-iso-classes}，
...
Properties, Lemma \ref{properties-lemma-colimit-kappa} 的证明。
我们建议读者先阅读该证明。
...
$$
见 Schemes, Lemma \ref{schemes-lemma-widetilde-pullback}。
利用选择公理，我们选择映射
$$
...
Lemma \ref{groupoids-lemma-groupoid-from-action} 构造的群胚概形。
...
Definitions \ref{groupoids-definition-equivariant-module} 和
\ref{groupoids-definition-groupoid-module}。
采用 Lemma \ref{groupoids-lemma-groupoid-from-action} 中的转译，
两个定义对交换性的要求完全一致。
\end{proof}
...
$$
并把它视为 $U \times_S U$ 上的群概形。Lemma
\ref{groupoids-lemma-groupoid-action-stabilizer} 中 $G$ 在 $R$ 上的作用
...
$$
（见 Lemma \ref{groupoids-lemma-groupoid-action-stabilizer-pseudo-torsor}）
...
设 $S$ 为概形，$(U, R, s, t, c)$ 为 $S$ 上的群胚概形，
$g : U' \to U$ 为概形态射。Lemma
\ref{groupoids-lemma-restrict-groupoid} 中构造的群胚态射
$(U', R', s', t', c') \to (U, R, s, t, c)$
...
Definitions \ref{groupoids-definition-restrict-groupoid} 与
\ref{groupoids-definition-restrict-relation} 所定义的群胚限制和
（预）等价关系限制，经 Lemmas
\ref{groupoids-lemma-groupoid-pre-equivalence} 与
\ref{groupoids-lemma-equivalence-groupoid} 的构造彼此一致。
\end{lemma}

\begin{proof}
这里所断言的是，Lemma \ref{groupoids-lemma-restrict-groupoid} 中的
$R'$ 也等于
$$
R' = (U' \times_S U')\times_{U \times_S U} R
\longrightarrow
...
\begin{proof}
第 (1) 点由 Lemmas
\ref{groupoids-lemma-pre-equivalence-equivalence-relation-points} 和
...
也对泛化封闭；见
（Morphisms, Lemma \ref{morphisms-lemma-generalizations-lift-flat} 及
Topology, Lemma \ref{topology-lemma-lift-specializations-images}）。
选取包含 $t(s^{-1}(W))$ 的拟紧开集 $W' \subset U$。
由 Properties, Lemma
\ref{properties-lemma-quasi-compact-quasi-separated-spectral}，
$W'$ 是谱空间（这里用到了 $U$ 拟分离）。对
$t(s^{-1}(W)) \subset W'$ 应用 Topology, Lemma
\ref{topology-lemma-make-spectral-space}，即得结论。
\end{proof}

\begin{lemma}
\label{groupoids-lemma-second-observation}
假设与记号同 Lemma \ref{groupoids-lemma-first-observation}。
...
由 Lemma \ref{groupoids-lemma-first-observation}，存在包含 $E$ 的拟紧开集
...
（见 Topologies, Section \ref{topologies-section-procedure}）。
于是可考虑函子
$$
h_U, h_R :
(\Sch/S)_\tau^{opp}
...
在 $U$ 上的作用，如 Lemma \ref{groupoids-lemma-groupoid-from-action}，
则有时把该商层记作 $U/G$。
\end{definition}
...
在 Definition \ref{groupoids-definition-quotient-sheaf} 的情形下，
...
Lemma \ref{groupoids-lemma-quotient-pre-equivalence} 及位点公理，
...
\begin{proof}
由 Lemmas \ref{groupoids-lemma-groupoid-pre-equivalence} 和
\ref{groupoids-lemma-quotient-pre-equivalence-relation-restrict} 即得单射性。
为证明满射性（层满射的刻画见
Sites, Section \ref{sites-section-sheaves-injective}），作如下论证。
...
\begin{proof}
由 Lemma \ref{groupoids-lemma-quotient-pre-equivalence}，商层有一个简单描述；
下文将不再说明而直接使用它。先证明
$$
...
可取其关联群胚；见 Lemma \ref{groupoids-lemma-equivalence-groupoid}。

\begin{lemma}
\label{groupoids-lemma-cartesian-equivalent-descent-datum}
设 $S$ 为概形，$f : X \to Y$ 为 $S$ 上的概形态射。
Lemma \ref{groupoids-lemma-characterize-cartesian-schemes} 的构造给出等价
$$
\begin{matrix}
\text{群胚概形范畴} \\
...
\begin{proof}
这直接源于 Lemma \ref{groupoids-lemma-characterize-cartesian-schemes}
以及 Descent, Definition \ref{descent-definition-descent-datum}
中概形上下降数据的定义。
\end{proof}
...
Lemma \ref{groupoids-lemma-groupoid-stabilizer}。因此，若
$j$ 分离（resp.\ 拟分离），则
$G \to U$ 分离（resp.\ 拟分离）
（见 Schemes, Lemma
...
（resp.\ 拟紧）；见 Schemes, Lemma
\ref{schemes-lemma-section-immersion}。
故在 (1) 与 (2) 中均有 (b) $\Rightarrow$ (a)。

\medskip\noindent
由 Lemma \ref{groupoids-lemma-diagram-diagonal} 的结果
（以及 Schemes, Lemmas \ref{schemes-lemma-base-change-immersion}
...
见 Definition \ref{groupoids-definition-quotient-sheaf}）由射影直线表示：
...
\noindent
回忆在 Exercises, Exercises \ref{exercises-exercise-trace-det} 和
...
\begin{proof}
考虑 Lemma \ref{groupoids-lemma-diagram} 的交换图
$$
\xymatrix{
& U & \\
R \ar[d]_s \ar[ru]^t &
...
\begin{proof}
由 Morphisms, Lemma \ref{morphisms-lemma-finite-locally-free}，
...
Lemma \ref{groupoids-lemma-diagram} 的记号，
$$
...
\begin{proof}
首先，由 Lemma \ref{groupoids-lemma-finite-locally-free-disjoint-free}，
...
\medskip\noindent
证明 (2)(b)。由于 $C \to A$ 整
（Lemma \ref{groupoids-lemma-integral-over-invariants}）且单，
$\Spec(A) \to \Spec(C)$ 满，见
...
\medskip\noindent
证明 (2)(a)。由 Lemma \ref{groupoids-lemma-finite-locally-free-disjoint-free}，
...
Lemma \ref{groupoids-lemma-determinant-trick} 及本引理的 (3)，
$P$ 的系数属于 $D$（比较 Lemma
\ref{groupoids-lemma-integral-over-invariants} 的证明）。
...
\begin{proof}
由于 $C \to A$ 整（Lemma \ref{groupoids-lemma-integral-over-invariants}）且单，
$\Spec(A) \to \Spec(C)$ 满，见
Algebra, Lemma \ref{algebra-lemma-integral-overring-surjective}。
因此 $|U| \to |M|$ 满。
...
由 Algebra, Lemma \ref{algebra-lemma-silly}，可得元素
...
A \otimes_C k
$$
由 Lemma \ref{groupoids-lemma-determinant-trick}，它属于经映射
...
下面说明这导致矛盾。把 Lemma \ref{groupoids-lemma-invariants-base-change}
...
$R' = \Spec(B')$。下文将直接使用 Lemmas
\ref{groupoids-lemma-integral-over-invariants}、
...
由 Lemma \ref{groupoids-lemma-invariants-base-change}，
...
由 Limits, Lemmas \ref{limits-lemma-descend-opens} 和
...
$J = \sqrt{\mathfrak m A}$。由 More on Algebra, Lemma
\ref{more-algebra-lemma-change-ideal-henselian-pair}，
...
$b \mapsto 1 \otimes b$。由 Lemma \ref{groupoids-lemma-diagram-pull}
及 $C$ 的定义，有交换图
$$
\xymatrix{
B \otimes_{s, A, t} B &
...
$$
由 Descent, Lemma \ref{descent-lemma-ff-exact}，
...
（该等式来自 Lemma \ref{groupoids-lemma-diagram} 中图上半部分的交换性）。
...
表示 fppf 拓扑下的商层 $U/R$
（见 Definition \ref{groupoids-definition-quotient-sheaf}）。
\end{proposition}

\begin{proof}
本证明中以 $s, t : A \to B$ 代替
$s^\sharp, t^\sharp$。由
Lemma \ref{groupoids-lemma-criterion-quotient-representable}，
...
\medskip\noindent
我们将如 Lemma \ref{groupoids-lemma-invariants-base-change} 那样，
...
\medskip\noindent
由 Lemma \ref{groupoids-lemma-finite-locally-free-disjoint-free}，
...
这样的 $f$ 由 Algebra, Lemma \ref{algebra-lemma-silly} 存在。
...
\noindent
可用 Lemma \ref{groupoids-lemma-find-invariant-affine}
说明某一类下降数据是有效的。
...
\begin{proof}
设 $\varphi$ 为 Descent, Definition
\ref{descent-definition-descent-datum} 意义下的下降数据。
回忆从 $Y$ 上概形到相对于 $\{X \to Y\}$ 的下降数据的函子全忠实，
见 Descent, Lemma
\ref{descent-lemma-refine-coverings-fully-faithful}。
因此利用 Constructions, Lemma
\ref{constructions-lemma-relative-glueing}，只需在 $Y$ 仿射时证明。
略去若干细节（若 $Y$ 分离或其对角仿射，则可避免这一论证，
因为此时从仿射概形到 $X$ 的每个态射都仿射）。

\medskip\noindent
假设 $Y$ 仿射。若 $V$ 也仿射，则有效性由
Descent, Lemma \ref{descent-lemma-affine} 得出。因此由
Descent, Lemma \ref{descent-lemma-effective-for-fpqc-is-local-upstairs}，
只需证明 $V$ 的每个点 $v$ 都有一个
$\varphi$-不变仿射开邻域。考虑群胚
$(X, X \times_Y X, \text{pr}_1, \text{pr}_0, \text{pr}_{02})$。
由 Lemma \ref{groupoids-lemma-cartesian-equivalent-descent-datum}，
...
至多有 $d$ 个点。再次使用 Lemma
\ref{groupoids-lemma-cartesian-equivalent-descent-datum} 的范畴等价，
可知 $V$ 的 $\varphi$-不变开集恰是 $V$ 的 $R$-不变开集。
由假设，存在包含轨道 $t(s^{-1}(\{v\}))$ 的 $V$ 的仿射开集，
因为该轨道的所有点都映到 $Y$ 的同一点。因此 Lemma
\ref{groupoids-lemma-find-invariant-affine} 给出包含 $v$ 的
$R$-不变仿射开集。
\end{proof}
...
\begin{proof}
与 Lemma \ref{groupoids-lemma-descend-along-finite-with-bound} 的证明一样，
可假设 $Y$ 仿射，特别是拟紧。于是存在整数 $d$，使
$X \to Y$ 的所有几何纤维至多有 $d$ 个点，见
Morphisms, Section \ref{morphisms-section-universally-bounded} 中的讨论。
关于 $V$ 的假设显然蕴含可应用
Lemma \ref{groupoids-lemma-descend-along-finite-with-bound}，证明完成。
\end{proof}
...
\begin{proof}
验证 Lemma \ref{groupoids-lemma-descend-along-finite} 中的条件。
...
若 $V$ 上存在充足可逆层，则由 Properties, Lemma
\ref{properties-lemma-ample-finite-set-in-affine}，
存在包含 $v_1, \ldots, v_d$ 的仿射开集。因此 (3) 成立。
在情形 (4) 与 (5) 中，可无妨把 $Y$ 替换为 $y$ 的仿射开邻域；
此时 $X$ 也仿射。在情形 (4) 中，由 Morphisms, Definition
\ref{morphisms-definition-relatively-ample}，$V$ 有充足可逆层，
...
\begin{proof}
取 $d = 1$，应用 Lemma \ref{groupoids-lemma-descend-along-finite-with-bound}。
\end{proof}
