...
$f : M \to M'$ 与 $g : N \to N'$。这样得到一个阿贝尔范畴；其中的核与余核如 Homology,
Lemma \ref{homology-lemma-cat-chain-abelian} 所述。
...
（Algebra, Lemma \ref{algebra-lemma-length-additive}）即得结论。
\end{proof}
...
$$
结合开头的观察与 Lemma \ref{chow-lemma-additivity-periodic-length} 的可加性，
...
\begin{proof}
应用 Lemma \ref{chow-lemma-additivity-periodic-length} 的可加性，并注意由
...
主要工具是 Algebra, Lemma \ref{algebra-lemma-order-vanishing-determinant}。
...
因为此时等式两边都为零，见 Lemma \ref{chow-lemma-finite-periodic-length}。
此外，若满足 (1)、(2) 的模之间有短正合列 $0 \to M \to M' \to M'' \to 0$，
则只要引理对其中两个模成立，由长度的可加性（Algebra, Lemma
\ref{algebra-lemma-length-additive}）和重数的可加性（Lemma
\ref{chow-lemma-additivity-periodic-length}），它对第三个模也成立。
...
此时，由 Algebra, Lemma
\ref{algebra-lemma-Noetherian-power-ideal-kills-module}，有有限滤过
...
（Algebra, Definition \ref{algebra-definition-lattice}）。于是 $x : M \to M$ 是单射，并且
$\text{length}_R(M/xM) = d(M, xM)$
（Algebra, Definition \ref{algebra-definition-distance}）。由于
$\text{length}_K(V) = \dim_K(V)$，可知
$\det(x : V \to V) = x^{\dim_K(V)}$，且
$\text{ord}_R(\det(x : V \to V)) = \dim_K(V) \text{ord}_R(x)$。
因而在此情形下，所需等式由 Algebra, Lemma
...
\begin{proof}
这些都是 Lemma \ref{chow-lemma-length-multiplication} 的特殊情形。
\end{proof}
...
由 Lemma \ref{chow-lemma-compare-periodic-lengths}，有
$e_R(M', \varphi, \psi) = e_R(M', x\varphi, \psi) =
e_R(M' \cap \Im(\varphi), 0, x) = 0$。
再由可加性（Lemma \ref{chow-lemma-additivity-periodic-length}）可知，
...
因此，由 More on Algebra, Lemma
\ref{more-algebra-lemma-application-formal-glueing}，存在 $A$-模同态
$\varphi : A \to B$，使 $\varphi_i$ 同构于 $\varphi$ 在 $\mathfrak m_i$ 处的局部化。
再利用 More on Algebra, Remark \ref{more-algebra-remark-formal-glueing-algebras}
...
\noindent
下面的引理与 Algebra, Lemma \ref{algebra-lemma-nonregular-dimension-one} 非常相似。
...
令 $X = \text{Proj}(\bigoplus_{d \geq 0} I^d)$。由 Divisors, Lemma
\ref{divisors-lemma-blowing-up-gives-effective-Cartier-divisor}，态射
$X \to \Spec(R)$ 在去心谱 $U = \Spec(R) \setminus \{\mathfrak m\}$ 上是同构。
因此可以而且将把 $U$ 看作 $X$ 的开子概形。由 Divisors, Lemma
\ref{divisors-lemma-blowing-up-projective}，态射 $X \to \Spec(R)$ 是射影的。
此外，例如由 Divisors, Lemma
\ref{divisors-lemma-blow-up-and-irreducible-components}，$X$ 的每个一般点都位于 $U$ 中。
于是由 Varieties, Lemma \ref{varieties-lemma-finite-in-codim-1}，
...
由 Divisors, Lemma \ref{divisors-lemma-blowing-up-gives-effective-Cartier-divisor}，
$IR'$ 是局部主理想。由于 $R'$ 是半局部环，$IR'$ 是主理想；见 Algebra, Lemma
\ref{algebra-lemma-locally-free-semi-local-free}。设 $IR' = (t)$，则
$a = a't$、$b = b't$，其余结论均显然。
\end{proof}
...
见 Algebra, Lemma \ref{algebra-lemma-dominate-by-dimension-1}。
...
$$
由 Lemma \ref{chow-lemma-glue-at-max}，存在唯一的有限扩张 $B \subset C$，其中
...
就可应用 Lemma \ref{chow-lemma-glue-at-max} 得到 $A$ 中 $a_1, a_2$ 的解。
选取 $x \in \{a_1, a_2\}$，使 $\ell = \text{length}_A(A/xA)$。
由 Lemma \ref{chow-lemma-compare-periodic-lengths} 
与 (\ref{chow-equation-multiplicity-coker-ker})，有
$\text{length}_A(A/xA) = \text{length}_A(A'/xA')$。另一方面，由 Algebra, Lemma
\ref{algebra-lemma-pushdown-module}，有
$$
\text{length}_A(A'/xA') =
...
\medskip\noindent
应用上一段的讨论，可以把 $A$ 替换为 Lemma
\ref{chow-lemma-Noetherian-domain-dim-1-two-elements} 对 $a_1, a_2 \in A$ 构造的环。
由于 $A$ 是局部环，必要时交换 $a_1$、$a_2$ 后，可设 $a_2 \in (a_1)$。
写成 $a_2 = a_1^m c$，其中 $m > 0$ 取最大值。事实上，由 Lemma
\ref{chow-lemma-not-infinitely-divisible}，即使把 $A$ 换成上一段所述的任意有限扩张
$A \subset A'$，也可假设 $m$ 仍为最大值。如果 $c$ 是单位，证明结束；否则，把 $A$ 替换为
Lemma \ref{chow-lemma-Noetherian-domain-dim-1-two-elements} 对 $a_1, c \in A$ 构造的环。
...
具体而言，给定非零因子 $a_1, a_2 \in A$，选择如 Lemma
\ref{chow-lemma-prepare-tame-symbol} 中的环扩张 $A \subset B$ 与局部分解。定义
\begin{equation}
...
\medskip\noindent
再证与 $B$ 的选择无关。设有 Lemma \ref{chow-lemma-prepare-tame-symbol} 中的两个扩张
...
由范数的传递性（Fields, Lemma \ref{fields-lemma-trace-and-norm-tower}），
...
且 $m = \sum m_k[\kappa_k:\kappa]$。首先，由 Lemma
\ref{chow-lemma-compare-periodic-lengths} 与 (\ref{chow-equation-multiplicity-coker-ker})，有
$m = \text{length}_B(B/xB) = \text{length}_B(C/\pi C)$。
最后，由 Algebra, Lemma \ref{algebra-lemma-pushdown-module}，有
...
为 $a_1, a_2, a_3$ 选择 Lemma \ref{chow-lemma-prepare-tame-symbol} 中的
$A \subset B$。等式
$$
...
并令 $a_3 = a_1 - a_2$。为 $a_1, a_2, a_3$ 选择 Lemma
\ref{chow-lemma-prepare-tame-symbol} 中的 $A \subset B$。只需证明
$$
...
\begin{proof}
为 $a, b$ 选择 Lemma \ref{chow-lemma-prepare-tame-symbol} 中的 $B_j \subset C_j$。
由 Lemma \ref{chow-lemma-glue-at-max}，可选取有限环扩张 $B \subset C$，
...
应用范数的传递性即得结论；见 Fields, Lemma
\ref{fields-lemma-trace-and-norm-tower}。
\end{proof}
...
$\partial_{A'}(a_1, a_2) = 1$，结论成立。否则，可以选择 Lemma
\ref{chow-lemma-prepare-tame-symbol} 中的环扩张 $A \subset B$ 与局部分解。
...
\end{equation}
在 $\kappa'$ 中成立。我们用 More on Algebra, Section
\ref{more-algebra-section-determinants-finite-length} 中有限长度模的自同态行列式构造证明这一点。
令 $M = B_j/\pi_j B_j$。由 More on Algebra, Lemma
\ref{more-algebra-lemma-multiplication}，有
$$
\text{Norm}_{\kappa_j/\kappa}(u \bmod \mathfrak m_j)^{m_j} =
\det\nolimits_\kappa(u : M \to M)
$$
因此，由 More on Algebra, Lemma \ref{more-algebra-lemma-flat-base-change-det}，
...
$$
令 $M'_l = B'_{j, l}/\pi_j B'_{j, l}$。再次由 More on Algebra, Lemma
\ref{more-algebra-lemma-multiplication}，可得
$\text{Norm}_{\kappa'_{j, l}/\kappa'}(u \bmod \mathfrak m'_{j, l})^{m'_{j, l}}
= \det_{\kappa'}(u_j : M'_l \to M'_l)$。
所以 (\ref{chow-equation-to-prove}) 由行列式构造的乘法性成立；见 More on Algebra, Lemma
\ref{more-algebra-lemma-ses}。
\end{proof}
...
\noindent
本节应用上述结果证明 Lemma \ref{chow-lemma-milnor-gersten-low-degree}。
该引理是如下陈述的低次数情形：Milnor K-理论存在一个类似于 Quillen K-理论中
Gersten--Quillen 复形的复形。见 Remark \ref{chow-remark-gersten-complex-milnor}。
...
则求和遍历 $i = 1, \ldots, r$。对每个 $i$，为 $a, b$ 选取 Lemma
\ref{chow-lemma-prepare-tame-symbol} 中的扩张 $A_{\mathfrak q_i} \subset B_i$。
把 Lemma \ref{chow-lemma-perpare-key} 用于 $t = ab$ 与给定的素理想列表，
...
Lemma \ref{chow-lemma-norm-down-tame-symbol} 和 Algebra, Lemma
\ref{algebra-lemma-finite-extension-dim-1}，有
$$
...
$$
其核与余核均支撑在 $\{\mathfrak m\}$ 上，因而具有有限长度。因此，由 Lemma
\ref{chow-lemma-compare-periodic-lengths}，有
$$
\sum e_A(M_i, a, b) = 0
$$
故只须证明
...
-\text{ord}_{A/\mathfrak q_i}(\partial_{A_{\mathfrak q_i}}(a, b))
\end{align*}
第二个等式来自 Lemma \ref{chow-lemma-multiply-period-length}。注意
...
第三个等式中的 $0$ 来自 Lemma \ref{chow-lemma-powers-period-length-zero}，
另外两项来自 Lemma \ref{chow-lemma-length-multiplication}。
...
由 Lemma \ref{chow-lemma-length-multiplication}，有
$$
...
$$
这由 Lemma \ref{chow-lemma-length-multiplication} 以及上文对在 $\mathfrak q_i$ 处局部化所得情形的描述推出。
\end{proof}
...
这一情形已在 Lemma \ref{chow-lemma-key-nonzerodivisors} 中证明。
\end{proof}
...
当 $i = 1$ 而 $j$ 任意时，结论来自 Lemma \ref{chow-lemma-milnor-gersten-low-degree}。
\end{remark}
...
\noindent
泛链状概形的概念见 Morphisms, Definition
\ref{morphisms-definition-universally-catenary}；维数函数的概念见 Topology, Definition
\ref{topology-definition-dimension-function}。回忆，任意局部诺特链状概形在局部都有维数函数；
见 Properties, Lemma \ref{properties-lemma-catenary-dimension-function}。
此外，泛链状概形相当常见；见 Morphisms, Lemma
\ref{morphisms-lemma-ubiquity-uc}。

\medskip\noindent
设 $(S, \delta)$ 如 Situation \ref{chow-situation-setup}。任意在 $S$ 上局部有限型的概形 $X$
...
见 Morphisms, Lemma \ref{morphisms-lemma-dimension-function-propagates}。
...
$(S, \delta)$ 是 Situation \ref{chow-situation-setup} 中设置的一个例子；
这来自 Morphisms, Lemma \ref{morphisms-lemma-ubiquity-uc}。
\end{example}
...
这是 Situation \ref{chow-situation-setup} 的一个例子；
结论来自 Morphisms, Lemma \ref{morphisms-lemma-ubiquity-uc}。
\end{example}

\begin{example}
\label{chow-example-CM-irreducible}
这里 $S$ 是 Cohen--Macaulay 概形。由 Morphisms, Lemma
\ref{morphisms-lemma-ubiquity-uc}，$S$ 泛链状。令
$\delta(s) = -\dim(\mathcal{O}_{S, s})$。若 $s' \leadsto s$ 是 $S$ 中点的非平凡特殊化，
则 $\mathcal{O}_{S, s'}$ 是 $\mathcal{O}_{S, s}$ 在非极大素理想
$\mathfrak p \subset \mathcal{O}_{S, s}$ 处的局部化；见 Schemes, Lemma
\ref{schemes-lemma-specialize-points}。因此由 Algebra, Lemma
\ref{algebra-lemma-CM-dim-formula}，有
$\dim(\mathcal{O}_{S, s}) =
...
换言之，$(S, \delta)$ 是 Situation \ref{chow-situation-setup} 的一个例子。
\end{example}
...
设 $(S, \delta)$ 如 Situation \ref{chow-situation-setup}。进一步假设 $S$ 是 Jacobson 概形，
...
$X$ 是 Jacobson 概形；见 Morphisms, Lemma
\ref{morphisms-lemma-Jacobson-universally-Jacobson}。因此，$X$ 的闭点在 $Z$ 的每个闭子集中稠密，
并映到 $S$ 的闭点。所以，给定 $Z$ 中不可约闭子集的任意链，可以用 $Z$ 的某个闭点将其终结。
从而，当 $z \in Z$ 遍历 $Z$ 的闭点时，
$\dim(Z) = \sup_z(\dim(\mathcal{O}_{Z, z})$；
见 Properties, Lemma \ref{properties-lemma-codimension-local-ring}。
由维数函数的性质，$\dim(\mathcal{O}_{Z, z}) = \delta(\xi) - \delta(z)$。
对每个闭点 $z \in Z$，域扩张 $\kappa(z)/\kappa(f(z))$ 有限；见 Morphisms,
...
设 $(S, \delta)$ 如 Situation \ref{chow-situation-setup}。对任意在 $S$ 上局部有限型的概形 $X$
...
设 $(S, \delta)$ 如 Situation \ref{chow-situation-setup}，设 $X$ 在 $S$ 上局部有限型，并设
$k \in \mathbf{Z}$。
\begin{enumerate}
...
Milnor K-理论的次数 $0$ 部分（见 Remark \ref{chow-remark-gersten-complex-milnor}）；
...
设 $(S, \delta)$ 如 Situation \ref{chow-situation-setup}，并设 $X$ 在 $S$ 上局部有限型。
...
设 $(S, \delta)$ 如 Situation \ref{chow-situation-setup}，设 $X$ 在 $S$ 上局部有限型，
并设 $Z \subset X$ 是闭子概形。
\begin{enumerate}
...
$\dim(\mathcal{O}_{Z, \xi}) = 0$；例如见 Properties, Lemma
\ref{properties-lemma-codimension-local-ring}。因此 $\mathcal{O}_{Z, \xi}$ 是维数为零的诺特局部环，
从而作为自身的模具有有限长度（见 Algebra, Proposition
\ref{algebra-proposition-dimension-zero-ring}）。所以它作为 $\mathcal{O}_{X, \xi}$-模也具有有限长度；
见 Algebra, Lemma \ref{algebra-lemma-length-independent}。
...
（Lemma \ref{chow-lemma-multiplicity-finite}）为 {\it $Z'$ 在 $Z$ 中的重数}。
...
（由 Divisors, Lemma \ref{divisors-lemma-components-locally-finite}，这是一个 $k$-循环。）
\end{enumerate}
\end{definition}
...
\begin{proof}
由 Cohomology of Schemes, Lemma \ref{coherent-lemma-coherent-support-closed}，
$\mathcal{F}$ 的支撑 $Z$ 是 $X$ 的闭子集。可以把 $Z$ 看成 $X$ 的约化闭子概形
（Schemes, Lemma \ref{schemes-lemma-reduced-closed-subscheme}）。
因此，把 Divisors, Lemma \ref{divisors-lemma-components-locally-finite} 用于 $Z$ 即得 (1)，
把 Lemma \ref{chow-lemma-multiplicity-finite} 用于 $Z$ 即得 (3)。
...
$\{\mathfrak m_\xi\}$。所以由 Algebra, Lemma \ref{algebra-lemma-support-point}，
它具有有限长度。
\end{proof}
...
（Lemma \ref{chow-lemma-length-finite}）为 {\it $Z'$ 在 $\mathcal{F}$ 中的重数}。
...
（由 Lemma \ref{chow-lemma-length-finite}，这是一个 $k$-循环。）
\end{enumerate}
\end{definition}
...
\begin{proof}
这立即来自长度的可加性；见 Algebra, Lemma \ref{algebra-lemma-length-additive}。
\end{proof}
...
设 $(S, \delta)$ 如 Situation \ref{chow-situation-setup}，设 $X$、$Y$ 在 $S$ 上局部有限型，
...
（Topology, Lemmas \ref{topology-lemma-image-irreducible-space} 和
\ref{topology-lemma-irreducible}）。如果 $\overline{f(X)} \not = Y$，结论已得。
如果 $\overline{f(X)} = Y$，则 $f$ 支配；由 Morphisms, Lemma
...
所以由 Morphisms, Lemma \ref{morphisms-lemma-finite-degree}，
$R(Y) \subset R(X)$ 是有限扩张；该引理适用是由 Morphisms, Lemma
\ref{morphisms-lemma-permanence-finite-type} 保证的。
\end{proof}
...
$\dim_\delta(f(Z)) = \dim_\delta(Z)$，则由 Lemma \ref{chow-lemma-equal-dimension}，
$Z$ 在 $f(Z)$ 上的次数有限。
\item 设 $\alpha = \sum n_Z [Z]$ 是 $X$ 上的 $k$-循环。定义 $\alpha$ 的{\it 推前}为和
$$
f_* \alpha = \sum n_Z f_*[Z]
$$
其中每个 $f_*[Z]$ 如上定义。由 Lemma
\ref{chow-lemma-quasi-compact-locally-finite}，这个和局部有限。
\end{enumerate}
\end{definition}
...
设 $(S, \delta)$ 如 Situation \ref{chow-situation-setup}，设 $X$、$Y$、$Z$ 在 $S$ 上局部有限型，
...
W''
$$
都满足 Lemma \ref{chow-lemma-equal-dimension} 的假设。因此
$$
g_*(f_*[W]) = \deg(W/W')\deg(W'/W'')[W''],
\quad
(g \circ f)_*[W] = \deg(W/W'')[W''].
$$
再应用 Morphisms, Lemma \ref{morphisms-lemma-degree-composition} 即得结论。
\end{proof}
...
设 $(S, \delta)$ 如 Situation \ref{chow-situation-setup}，并设 $f : X \to Y$ 是在 $S$ 上局部有限型的概形之间的固有态射。
\begin{enumerate}
...
$$
\end{enumerate}
该陈述有意义，因为由 Cohomology of Schemes, Proposition
\ref{coherent-proposition-proper-pushforward-coherent}，$f_*\mathcal{F}$ 和
$f_*\mathcal{O}_Z$ 都是凝聚 $\mathcal{O}_Y$-模。
\end{lemma}

\begin{proof}
(1) 由 (2) 和 Lemma \ref{chow-lemma-cycle-closed-coherent} 推出。
...
$f|_Z : Z \to Z'$ 是支配态射；见 Morphisms, Lemma
...
（见 Lemma \ref{chow-lemma-multiplicity-finite}）。由于 $f$ 拟紧且 $X$ 局部诺特，
这样的点只有有限多个，故 $f^{-1}(\{\eta\})$ 有限。
由 Morphisms, Lemma \ref{morphisms-lemma-generically-finite}，存在开邻域
...
$$
所需等式于是来自 Algebra, Lemma \ref{algebra-lemma-pushdown-module}。
\end{proof}
...
就称它的相对维数为 $r$。见 Morphisms, Definition
...
故 Morphisms, Lemma \ref{morphisms-lemma-fppf-open} 适用，并表明 $f$ 是开态射。
...
所以由 Morphisms, Lemma \ref{morphisms-lemma-dimension-fibre-at-a-point}，
...
对此的讨论见 Schemes, Section \ref{schemes-section-closed-immersion}、
Lemma \ref{schemes-lemma-fibre-product-immersion} 和 Definition
\ref{schemes-definition-inverse-image-closed-subscheme}。
...
f^*[Z] = [f^{-1}(Z)]_{k+r}.
$$
这是有意义的，因为由 Lemma \ref{chow-lemma-flat-inverse-image-dimension}，
...
$$
其中每个 $f^*[Z_i]$ 如上定义。由 Lemma \ref{chow-lemma-inverse-image-locally-finite}，
该和局部有限。
...
（见 Schemes, Lemma \ref{schemes-lemma-reduced-closed-subscheme}）。
...
\begin{proof}
由 Morphisms, Lemma \ref{morphisms-lemma-composition-relative-dimension-d}，
...
$[W'']$ 在 $f^*(g^*[W])$ 中的系数 $m$。由 Lemma
\ref{chow-lemma-flat-inverse-image-dimension}，只核查这些情形已经足够。
...
$$
所需等式由 Algebra, Lemma \ref{algebra-lemma-pullback-transitive} 推出。
\end{proof}
...
\begin{proof}
关于维数的断言立即来自 Lemma \ref{chow-lemma-flat-inverse-image-dimension}。
(1) 由 (2)、Lemma \ref{chow-lemma-cycle-closed-coherent} 以及等式
$f^*\mathcal{O}_Z = \mathcal{O}_{f^{-1}(Z)}$ 推出。

\medskip\noindent
下面证明 (2)。由于 $X$、$Y$ 局部诺特，可以应用 Cohomology of Schemes, Lemma
\ref{coherent-lemma-coherent-Noetherian}，得知 $\mathcal{F}$ 有限型；所以
$f^*\mathcal{F}$ 有限型（Modules, Lemma \ref{modules-lemma-pullback-finite-type}），
进而 $f^*\mathcal{F}$ 凝聚（再次应用 Cohomology of Schemes, Lemma
\ref{coherent-lemma-coherent-Noetherian}）。因此该引理有意义。
...
但实际上无需验证这一点；见 Lemma \ref{chow-lemma-length-finite}。
...
$$
所需等式来自 Algebra, Lemma \ref{algebra-lemma-pullback-module}。
\end{proof}
...
设 $(S, \delta)$ 如 Situation \ref{chow-situation-setup}。设
$$
\xymatrix{
...
\begin{proof}
$f'$ 固有的断言来自 Morphisms, Lemma \ref{morphisms-lemma-base-change-proper}。
$g'$ 平坦且相对维数为 $r$ 的断言来自 Morphisms, Lemmas
\ref{morphisms-lemma-base-change-relative-dimension-d} 和
\ref{morphisms-lemma-base-change-flat}。只须在 $\alpha = [W]$ 时证明循环等式，
其中 $W \subset X$ 是 $\delta$-维数为 $k$ 的整闭子概形。
注意，此时有 $\alpha = [\mathcal{O}_W]_k$；见 Lemma
\ref{chow-lemma-cycle-closed-coherent}。所以由 Lemmas
\ref{chow-lemma-cycle-push-sheaf} 和 \ref{chow-lemma-pullback-coherent}，
只须证明 $f'_*(g')^*\mathcal{O}_W$ 同构于 $g^*f_*\mathcal{O}_W$。
这来自上同调与基变换；见 Cohomology of Schemes, Lemma
...
（见 Morphisms, Definition \ref{morphisms-definition-finite-locally-free}）。
...
\begin{proof}
由 Morphisms, Lemma \ref{morphisms-lemma-finite-flat}，有限局部自由态射平坦且有限；
由 Morphisms, Lemma \ref{morphisms-lemma-finite-proper}，有限态射固有。
...
$$
这里使用了 Lemmas \ref{chow-lemma-pullback-coherent} 和
...
\noindent
本节的一部分材料与 Divisors, Section \ref{divisors-section-Weil-divisors}
中的讨论有所重叠。

\begin{lemma}
\label{chow-lemma-divisor-delta-dimension}
设 $(S, \delta)$ 如 Situation \ref{chow-situation-setup}。
...
\begin{proof}
(1) 由素除子的定义（Divisors, Definition
\ref{divisors-definition-Weil-divisor}）和维数函数的定义（Topology,
Definition \ref{topology-definition-dimension-function}）立即得到。
设 $\xi \in Z$ 是有效 Cartier 除子 $D \subset X$ 的某个不可约分支 $Z$ 的一般点。
那么 $\dim(\mathcal{O}_{D, \xi}) = 0$，并且对某个非零因子
$f \in \mathcal{O}_{X, \xi}$，有
$\mathcal{O}_{D, \xi} = \mathcal{O}_{X, \xi}/(f)$（Divisors,
Lemma \ref{divisors-lemma-effective-Cartier-in-points}）。
由 Algebra, Lemma \ref{algebra-lemma-one-equation}，
$\dim(\mathcal{O}_{X, \xi}) = 1$。于是 Properties, Lemma
\ref{properties-lemma-codimension-local-ring} 表明 $Z$ 满足 (1)，证明完毕。
\end{proof}
...
\begin{proof}
本引理是 Varieties, Lemma \ref{varieties-lemma-finite-in-codim-1} 的特例。
这里给出本情形下的直接论证。由 Cohomology of Schemes, Lemma
...
因此该引理由 Algebra, Lemma \ref{algebra-lemma-finite-in-codim-1} 得出。
\end{proof}
...
\noindent
下面的定义是 Divisors, Definition
\ref{divisors-definition-principal-divisor} 在当前设定下的对应形式。
...
$$
其中右端定义见 Divisors, Definition
\ref{divisors-definition-principal-divisor}。由 Lemma
\ref{chow-lemma-divisor-delta-dimension}，素除子的 $\delta$-维数为 $n - 1$，
故此定义有意义。
\end{definition}
...
$$
见 Divisors, Lemma \ref{divisors-lemma-div-additive}。
下面的引理将被更一般的 Lemma
\ref{chow-lemma-flat-pullback-rational-equivalence} 所涵盖。
...
\text{ord}_B(g).
$$
这由 Algebra, Lemma \ref{algebra-lemma-pullback-module} 得出
（细节从略）。
\end{proof}
...
可以把 Lemma \ref{chow-lemma-finite-in-codimension-one} 用于态射
...
$$
所需等式于是由 Algebra, Lemma
\ref{algebra-lemma-finite-extension-dim-1} 得出。
\end{proof}
...
它们是有效 Cartier 除子；见 Divisors, Definition
\ref{divisors-definition-effective-Cartier-divisor} 和 Lemma
\ref{divisors-lemma-characterize-OD}。下面的引理说，粗略而言，
...
（Morphisms, Lemma \ref{morphisms-lemma-composition-proper}），故结论成立。
...
因此 (3) 由 Divisors, Lemma
\ref{divisors-lemma-pullback-effective-Cartier-defined} 得到。
再由 Lemma \ref{chow-lemma-divisor-delta-dimension} 得到
...
$$
其中 $Y'_0 \subset X$ 是 $Y_0 \to X$ 的像。由 Lemma
\ref{chow-lemma-cycle-push-sheaf}，有
$p_*[Y_0]_{n - 1} = [Y'_0]_{n - 1}$。对 $D_\infty$ 和 $Y_\infty$ 同样成立，
故 (6) 是 (5) 的推论。最后，(5) 立即由 Lemma
\ref{chow-lemma-proper-pushforward-alteration} 得到。
\end{proof}
...
0
$$
其中 $\text{ord}$ 如 Algebra, Definition \ref{algebra-definition-ord} 所定义。
换言之，$c_*\text{div}(f) = 0$。
\end{lemma}
...
$$
这是在 Lemma \ref{chow-lemma-rational-function} 中，从 $S = \Spec(K)$ 上的
...
再由 Lemma \ref{chow-lemma-finite-flat} 得到
...
例如由 Lemma \ref{chow-lemma-finite-in-codimension-one} 可知 $q$ 是有限态射。
$\mathbf{P}^1_K$ 在各闭点处的局部环都是维数为 $1$ 的正则局部环
（即离散赋值环），因为它们是 $K[T]$ 的局部化；见 Algebra, Lemma
\ref{algebra-lemma-dim-affine-space}。因此，对闭点 $y\in Y$，只要
...
所以 $q$ 平坦。最后由 Morphisms, Lemma
\ref{morphisms-lemma-finite-flat}，$q$ 是有限局部自由态射。
\end{proof}
...
（沿闭浸入）的局部有限和。这会产生一些颇为奇特的现象；见 Example
\ref{chow-example-weird}。然而，若不允许这种和，我们便不知道如何证明线丛陈类的帽积
可下降到有理等价类。
...
\CH_k(X) \to 0
$$
这里采用 Remark \ref{chow-remark-cycles-pointwise} 中引入的记号与约定，此外：
\begin{enumerate}
\item $K_1^M(\kappa(x)) = \kappa(x)^*$ 是点 $x \in X$ 的剩余域
$\kappa(x)$ 的 Milnor K-理论的次数 $1$ 部分；见 Remark
...
$$
其属于 $Z_k(X)$。这是有意义的，因为 Remark
\ref{chow-remark-cycles-pointwise} 已说明该复形的第二项等于 $Z_k(X)$。
\end{enumerate}
与 Definition \ref{chow-definition-rational-equivalence} 比较，立即可知这确实给出
$\CH_k(X)$ 的一个表现。
\end{remark}
...
$$
是支撑在 $Y$ 上的循环，引理即得；见 Lemma
\ref{chow-lemma-exact-sequence-open}。
\end{proof}
...
\begin{proof}
由 Lemma \ref{chow-lemma-exact-sequence-closed}，箭头
...
下面是一个“奇特”的例子。设 $S$ 是域 $k$ 的谱，$\delta$ 如 Example
\ref{chow-example-field}。设 $X = C_1 \cup C_2 \cup \ldots$ 是曲线
...
设 $Z'$ 是 $f(Z)$ 的闭包；它是 $Y$ 的整闭子概形。由 Lemma
\ref{chow-lemma-flat-inverse-image-dimension}，有
...
\text{length}_B(B/xB)
$$
（此等式使用 Algebra, Lemma \ref{algebra-lemma-pullback-module}）等于
$$
...
故所需等式由 Lemma \ref{chow-lemma-additivity-divisors-restricted} 得出。
\end{proof}
...
$$
在 $X$ 上有理等价于零。由 Lemma
\ref{chow-lemma-inverse-image-locally-finite}，$\{W_j\}$ 在 $X$ 中局部有限，
故右端的和有意义。
...
$$
并以 $f_j : W'_j \to W_j$ 表示第一投影。由 Lemma
\ref{chow-lemma-flat-pullback-proper-pushforward}，上述和可写为
$$
\sum i'_{j, *}(f_j^*\text{div}(g_j))
$$
由 Lemma \ref{chow-lemma-prepare-flat-pullback-rational-equivalence}，
每个 $f_j^*\text{div}(g_j)$ 在 $W'_j$ 上有理等价于零。
因此每个 $i'_{j, *}(f_j^*\text{div}(g_j))$ 都有理等价于零。
由 Remark \ref{chow-remark-infinite-sums-rational-equivalences} 中的讨论，
展示的和也有同样的性质。
\end{proof}
...
由 Lemma \ref{chow-lemma-quasi-compact-locally-finite}，族 $\{W'_j\}$ 在 $Y$
...
$$
由 Lemma \ref{chow-lemma-compose-pushforward}，
...
由 Lemma \ref{chow-lemma-curve-principal-divisor}，该重数为零。

\medskip\noindent
情形 $\dim_\delta(W') = k + 1$。此时可应用 Lemma
\ref{chow-lemma-proper-pushforward-alteration}，从而确有
...
\noindent
设 $(S, \delta)$ 如 Situation \ref{chow-situation-setup}，并设概形 $X$ 在 $S$
上局部有限型。给定任意闭子概形
...
$D_0$ 或 $D_\infty$，所以 (1) 由 Divisors, Lemma
\ref{divisors-lemma-pullback-effective-Cartier-defined} 得出。
...
由 Lemma \ref{chow-lemma-rational-function}，作为 $W$ 上的循环，有
$[W_0]_k \sim_{rat} [W_\infty]_k$。而显然
$p_*[W_0]_k = [W_0]_k$，对 $W_\infty$ 也一样，所以 (2) 由 Lemma
\ref{chow-lemma-proper-pushforward-rational-equivalence} 得出。
...
$$
为 Lemma \ref{chow-lemma-rational-function} 中有理映射 $f_i$ 的图的闭包。
...
\item 对 $Z$ 的任意嵌入点 $\xi$（Divisors, Definition
\ref{divisors-definition-embedded}），要么
...
[Z]_{k + 1} = \sum n_i[W_i]
$$
其中 $n_i > 0$。由 Divisors, Lemma
\ref{divisors-lemma-components-locally-finite}，$\{W_i\}$ 是
$X \times_S \mathbf{P}^1_S$ 的闭子集的局部有限族。我们断言
$$
[Z_0]_k = \sum n_i[(W_i)_0]_k
$$
并且对 $[Z_\infty]_k$ 也有类似等式。一旦证明此点，引理即由 Lemma
\ref{chow-lemma-rational-equivalence-family} 得出。
...
$$
结论于是由 Lemma \ref{chow-lemma-additivity-divisors-restricted} 得出。
\end{proof}
...
$$
其中 $n_i > 0$。由 Lemma \ref{chow-lemma-length-finite}，$\{W_i\}$ 是
$X \times_S \mathbf{P}^1_S$ 的闭子集的局部有限族。我们断言
$$
[\mathcal{F}_0]_k = \sum n_i[(W_i)_0]_k
$$
并且对 $[\mathcal{F}_\infty]_k$ 也有类似等式。一旦证明此点，引理即由 Lemma
\ref{chow-lemma-rational-equivalence-family} 得出。
...
\noindent
Lemma \ref{chow-lemma-envelope} 的正合序列是采用该定义的主要动机。
...
\begin{proof}
这由 Morphisms, Lemma \ref{morphisms-lemma-composition-proper} 和
More on Morphisms, Lemma \ref{more-morphisms-lemma-composition-cd} 得出。
\end{proof}
...
\begin{proof}
这由 Morphisms, Lemma \ref{morphisms-lemma-base-change-proper} 和
More on Morphisms, Lemma \ref{more-morphisms-lemma-base-change-cd} 得出。
\end{proof}
...
\begin{proof}
由于 $f$ 是包络，$f$ 是固有态射，故循环及循环类的推前都有定义；见 Sections
\ref{chow-section-proper-pushforward} 和 \ref{chow-section-push-pull}。
类似地，态射 $p$ 和 $q$ 是 $f$ 的基变换，所以也固有。两个箭头的复合为零，因为
$f_* \circ p_* = (p \circ f)_* = (q \circ f)_* = f_* \circ q_*$；
见 Lemma \ref{chow-lemma-compose-pushforward}。
...
$$
则 $f_*\beta' = 0$。（这里使用 Lemma \ref{chow-lemma-compose-pushforward}。）
...
（Lemma \ref{chow-lemma-quasi-compact-locally-finite}），$Y \times_X Y$ 上的
$k$-循环
$$
...
这是 Abel 范畴；见 Cohomology of Schemes, Lemma
\ref{coherent-lemma-coherent-abelian-Noetherian}。对任意
...
\begin{proof}
Serre 子范畴的定义见 Homology, Definition
\ref{homology-definition-serre-subcategory}。该引理的证明直接，故略去。
\end{proof}
...
$\textit{Coh}_{\leq k}(X)$ 上可加；见 Lemma
\ref{chow-lemma-additivity-sheaf-cycle}。而若
$\mathcal{F} \in \textit{Coh}_{\leq k - 1}(X)$，则
$[\mathcal{F}]_k = 0$。由 Homology, Lemma
\ref{homology-lemma-serre-subcategory-K-groups} 的 (1)，第二个映射也有良好定义。
...
$\xi_i$ 的开邻域上为同构。Cohomology of Schemes, Lemma
\ref{coherent-lemma-extend-coherent} 保证可以这样选择。然后考虑短正合列
$$
0 \to
...
\begin{proof}
有限态射是仿射态射，所以由 Cohomology of Schemes, Lemma
\ref{coherent-lemma-relative-affine-vanishing}，沿 $\pi$ 推前拟凝聚模是正合函子。
有限态射是固有态射，所以 $\pi_*$ 把凝聚层送到凝聚层；见 Cohomology of Schemes,
Proposition \ref{coherent-proposition-proper-pushforward-coherent}。
关于支撑维数的断言显然。右侧的交换性立即由 Lemma
\ref{chow-lemma-cycle-push-sheaf} 得到。由于各水平箭头是双射，左侧也交换。
\end{proof}

\begin{lemma}
\label{chow-lemma-from-chow-to-K}
设 $X$ 是 Situation \ref{chow-situation-setup} 中在 $(S, \delta)$ 上局部有限型的概形。
存在典范映射
$$
\CH_k(X)
\longrightarrow
...
$$
它由 Lemma \ref{chow-lemma-cycles-k-group} 中的映射
...
$K_0(\textit{Coh}_{\leq k + 1}(X)/\textit{Coh}_{\leq k - 1}(X))$
中映到零。按照 Definition \ref{chow-definition-rational-equivalence} 写成
$\alpha = \sum (i_j)_*\text{div}(f_j)$。注意
$$
\pi = \coprod i_j : W = \coprod W_j \longrightarrow X
$$
是有限态射，因为每个 $i_j : W_j \to X$ 都是闭浸入，且 $W_j$ 的族在
$X$ 中局部有限。因此可以使用 Lemma \ref{chow-lemma-finite-cycles-k-group}
...
$K_0(\textit{Coh}_{\leq k + 1}(X)/\textit{Coh}_{\leq k - 1}(X))$
中映到零。设 $\mathcal{I} \subset \mathcal{O}_X$ 是 $f$ 的分母理想；见 Divisors,
...
$$
见 Divisors, Lemma
\ref{divisors-lemma-regular-meromorphic-ideal-denominators}。我们断言
$$
...
$$
利用 Algebra, Definition \ref{algebra-definition-distance} 的距离函数，
可将其改写为
$$
d(A, I) - d(A, fI) = d(I, fI) = \text{ord}_A(f)
$$
这些等式由 Algebra, Lemmas
\ref{algebra-lemma-properties-distance-function} 和
\ref{algebra-lemma-order-vanishing-determinant} 得出。
使用这些引理并非必要，但很方便。
\end{proof}
...
设 $(S, \delta)$ 如 Situation \ref{chow-situation-setup}。
设概形 $X$ 在 $S$ 上局部有限型。稍后将在 Lemma
\ref{chow-lemma-cycles-rational-equivalence-K-group} 中看到，Lemma
\ref{chow-lemma-from-chow-to-K} 的映射
$$
\CH_k(X)
\longrightarrow
...
$$
完全如 Lemma \ref{chow-lemma-from-chow-to-K} 的证明，这个滤过局部有限，因而
$\bigoplus_{p \geq 0} \mathcal{F}^p$、
$\bigoplus_{p \geq 1} \mathcal{F}^p$ 与
$\bigoplus_{p \geq 0} \mathcal{F}^p/\mathcal{F}^{p + 1}$
都是支撑在 $Z$ 上的凝聚 $\mathcal{O}_X$-模。因此，与 Lemma
\ref{chow-lemma-from-chow-to-K} 的证明完全一样，在 $K_0(\textit{Coh}_Z(X))$ 中有
$$
[\mathcal{F}] =
...
\noindent
下面的定义是 Divisors, Definition
\ref{divisors-definition-divisor-invertible-sheaf} 在当前设定下的对应形式。
...
$$
其定义见 Divisors, Definition
\ref{divisors-definition-divisor-invertible-sheaf}。由 Lemma
\ref{chow-lemma-divisor-delta-dimension}，Weil 除子的 $\delta$-维数为 $n - 1$，
故此定义有意义。
...
$$
其中 $s$ 是 $X$ 上 $\mathcal{L}$ 的任意非零亚纯截面。由 Divisors, Lemma
\ref{divisors-lemma-divisor-meromorphic-well-defined}，此定义与选择无关。
\end{enumerate}
\end{definition}

\noindent
设 $X$、$S$ 如上面的 Definition \ref{chow-definition-divisor-invertible-sheaf}，
并置 $n = \dim_\delta(X)$。由定义显然有
$Cl(X) = \CH_{n - 1}(X)$，其中 $Cl(X)$ 是 Divisors, Definition
\ref{divisors-definition-class-group} 中定义的 $X$ 的 Weil 除子类群。映射
$$
...
若 $X$ 的所有局部环都是 UFD，则它是同构。见 Divisors, Lemmas
\ref{divisors-lemma-normal-c1-injective} 和
...
（见 Divisors, Definition \ref{divisors-definition-zero-scheme-s}），
...
\noindent
下面的引理将被更一般的 Lemma \ref{chow-lemma-flat-pullback-cap-c1} 所涵盖。
...
$$
于是，$f^{-1}(V)$ 上所需的循环等式由主除子拉回的相应结论得到；见 Lemma
\ref{chow-lemma-flat-pullback-principal-divisor}。
\end{proof}
...
$$
其中右端由 Definition \ref{chow-definition-divisor-invertible-sheaf} 定义。
...
\begin{proof}
可加性直接由 Divisors, Lemma \ref{divisors-lemma-c1-additive} 与定义得到。
...
$$
它由 Lemma \ref{chow-lemma-additivity-divisors-restricted} 得出。
\end{proof}
...
由 Lemma \ref{chow-lemma-compute-c1}，$[Z(s_i)]_k$ 表示
$c_1(\mathcal{L}|_{Y_i})$。所以按定义
$$
c_1(\mathcal{L}) \cap [Y]_{k + 1} = \sum n_i[Z(s_i)]_k
$$
结论于是由 Lemma \ref{chow-lemma-prepare-geometric-cap} 得出。
\end{proof}
...
$$
（记号含义显然），则由可加性即得结论；见 Divisors, Lemma
\ref{divisors-lemma-c1-additive}。所以可以假设
...
= [f^{-1}(Z(s))]_{k + r - 1},
$$
这由 Lemmas \ref{chow-lemma-compute-c1} 和 \ref{chow-lemma-pullback-coherent} 得出。
...
$$
Remark \ref{chow-remark-infinite-sums-rational-equivalences} 中的讨论于是表明，
引理的等式成立。
...
对每个 $i$，$s_i = s|_{W'_i}$ 都是非零亚纯截面。由 Lemma
\ref{chow-lemma-prepare-flat-pullback-cap-c1}，有循环等式
$$
...
可以再次使用 Lemma \ref{chow-lemma-proper-pushforward-alteration} 来证明第一个等式。
...
$$
与上述 Lemma \ref{chow-lemma-proper-pushforward-alteration} 的结果比较，
由 $g \in R(Y)^*$ 时
...
$f : X \to Y$ 支配，且 $\alpha = [X]$。此时结论由 Lemma
\ref{chow-lemma-equal-c1-as-cycles} 得出。
\end{proof}
...
由 Divisors, Lemma
\ref{divisors-lemma-divisor-meromorphic-locally-finite}，这样的集合存在。
于是
$$
...
$$
见 Section \ref{chow-section-tame-symbol}。
...
$$
而关键引理 Lemma \ref{chow-lemma-milnor-gersten-low-degree} 说明它为零。
\end{proof}
...
\bigoplus\nolimits_{\delta(x) = k}' K_0^M(\kappa(x))
$$
这里采用 Remark \ref{chow-remark-chow-group-pointwise} 中引入的记号和约定，
并另作如下约定：
\begin{enumerate}
\item $K_2^M(\kappa(x))$ 是点 $x \in X$ 的剩余域 $\kappa(x)$ 的
Milnor K-理论的次数 $2$ 部分（见 Remark
\ref{chow-remark-gersten-complex-milnor}）；它是
...
是 Section \ref{chow-section-tame-symbol} 中构造的驯顺符号。
\end{enumerate}
...
$$
关键代数引理 Lemma \ref{chow-lemma-milnor-gersten-low-degree}
恰好说明这个和为零。
\end{remark}
...
设 $(S, \delta)$ 如 Situation \ref{chow-situation-setup}。
设 $X$ 在 $S$ 上局部有限型，并设 $k \in \mathbf{Z}$。
Remark \ref{chow-remark-higher-chow-pointwise} 中的复形以及 Remark
\ref{chow-remark-chow-group-pointwise} 中 $\CH_k(X)$ 的表示提示我们，
可以定义第一高阶 Chow 群
$$
\CH^M_k(X, 1) =
...
$$
它与 Remark \ref{chow-remark-higher-chow-pointwise} 中各复形的边界映射相容。
应用蛇形引理（见 Homology, Lemma
\ref{homology-lemma-long-exact-sequence-chain}），得到六项正合列
$$
\CH^M_k(Y, 1) \to \CH^M_k(X, 1) \to \CH^M_k(U, 1) \to
\CH_k(Y) \to \CH_k(X) \to \CH_k(U) \to 0
$$
它延拓了 Lemma \ref{chow-lemma-restrict-to-open} 的典范正合列。
...
\begin{proof}
这立即由关键引理 Lemma \ref{chow-lemma-key-formula} 及其之前的讨论得出。
\end{proof}
...
需要证明 Definition \ref{chow-definition-cap-c1} 所定义的
$c_1(\mathcal{L}) \cap \alpha$ 为零。
由 Definition \ref{chow-definition-rational-equivalence}，存在局部有限族
...
由 Lemma \ref{chow-lemma-pushforward-cap-c1}，
...
由 Lemma \ref{chow-lemma-commutativity-on-integral} 可得
$$
c_1(\mathcal{L}) \cap \alpha = c_1(\mathcal{O}_X) \cap \beta.
$$
然而 Lemma \ref{chow-lemma-c1-cap-additive} 说明右边在 $\CH_k(X)$ 中为零，
正合所需。
\end{proof}
...
c_1(\mathcal{L}) \cap - : \CH_{k + 1}(X) \to \CH_k(X)
$$
由 Lemma \ref{chow-lemma-factors}，这个记法有意义。对所有 $s \geq 0$，
...
$(k + 2)$-循环。反复应用 Lemma \ref{chow-lemma-pushforward-cap-c1}，得到
$c_1(\mathcal{L}) \cap c_1(\mathcal{N}) \cap \alpha
...
此时结论由 Lemma \ref{chow-lemma-commutativity-on-integral} 和定义得出。
\end{proof}
...
$D$，可以更灵活地表述各种相容性，见 Remark
\ref{chow-remark-pullback-pairs} 以及 Lemmas
...
（Remark \ref{chow-remark-generalize-to-virtual}），或推广到伪除子
（Remark \ref{chow-remark-generalize-to-pseudo-divisor}）。
...
设 $X$ 是 Situation \ref{chow-situation-setup} 中那样在 $S$ 上局部有限型
...
Definition \ref{chow-definition-gysin-homomorphism} 中的构造
...
设 $(S, \delta)$ 如 Situation \ref{chow-situation-setup}，并设 $X$ 在 $S$ 上
局部有限型。设 $(\mathcal{L}, s, i : D \to X)$ 如 Definition
\ref{chow-definition-gysin-homomorphism}，而 $\alpha$ 是 $X$ 上的
$(k + 1)$-循环。那么在 $\CH_k(X)$ 中，
...
因此，对每个满足 $W_j \not \subset D$ 的 $j$，由 Lemma
\ref{chow-lemma-geometric-cap} 有
$c_1(\mathcal{L}) \cap [W_j] = [D \cap W_j]_k$。于是，由 Definition
\ref{chow-definition-cap-c1}，在 $\CH_k(X)$ 中有
$$
...
$$
右边逐项等于 Definition \ref{chow-definition-gysin-homomorphism} 中
...
设 $(S, \delta)$ 如 Situation \ref{chow-situation-setup}。
设 $X$ 在 $S$ 上局部有限型。设
$(\mathcal{L}, s, i : D \to X)$ 如 Definition
\ref{chow-definition-gysin-homomorphism}。
\begin{enumerate}
...
见 Lemma \ref{chow-lemma-cycle-closed-coherent}。因此 (1) 由 (2) 得出。
...
因此，该等式由 Lemma \ref{chow-lemma-additivity-divisors-restricted} 得出。
\end{proof}

\begin{remark}
\label{chow-remark-gysin-on-cycles}
设 $X \to S$、$\mathcal{L}$、$s$、$i : D \to X$ 如 Definition
\ref{chow-definition-gysin-homomorphism}，并假设
...
设 $f : X' \to X$ 是 Situation \ref{chow-situation-setup} 中那样的
$S$ 上局部有限型概形间的态射。设
$(\mathcal{L}, s, i : D \to X)$ 是 Definition
\ref{chow-definition-gysin-homomorphism} 中的三元组。可以置
...
设 $(\mathcal{L}, s, i : D \to X)$ 如 Definition
\ref{chow-definition-gysin-homomorphism}。依 Remark
\ref{chow-remark-pullback-pairs} 构成图表
$$
\xymatrix{
D' \ar[d]_g \ar[r]_{i'} & X' \ar[d]^f \\
D \ar[r]^i & X
...
$$
因此，由 Lemma \ref{chow-lemma-equal-c1-as-cycles}，第一个循环沿
$f_*$ 的像等于第二个循环，故等式作为循环成立。
若 $W' \subset D'$，则 $W \subset D$；由 Lemma
\ref{chow-lemma-equal-c1-as-cycles} 的第二个断言，
$f_*(c_1(\mathcal{L}|_{W'}) \cap [W'])$ 在 $\CH_k(W)$ 中等于
$c_1(\mathcal{L}|_W) \cap [W]$。
由 Remark \ref{chow-remark-infinite-sums-rational-equivalences}，
对一般的 $\alpha'$ 也得到结论。
\end{proof}
...
并有 $[W']_{k + 1 + r} = g^*[W]$；此时等式由 Lemma
\ref{chow-lemma-flat-pullback-cap-c1} 在 $\CH_{k + r}(D')$ 中成立。
由 Remark \ref{chow-remark-infinite-sums-rational-equivalences}，
对一般的 $\alpha'$ 也得到结论。
\end{proof}
...
表示典范截面。将把 Section \ref{chow-section-key} 中的讨论应用于当前情形。
...
$s_i|_{Z_i} = s|_{Z_i}$ 是正则截面；见 Lemma
\ref{chow-lemma-compute-c1}。类似地，
$$
c_1(\mathcal{N}) \cap [D]_{n - 1} =
\sum \text{ord}_{B_i}(f_i) \text{div}_{\mathcal{N}|_{Z_i}}(t_i|_{Z_i})
$$
关键公式（Lemma \ref{chow-lemma-key-formula}）给出循环等式
$$
\sum \left(
\text{ord}_{B_i}(f_i) \text{div}_{\mathcal{N}|_{Z_i}}(t_i|_{Z_i}) -
\text{ord}_{B_i}(g_i) \text{div}_{\mathcal{L}|_{Z_i}}(s_i|_{Z_i}) \right) =
...
设 $X$ 在 $S$ 上局部有限型。设
$(\mathcal{L}, s, i : D \to X)$ 如 Definition
\ref{chow-definition-gysin-homomorphism}。Gysin 同态下降到有理等价类，
...
置 $X' = \coprod W_i$，并考虑 Remark
\ref{chow-remark-pullback-pairs} 中的图表
$$
\xymatrix{
...
$$
由于 $X' \to X$ 固有，由 Lemma \ref{chow-lemma-closed-in-X-gysin} 有
...
$c_1(\mathcal{L}) \cap \alpha$，而由 Lemma \ref{chow-lemma-factors}，
它有理等价于零。若 $D \not = X$，则由 Lemma
\ref{chow-lemma-gysin-factors-general}，$i^*\text{div}_X(f)$ 在
$\CH_{n - 2}(D)$ 中等于
...
设 $(S, \delta)$ 如 Situation \ref{chow-situation-setup}，并设 $X$ 在 $S$ 上
局部有限型。设 $(\mathcal{L}, s, i : D \to X)$ 如 Definition
\ref{chow-definition-gysin-homomorphism}。则
$i^*i_* : \CH_k(D) \to \CH_{k - 1}(D)$ 把 $\alpha$ 映为
$c_1(\mathcal{L}|_D) \cap \alpha$。
\end{lemma}

\begin{proof}
这立即由循环上 $i_*$ 的定义以及 Definition
\ref{chow-definition-gysin-homomorphism} 中 $i^*$ 的定义得出。
\end{proof}

\begin{lemma}
\label{chow-lemma-gysin-commutes-cap-c1}
设 $(S, \delta)$ 如 Situation \ref{chow-situation-setup}，并设 $X$ 在 $S$ 上
局部有限型。设 $(\mathcal{L}, s, i : D \to X)$ 是 Definition
\ref{chow-definition-gysin-homomorphism} 中的三元组。
...
\begin{proof}
采用与 Lemma \ref{chow-lemma-gysin-factors} 完全相同的证明，
结论由 Lemmas \ref{chow-lemma-pushforward-cap-c1}、
...
$(\mathcal{L}', s', i' : D' \to X)$ 是 Definition
\ref{chow-definition-gysin-homomorphism} 中的两个三元组。则图表
$$
\xymatrix{
...
$$
如 Section \ref{chow-section-key} 的讨论。于是，若 $n_i \not = 0$，
则 $Z_i \subset D$；若 $n'_i \not = 0$，则 $Z'_i \subset D'$。
对每个 $i$，设 $\xi_i \in Z_i$ 为一般点。与 Section
\ref{chow-section-key} 中一样，对每个 $i$ 选取元素
...
$$
此时关键公式（Lemma \ref{chow-lemma-key-formula}）给出循环等式
$$
\sum \left(
...
\noindent
相对有效 Cartier 除子在 Divisors, Section
\ref{divisors-section-effective-Cartier-morphisms} 中定义并研究。
...
由 Divisors, Lemma \ref{divisors-lemma-relative-Cartier}，
$D \cap p^{-1}W$ 是 $p^{-1}W$ 上的有效 Cartier 除子。
由 Lemma \ref{chow-lemma-easy-gysin}，下式中的第一个等式成立：
$$
i^*[p^{-1}W]_{k + r + 1} =
[D \cap p^{-1}W]_{k + r} =
[(p|_D)^{-1}(W)]_{k + r}.
...
这就证明了第一个等式。再应用 Lemma
\ref{chow-lemma-support-cap-effective-Cartier}，即可由第一个等式推出第二个。
\end{proof}
...
（见 Remark \ref{chow-remark-infinite-sums-rational-equivalences}）。
...
$\alpha|_{f^{-1}(V)} = f^*\beta$。由 Lemma
\ref{chow-lemma-exact-sequence-open}，对某个 $\beta' \in Z_k(Y)$ 有
$\beta = \beta'|_V$。由 Lemma \ref{chow-lemma-restrict-to-open} 的正合列
...
\begin{proof}
满射性见 Lemma \ref{chow-lemma-pullback-affine-fibres-surjective}。
...
Cartier 除子。故由 Lemma \ref{chow-lemma-relative-effective-cartier}，
$o^* \circ p^* = \text{id}$，从而 $p^*$ 也是单射。
\end{proof}

\begin{remark}
\label{chow-remark-when-isomorphism}
后文将看到（Lemma \ref{chow-lemma-vectorbundle}），若 $X$ 是 $Y$ 上秩为
$r$ 的向量丛，则拉回映射 $\CH_k(Y) \to \CH_{k + r}(X)$ 是同构。
只要 $X \to Y$ 满足 Lemma
\ref{chow-lemma-pullback-affine-fibres-surjective} 的假设，这一结论就成立；
见 \cite[Lemma 2.2]{Totaro-group}。在 Remark
\ref{chow-remark-higher-chow-isomorphism} 中，我们将用高阶 Chow 群勾勒一个证明。
\end{remark}

\begin{lemma}
\label{chow-lemma-linebundle-formulae}
在 Lemma \ref{chow-lemma-linebundle} 的情形中，以 $o : X \to L$
表示零截面（见该引理的证明）。则有
\begin{enumerate}
...
（Cohomology, Lemma \ref{cohomology-lemma-projection-formula}）给出
...
\medskip\noindent
(2)、(3) 和 (4) 已在 Lemma \ref{chow-lemma-linebundle} 的证明过程中看到。
当然，(4) 正是 Lemma \ref{chow-lemma-relative-effective-cartier} 中的第一个公式。

\medskip\noindent
(5) 由 Lemma \ref{chow-lemma-relative-effective-cartier} 中的第二个公式、
与 $c_1$ 作帽积的可加性（Lemma \ref{chow-lemma-c1-cap-additive}），
以及与 $c_1$ 作帽积和平坦拉回交换这一事实
（Lemma \ref{chow-lemma-flat-pullback-cap-c1}）得出。

\medskip\noindent
(6) 由 Lemma \ref{chow-lemma-gysin-back} 以及
...
在 Lemma \ref{chow-lemma-decompose-section} 的情形中，假设 $Y$ 是
Situation \ref{chow-situation-setup} 中那样在 $(S, \delta)$ 上局部有限型。
...
\medskip\noindent
假设 $W \subset D$。由 Gysin 同态的定义以及 Lemma
\ref{chow-lemma-c1-cap-additive} 的可加性，在 $\CH_{k - 1}(D)$ 中有
...
由于 $W_1 \cap W_2$ 是线丛 $W_1 \to W$ 的零截面，Lemma
\ref{chow-lemma-linebundle-formulae} 给出
...
在 Situation \ref{chow-situation-setup} 中，设 $X$ 是 $S$ 上局部有限型的概形。
设 $(\mathcal{L}, s, i : D \to X)$ 是 Definition
\ref{chow-definition-gysin-homomorphism} 中的三元组。则存在交换图
$$
\xymatrix{
...
\begin{proof}
注意，$(i')^*$ 已有定义，因为存在 Definition
\ref{chow-definition-gysin-homomorphism} 中那样的三元组
$(\mathcal{O}_{X'}, s', i' : D' \to X')$。因此该陈述有意义。

\medskip\noindent
置 $\mathcal{L}_1 = \mathcal{O}_X$、$\mathcal{L}_2 = \mathcal{L}$，
并把 Lemma \ref{chow-lemma-decompose-section} 应用于
$\mathcal{L} = \mathcal{L}_1 \otimes_{\mathcal{O}_X} \mathcal{L}_2$
的截面 $s$。取 $D' = D_1$。此时结论由该引理和 Lemma
\ref{chow-lemma-decompose-section-formulae} 得出，而单射性由 Lemma
\ref{chow-lemma-linebundle} 得出。
\end{proof}
...
首先，由 Lemma \ref{chow-lemma-pullback-affine-fibres-surjective}，
...
$$
这里使用了 $V$ 和 $U$ 的第一高阶 Chow 群、Remark
\ref{chow-remark-higher-chow} 中构造的六项正合列，以及这些高阶 Chow 群
...
$$
为满射即可完成论证。仿照 Lemma
\ref{chow-lemma-pullback-affine-fibres-surjective} 的证明进行论证，
这又约化到下面的第 3 步。
...
后文将在 Lemma \ref{chow-lemma-gysin-commutes} 中看到，我们的双变类
...
\item 若 $(\mathcal{L}', s', i' : D' \to Y')$ 如 Definition
\ref{chow-definition-gysin-homomorphism}，且其到 $X'$ 的拉回为
...
\noindent
设 $(S, \delta)$ 如 Situation \ref{chow-situation-setup}。设 $X \to Y$
...
\begin{proof}
这由 Lemmas \ref{chow-lemma-flat-pullback-rational-equivalence}、
...
\begin{proof}
这由 Lemmas \ref{chow-lemma-gysin-factors}、
\ref{chow-lemma-closed-in-X-gysin}、\ref{chow-lemma-gysin-flat-pullback} 和
...
\begin{proof}
这由 Lemmas \ref{chow-lemma-compose-pushforward}、
\ref{chow-lemma-flat-pullback-proper-pushforward} 和
...
$$
这里 $res(c_1) \in A^{p_1}(Z_1 \times_X Z_2 \to Z_2)$ 是 Remark
\ref{chow-remark-restriction-bivariant} 中那样的 $c_1$ 的限制；
$res(c_2)$ 类似。
\end{remark}
...
是双变类。则 Lemma \ref{chow-lemma-gysin-bivariant} 的双变 Gysin 类
$c' \in A^1(D \to X)$ 按 Remark
\ref{chow-remark-bivariant-commute} 的意义与 $c$ 交换。
这正是 Definition \ref{chow-definition-bivariant-class} 中条件 (3) 的重述。
\end{example}
...
$$
其中各概形如 Situation \ref{chow-situation-setup} 中那样在
...
\item 给定 Definition \ref{chow-definition-gysin-homomorphism} 中那样的
$D' \subset Z'$ 时的 Gysin 映射。
\end{enumerate}
这里略去详细表述。把所有这类运算所成的集合记为
$A^p(X \to Z \leftarrow Y)$。这个概念有用的一个简单例子，是存在固有态射
$f : X_2 \to X_1$ 的情形。此时，按 Definition
\ref{chow-definition-bivariant-class} 的意义，$f_*$ 不是双变运算；
但按上述推广意义它是，即
...
作为 Remark \ref{chow-remark-restriction-bivariant} 的一个特例，
...
\begin{proof}
这由 Lemmas \ref{chow-lemma-factors}、\ref{chow-lemma-pushforward-cap-c1}、
...
设 $(S, \delta)$ 如 Situation \ref{chow-situation-setup}。设 $X$ 在 $S$ 上
局部有限型，$\mathcal{L}$ 是可逆 $\mathcal{O}_X$-模。
$\mathcal{L}$ 的{\it 第一陈类}
$c_1(\mathcal{L}) \in A^1(X)$ 是 Lemma
\ref{chow-lemma-cap-c1-bivariant} 的双变类。
\end{definition}

\noindent
有限局部自由模的陈类将在 Section
\ref{chow-section-intersecting-chern-classes} 中构造。下面证明
...
\begin{proof}
当然，(2) 蕴含 (1)。设 $p : L \to X$ 如 Lemma
\ref{chow-lemma-linebundle}，并设 $o : X \to L$ 为零截面。
以 $p' : L' \to X'$ 和 $o' : X' \to L'$ 表示它们的基变换。
由 Lemma \ref{chow-lemma-linebundle-formulae}，有
$$
...
集合，所以每个可逆模都平凡。假设 $d > 0$。由 Divisors, Lemma
...
表示 Lemma \ref{chow-lemma-gysin-bivariant} 中由 Gysin 同态给出的双变类。
由 Lemma \ref{chow-lemma-support-cap-effective-Cartier}，在 $A^1(X)$ 中有
$i_* \circ i^* = c_1(\mathcal{L}_{d + 1})$
（左边的意义由 Lemma \ref{chow-lemma-push-proper-bivariant} 保证）。
...
$A^i(X) \to A^i(Z)$（Remark \ref{chow-remark-restriction-bivariant}）
以及复合 $A^i(Z) \times A^j(Z \to X) \to A^{i + j}(Z \to X)$。
对第三个映射，把 $(c, c')$ 映为 $res(c) \circ c'$，其中
$res : A^i(Z \to X) \to A^i(Z)$ 是限制映射
（见 Remark \ref{chow-remark-restriction-bivariant}）。
...
以 $res : A^p(Z \to X) \to A^p(Z)$ 表示 Remark
\ref{chow-remark-restriction-bivariant} 的限制映射。
...
$\mathcal{L}'|_{D'} \cong \mathcal{O}_{D'}$ 时，该规则满足 Definition
\ref{chow-definition-bivariant-class} 的条件 (3)。则
$c \cap -$ 下降到有理等价类。
\end{lemma}

\begin{proof}
该陈述有意义：若给定 Definition
\ref{chow-definition-gysin-homomorphism} 中那样的三元组
$(\mathcal{L}, s, i : D \to X)$，且
$\mathcal{L}|_D \cong \mathcal{O}_D$，则运算 $i^*$
已在循环层次上定义；见 Remark \ref{chow-remark-gysin-on-cycles}。
设 $\alpha \in Z_k(X')$ 是有理等价于零的循环。需要证明
$c \cap \alpha = 0$。由 Lemma \ref{chow-lemma-rational-equivalence-family}，
...
（由 Lemma \ref{chow-lemma-rational-equivalence-family}），结论成立。
\end{proof}
...
c \cap - : \CH_k(Y') \longrightarrow \CH_{k - p}(X')
$$
其中 $Y' = X' \times_X Y$；该规则满足 Definition
\ref{chow-definition-bivariant-class} 的条件 (1)、(2)，并且每当
...
$(\mathcal{L}', s', i' : D' \to Y')$ 如 Definition
\ref{chow-definition-gysin-homomorphism}，其到 $X'$ 的拉回为
$(\mathcal{N}', t', j' : E' \to X')$。
...
\medskip\noindent
以 $g : Y'' \to Y'$ 表示 Lemma
\ref{chow-lemma-normal-cone-effective-Cartier} 中构造的相对维数为 $1$
...
分别是 $X$、$Y$ 到各连通分支的分解
（因为 $X$、$Y$ 局部 Noether，连通分支是开的；见 Topology, Lemma
\ref{topology-lemma-locally-Noetherian-locally-connected} 和
Properties, Lemma \ref{properties-lemma-Noetherian-topology}）。
设 $a(i) \in J$ 是满足 $f(X_i) \subset Y_{a(i)}$ 的指标。
由 Lemma \ref{chow-lemma-disjoint-decomposition-bivariant}，
...
设 $(S, \delta)$ 如 Situation \ref{chow-situation-setup}。
设 $f : X \to Y$ 是 $S$ 上局部有限型概形间的态射。
设 $g : Y' \to Y$ 是包络（Definition \ref{chow-definition-envelope}），
...
方块的限制（见 Remark \ref{chow-remark-restriction-bivariant}）；
元素 $res_2(c')$ 是 $c'$ 关于由态射 $b, f', q, f''$ 组成的 Cartesian
方块的限制。假设 $res_1(c') = res_2(c')$，并设
$\beta \in \CH_k(Y)$。由 Lemma \ref{chow-lemma-envelope}，可以找到
...
$Y_1 \to Y$ 构造满足 Definition \ref{chow-definition-bivariant-class}
所列条件的映射
$c \cap -: \CH_k(Y_1) \to \CH_{k + p}(Y_1 \times_Y X)$。
记 $Y'_1 = Y' \times_Y Y_1$、$X_1 = X \times_Y Y_1$。
由 Lemma \ref{chow-lemma-base-change-envelope}，态射
...
\medskip\noindent
接下来，需要验证 $c$ 满足 Definition
\ref{chow-definition-bivariant-class} 的条件 (1)、(2)、(3)。
...
因此，反复应用 Lemma \ref{chow-lemma-geometric-cap}，依次得到
\begin{eqnarray*}
\pi^*[Z] & = & [\pi^{-1}(Z)] \\
...
且映到零。由 Lemma \ref{chow-lemma-cap-projective-bundle} 可知
$$
0 = \pi_*(\pi^*\alpha_0 +
c_1(\mathcal{O}_P(1)) \cap \pi^*\alpha_1
+ \ldots +
...
Lemma \ref{chow-lemma-flat-pullback-proper-pushforward}。由
Lemma \ref{chow-lemma-pushforward-cap-c1}，与
...
Lemma \ref{chow-lemma-restrict-to-open} 看出，
...
再次应用 Lemma \ref{chow-lemma-pullback-affine-fibres-surjective}，
可对某个 $\alpha_1 \in \CH_{k + 1}(X)$ 写成
$\alpha'|_{U^{r - 2}} = (\pi^{r - 2})^*\alpha_1|_{U^{r - 2}}$。
由 Lemma \ref{chow-lemma-relative-effective-cartier}，
$(\pi^{r - 2})^*\alpha_1$ 的像表示
$c_1(\mathcal{O}_P(1)) \cap \pi^*\alpha_1$。
还可看出，
...
\begin{proof}
（秩一向量丛的情形见 Lemma \ref{chow-lemma-linebundle}。）
满射性见 Lemma \ref{chow-lemma-pullback-affine-fibres-surjective}。
...
（见 Divisors, Lemma \ref{divisors-lemma-characterize-OD}）。
...
Lemma \ref{chow-lemma-restrict-to-open}。所以，若 $p^*\alpha = 0$，
则对某个 $\beta \in \CH_{k + r}(D)$ 有
$\pi^*\alpha = i_*\beta$。由
Lemma \ref{chow-lemma-chow-ring-projective-bundle}，可写成
$$
\beta = \pi_D^*\beta_0 +
\ldots + c_1(\mathcal{O}_D(1))^{r - 1} \cap \pi_D^* \beta_{r - 1}.
$$
其中 $\beta_i \in \CH_{k + i}(X)$。由 Lemmas
\ref{chow-lemma-relative-effective-cartier} 和
...
Lemma \ref{chow-lemma-pushforward-cap-c1} 矛盾。
\end{proof}
...
\begin{enumerate}
\item 由 Lemma \ref{chow-lemma-chow-ring-projective-bundle}，存在元素
...
$\mathcal{O}_X$-模。Definition \ref{chow-definition-chern-classes}
...
Definition \ref{chow-definition-divisor-invertible-sheaf} 中的构造。
...
Lemma \ref{chow-lemma-cap-cp-bivariant} 及该引理之后的讨论。
我们的构造遵循熟知的模式：先在素循环上定义运算，再求和。
在 Lemma \ref{chow-lemma-determine-intersections} 中，将证明结果由
相应射影丛上的通常公式所确定。随后证明，与陈类作帽积可下降到
有理等价，并与正常推前、平坦拉回以及有效 Cartier 除子嵌入的
Gysin 映射交换。也可以直接使用
Lemma \ref{chow-lemma-determine-intersections} 中的刻画和
Lemma \ref{chow-lemma-push-proper-bivariant}，从而避开这些引理；
希望看到具体推导的读者可参阅 Chow Groups of Spaces,
Lemma \ref{spaces-chow-lemma-segre-classes}。
...
Definition \ref{chow-definition-chern-classes} 中所定义。
...
（Definition \ref{chow-definition-cap-c1}）一致，这是
Lemma \ref{chow-lemma-first-chern-class} 的结论。
...
Lemma \ref{chow-lemma-flat-pullback-proper-pushforward}，有
$\pi^*p_*\beta_j = p'_*(\pi')^*\beta_j$。
再由 Lemma \ref{chow-lemma-pushforward-cap-c1} 的投影公式，得到
$$
\sum\nolimits_{i = 0}^r
...
\begin{proof}
由 Lemma \ref{chow-lemma-determine-intersections}，元素
...
Lemma \ref{chow-lemma-flat-pullback-rational-equivalence}。）因此它们相等。
\end{proof}
...
$\alpha_0 = \alpha$。由 Lemma \ref{chow-lemma-determine-intersections}，有
$$
\sum\nolimits_{i = 0}^r
(-1)^i c_1(\mathcal{O}_P(1))^i \cap
\pi_X^*(\alpha_{r - i}) = 0
...
对上述等式施以正常推前 $p'_*$
（Lemma \ref{chow-lemma-proper-pushforward-rational-equivalence}）。
利用 Lemmas \ref{chow-lemma-pushforward-cap-c1} 和
...
$$
在 $P$ 的 Chow 群中成立。由
Lemma \ref{chow-lemma-determine-intersections} 的刻画即得结论。
\end{proof}
...
对上述方程施以平坦拉回 $(f')^*$
（Lemma \ref{chow-lemma-flat-pullback-rational-equivalence}）。由
Lemmas \ref{chow-lemma-flat-pullback-cap-c1} 和
...
如 Definition \ref{chow-definition-gysin-homomorphism}。
...
对上述方程施以 Gysin 映射 $(i')^*$
（Lemma \ref{chow-lemma-gysin-factors}）。应用 Lemmas
\ref{chow-lemma-gysin-commutes-cap-c1} 和
...
以及 Definition \ref{chow-definition-bivariant-class}。
\end{proof}
...
{\it 第 $i$ 陈类}是 Lemma \ref{chow-lemma-cap-cp-bivariant}
所构造的次数为 $i$ 的双变类
...
\noindent
由 Definition \ref{chow-definition-cap-chern-classes} 后的注记，
若 $\mathcal{E}$ 可逆，则这个定义与
Definition \ref{chow-definition-first-chern-class} 一致。
...
Lemma \ref{chow-lemma-c1-center} 及双变类的性质，得到
\begin{align*}
0 & = c \cap \left(\sum\nolimits_{i = 0}^r
(-1)^i c_1(\mathcal{O}_P(1))^i \cap
\pi^*(\alpha_{r - i})\right) \\
...
Lemma \ref{chow-lemma-determine-intersections} 的刻画，
$c \cap \alpha_j$ 等于
...
由 Lemma \ref{chow-lemma-disjoint-decomposition-bivariant}，有
...
\in A^*(X)^\wedge
$$
参见 Remark \ref{chow-remark-completion-bivariant}。
...
\noindent
设 $(S, \delta)$ 如 Situation \ref{chow-situation-setup}，设 $X$ 在 $S$
上局部有限型，并设 $\mathcal{E}_i$ 是 $X$ 上有限局部自由层的有限族。
由 Lemma \ref{chow-lemma-cap-commutative-chern}，陈类
$$
...
$$
都为零。由 Lemma \ref{chow-lemma-bivariant-zero}，这等价于如下要求：
...
\medskip\noindent
一个具体例子是 Lemma \ref{chow-lemma-c1-cap-additive} 中证明的关系
$$
...
特别地，它应在 $X$ 整时成立；由 Lemma \ref{chow-lemma-bivariant-zero}，
...
$$
参见 Constructions, Lemma \ref{constructions-lemma-twisting-and-proj}。
它满足
$g^*\mathcal{O}_{P'}(1)
...
\begin{proof}
由 Lemma \ref{chow-lemma-bivariant-zero}，只须证明：若 $X$ 整，则
...
Lemma \ref{chow-lemma-relative-effective-cartier}，对 $X$ 上任意循环类
$\alpha$ 有
$$
c_1(\mathcal{O}_P(1)) \cap \pi^*\alpha = i_*((\pi')^*\alpha)
$$
由 Lemma \ref{chow-lemma-determine-intersections}，
...
$$
把它推前到 $P$，并使用上述注记及
Lemma \ref{chow-lemma-pushforward-cap-c1}，得到
$$
\sum\nolimits_{j = 0}^{r - 1}
(-1)^j c_1(\mathcal{O}_P(1))^{j + 1}
\cap \pi^*\alpha_j = 0
$$
由 Lemma \ref{chow-lemma-determine-intersections} 中的唯一性，可得
...
\begin{proof}
由 Lemma \ref{chow-lemma-bivariant-zero}，可设 $X$ 整，并只须证明与
$[X]$ 作帽积后的恒等式。对 $r_1$ 归纳。当 $r_1 = 1$ 时，结论是
Lemma \ref{chow-lemma-additivity-invertible-subsheaf}。设 $r_1 > 1$。
...
\end{eqnarray*}
由于 $[P] = \pi^*[X]$，由
Lemma \ref{chow-lemma-flat-pullback-cap-cj} 即得结论。
\end{proof}
...
\begin{proof}
应用 Lemma \ref{chow-lemma-additivity-invertible-subsheaf} 并作归纳。
\end{proof}
...
Lemma \ref{chow-lemma-spell-out-degree-zero-cycle} 直接定义，
再由 Lemma \ref{chow-lemma-curve-principal-divisor} 证明它良定义。
这里改用如下定义。

\begin{definition}
\label{chow-definition-degree-zero-cycle}
设 $k$ 是域（Example \ref{chow-example-field}），并设
...
\noindent
下面把它与 Varieties, Section \ref{varieties-section-divisors-curves}
...
$$
其中左端由 Varieties, Definition
\ref{varieties-definition-degree-invertible-sheaf} 定义。
\end{lemma}
...
由 Varieties, Lemma \ref{varieties-lemma-degree-in-terms-of-components}，
$\mathcal{E}$ 的次数也有类似分解，故只须证明 $X$ 是 $k$ 上正常曲线的情形。

\medskip\noindent
假设 $X$ 是 $k$ 上正常曲线。由 Divisors, Lemma
\ref{divisors-lemma-filter-after-modification}，存在修改
$f : X' \to X$，使 $f^*\mathcal{E}$ 有一个滤过，其逐次商是可逆
$\mathcal{O}_{X'}$-模。由于 $f_*[X']_1 = [X]_1$，由
Lemma \ref{chow-lemma-pushforward-cap-cj} 可得
$$
\deg(c_1(\mathcal{E}) \cap [X]_1) = \deg(c_1(f^*\mathcal{E}) \cap [X']_1)
$$
由 Varieties, Lemma \ref{varieties-lemma-degree-birational-pullback}，
次数也满足类似关系，因此归约到 $\mathcal{E}$ 有逐次商为可逆
$\mathcal{O}_X$-模的滤过这一情形。此时利用次数的可加性
（Varieties, Lemma \ref{varieties-lemma-degree-additive}）以及第一陈类的可加性
（Lemma \ref{chow-lemma-additivity-chern-classes}），可归约到下一段的情形。

\medskip\noindent
假设 $X$ 是 $k$ 上正常曲线，且 $\mathcal{E}$ 是可逆
$\mathcal{O}_X$-模。由 Divisors, Lemma
\ref{divisors-lemma-quasi-projective-Noetherian-pic-effective-Cartier}，
$\mathcal{E}$ 同构于
$\mathcal{O}_X(D) \otimes \mathcal{O}_X(D')^{\otimes -1}$，
其中 $D, D'$ 是 $X$ 上的有效 Cartier 除子（这里还用到 $X$ 射影；
例如参见 Varieties, Lemma \ref{varieties-lemma-dim-1-proper-projective}）。
利用可逆层张量积下次数的可加性
（Varieties, Lemma \ref{varieties-lemma-degree-tensor-product}）
以及可逆层张量积下 $c_1$ 的可加性
（Lemma \ref{chow-lemma-c1-cap-additive} 或 \ref{chow-lemma-chern-classes-E-tensor-L}），
归约到 $\mathcal{E} = \mathcal{O}_X(D)$ 的情形。此时左端给出
$\deg(D)$（Varieties, Lemma
\ref{varieties-lemma-degree-effective-Cartier-divisor}），而由
Lemma \ref{chow-lemma-geometric-cap}，右端给出 $\deg([D]_0)$。
根据定义，
$$
[D]_0 = \sum\nolimits_{x \in D}
...
$$
倒数第二个等式由 Algebra, Lemma
\ref{algebra-lemma-pushdown-module} 以及 $D$ 为仿射这一事实得到。
\end{proof}

\noindent
最后，把以上内容与 Varieties, Section
\ref{varieties-section-num} 所定义的数值交数联系起来。
...
$$
其中左端由 Varieties, Definition
\ref{varieties-definition-intersection-number} 定义。特别地，
...
\begin{proof}
对 $d$ 归纳。若 $d = 0$，结论由 Varieties, Lemma
\ref{varieties-lemma-chi-tensor-finite} 成立。设 $d > 0$。
...
在 $k$ 上 H-射影。参见 Cohomology of Schemes, Lemma
\ref{coherent-lemma-chow-Noetherian} 和 Remark
\ref{coherent-remark-chow-Noetherian}。此时，由 Varieties, Lemma
\ref{varieties-lemma-intersection-number-and-pullback} 有
$$
(f^*\mathcal{L}_1 \cdots f^*\mathcal{L}_d \cdot Y) =
(\mathcal{L}_1 \cdots \mathcal{L}_d \cdot X)
$$
并由 Lemma \ref{chow-lemma-pushforward-cap-c1} 有
$$
...
\medskip\noindent
若 $X$ 是正常的 $d$ 维射影簇，则由 Divisors, Lemma
\ref{divisors-lemma-quasi-projective-Noetherian-pic-effective-Cartier}，
可对某些有效 Cartier 除子 $D, D' \subset X$ 写成
$\mathcal{L}_1 = \mathcal{O}_X(D) \otimes \mathcal{O}_X(D')^{\otimes -1}$。
利用等式两端的可加性（Varieties, Lemma
\ref{varieties-lemma-intersection-number-additive} 和
Lemma \ref{chow-lemma-c1-cap-additive}），可归约到
$\mathcal{L}_1 = \mathcal{O}_X(D)$，其中 $D$ 是有效 Cartier 除子。
由 Varieties, Lemma
\ref{varieties-lemma-numerical-intersection-effective-Cartier-divisor}，有
$$
(\mathcal{L}_1 \cdots \mathcal{L}_d \cdot X) =
(\mathcal{L}_2 \cdots \mathcal{L}_d \cdot D)
$$
而由 Lemma \ref{chow-lemma-geometric-cap}，有
$$
c_1(\mathcal{L}_1) \cap \ldots \cap c_1(\mathcal{L}_d) \cap [X] =
...
设 $(S, \delta)$ 如 Situation \ref{chow-situation-setup}。
设 $X$ 在 $S$ 上局部有限型。如 Section \ref{chow-section-setup}，记
...
只须 $X$ 是 $(S_2)$。可与 Local Cohomology, Lemma
\ref{local-cohomology-lemma-catenary-S2-equidimensional} 比较。}。
\end{lemma}
...
是 $X$ 上的维数函数（见 Example \ref{chow-example-CM-irreducible}）。
由于 $\delta - \delta'$ 局部常值
...
\noindent
设 $(S, \delta)$ 如 Situation \ref{chow-situation-setup}。设 $X$ 在 $S$
上局部有限型，并满足 Lemma \ref{chow-lemma-locally-equidimensional}
中的等价条件。对整闭子概形 $Z \subset X$，有 $Z$ 在 $X$ 中的
余维数 $\text{codim}(Z, X)$，见 Topology, Definition
\ref{topology-definition-codimension}。定义一个{\it 余维数 $p$ 的循环}
为 $X$ 上满足
$n_Z \not = 0 \Rightarrow \text{codim}(Z, X) = p$ 的循环
$\alpha = \sum n_Z[Z]$。所有余维数 $p$ 的循环所成的阿贝尔群记作
$Z^p(X)$。令 $X = \coprod X_n$ 为
Lemma \ref{chow-lemma-locally-equidimensional} (1) 中的分解。
回忆循环被定义为局部有限和，显然
$$
...
Lemma \ref{chow-lemma-locally-equidimensional} 的 $X$，可以从头重新发展
...
$\CH^*(X)$ 的自然对应版本。证明方法是如
Lemma \ref{chow-lemma-locally-equidimensional} 把有关概形分解成“等维”部分，
再对上述积分解中的各因子应用已经证明的结果。列举如下：
\begin{enumerate}
\item 若 $f : X \to Y$ 是 $S$ 上局部有限型概形之间的平坦态射，
且 $X$、$Y$ 满足 Lemma \ref{chow-lemma-locally-equidimensional} 的等价条件，
则平坦拉回给出映射
$$
f^* : \CH^p(Y) \to \CH^p(X)
$$
...
Lemma \ref{chow-lemma-locally-equidimensional} 的等价条件，且
...
设 $(S, \delta)$ 如 Situation \ref{chow-situation-setup}。
设 $X$ 在 $S$ 上局部有限型，并满足
Lemma \ref{chow-lemma-locally-equidimensional} 的等价条件。
...
（Remark \ref{chow-remark-pullback-cohomology}）是单射。
\end{lemma}
...
（Lemma \ref{chow-lemma-chow-ring-projective-bundle}），在任意基变换后，
...
Lemma \ref{chow-lemma-splitting-principle} 的证明还表明态射
$\pi : P \to X$ 具有下列性质：
\begin{enumerate}
...
\end{enumerate}
第二点由第一点及 Lemma \ref{chow-lemma-cap-projective-bundle} 得出。
以后可按需要在此添加更多说明。
\end{remark}

\noindent
设 $(S, \delta)$、$X$ 和 $\mathcal{E}_i$ 如
Lemma \ref{chow-lemma-splitting-principle}。所谓{\it 分裂原理}，是指形式地写成
$$
...
\medskip\noindent
具体地，设 $\pi : P \to X$ 如 Lemma \ref{chow-lemma-splitting-principle}。
回忆有规范的 $\mathbf{Z}$-代数映射
$\pi^* : A^*(X) \to A^*(P)$，见
Remark \ref{chow-remark-pullback-cohomology}。对 $X$ 上每个 $Y$，
$\pi_Y^*$ 在 Chow 群上的单射性推出映射
$\pi^* : A^*(X) \to A^*(P)$ 是单射（略去细节）。由
Lemma \ref{chow-lemma-chern-filter-by-linebundles}，有
$$
...
在 Situation \ref{chow-situation-setup} 中，设 $X$ 在 $S$ 上局部有限型。
...
\begin{proof}
选取 Lemma \ref{chow-lemma-splitting-principle} 中的态射
...
由 Lemma \ref{chow-lemma-c1-cap-additive}，有
$c_1(\mathcal{L}_i^{\otimes -1}) = - x_i$。由
Lemma \ref{chow-lemma-chern-filter-by-linebundles}，在 $A^*(X)$ 中有
$$
c(\mathcal{E}) = \prod\nolimits_{i = 1}^r (1 + x_i)
\quad\text{且}\quad
...
\begin{proof}
完全仿照 Lemma \ref{chow-lemma-chern-classes-dual} 的证明，可以设有可逆
...
$$
由 Lemma \ref{chow-lemma-chern-filter-by-linebundles}，在 $A^*(X)$ 中有
$$
...
\in A^p(X)
$$
记号与约定见 Remark \ref{chow-remark-extend-to-finite-locally-free}。
...
\mathcal{E} \otimes \mathcal{E}^\vee$，该式容易从上述 Lemmas
\ref{chow-lemma-chern-classes-dual} 和
\ref{chow-lemma-chern-classes-tensor-product} 得出：只须把 $X$ 如
Remark \ref{chow-remark-extend-to-finite-locally-free} 分解成
$\mathcal{E}$ 的秩为常数的部分。
\end{remark}
...
（如 Remarks \ref{chow-remark-extend-to-finite-locally-free} 和
...
\medskip\noindent
设 $(S, \delta)$ 如 Situation \ref{chow-situation-setup}。设 $X$ 是 $S$
...
（Lemma \ref{chow-lemma-cap-cp-bivariant}），可以设 $X = X'$，
并须证明 $\CH_{n - r}(X)$ 中的等式
$c_r(\mathcal{E}) \cap [X]_n = [Z(s)]_{n - r}$。
对 $r \geq 0$ 归纳。（$r = 0$ 的情形显然。）当 $r = 1$ 时，
结论由 Lemma \ref{chow-lemma-geometric-cap} 给出。设 $r > 1$。
...
$$
由射影空间丛公式（Lemma \ref{chow-lemma-chow-ring-projective-bundle}），
只须在沿 $\pi$ 拉回后证明该等式。注意，
$\pi^{-1}Z(s) = Z(\pi^*s)$ 的 $\delta$-维数 $\leq n - 1$，
且在 $\delta$-维数为 $n - 1$ 的一般点处关于正则序列的假设，
由平坦拉回仍成立；见 Algebra, Lemma
\ref{algebra-lemma-flat-increases-depth}。设
...
\medskip\noindent
证明断言。它来自对已经使用过的
Lemma \ref{chow-lemma-geometric-cap} 的一次应用。我们有
...
Algebra, Lemma \ref{algebra-lemma-grothendieck}，可在纤维上检验）。
...
\begin{proof}
这里 $c_{top}(\mathcal{E})$ 是 Remark \ref{chow-remark-top-chern-class}
...
\end{enumerate}
由 Lemma \ref{chow-lemma-top-chern-class}，得到
$$
...
\end{align*}
其中 $P_p$ 是 Example \ref{chow-example-power-sum} 中陈类的多项式。
上述表达式的次数 $p$ 分量是
$$
...
见 Remarks \ref{chow-remark-extend-to-finite-locally-free} 和
...
Lemma \ref{chow-lemma-vanish-above-dimension} 和分裂原理可证明
...
其中 $P_p$ 如 Example \ref{chow-example-power-sum}。
\end{lemma}

\begin{proof}
只须证明更精确的陈述。由 Section \ref{chow-section-splitting-principle}，
...
是 $\mathcal{E}$ 的陈根。因此，结论来自 Example
\ref{chow-example-power-sum} 中对 $P_p$ 的选取。
\end{proof}
...
$(x_{1, i} + x_{2, j})^p$ 的二项式公式以及 Example
\ref{chow-example-power-sum} 中多项式 $P_p$ 的形式。
\end{proof}
...
$ch(\mathcal{E}^\bullet)$ 的次数 $p$ 部分。最后，对 Example
\ref{chow-example-power-sum} 中的
...
Lemma \ref{chow-lemma-chern-character-additive} 代替
Lemma \ref{chow-lemma-additivity-chern-classes}。

\medskip\noindent
如 Remark \ref{chow-remark-extend-to-finite-locally-free}，为定义全陈类
...
$Y$ 整。由 Lemma \ref{chow-lemma-bivariant-zero}，只须证明
...
$\mathcal{O}_Y$-模的有界复形。接着，由 More on Flatness, Lemma
\ref{flat-lemma-blowup-complex-integral}，可设对所有
...
为这些复形的微分。由 More on Flatness, Remark
\ref{flat-remark-when-you-have-a-complex}，对所有 $i$，
$\Im(d_\mathcal{E}^i)$、$\Ker(d_\mathcal{E}^i)$、
$\Im(d_\mathcal{F}^i)$ 和 $\Ker(d_\mathcal{F}^i)$ 都是有限局部自由
$\mathcal{O}_X$-模。由可加性（Lemma \ref{chow-lemma-additivity-chern-classes}），
$$
c(\mathcal{E}^\bullet) = \prod\nolimits_i
...
见 Algebra, Lemma \ref{algebra-lemma-Schanuel}。）因此
$$
...
Lemma \ref{chow-lemma-pre-derived-chern-class}，双变类
...
$c_p(Lg^*E)$ 在 $T'$ 上的限制
（Remark \ref{chow-remark-restriction-bivariant}；严格来说，这里需要应用
Lemma \ref{chow-lemma-cap-cp-bivariant}）。

\medskip\noindent
取引理陈述中的 $f : Y \to X$ 和 $\mathcal{E}^\bullet$。
把 Lemma \ref{chow-lemma-envelope-bivariant} 应用于
$f : Y \to X$ 以及上一段构造的类
...
$Y \times_X Y' \to X$ 是包络（Lemmas
\ref{chow-lemma-base-change-envelope} 和
\ref{chow-lemma-composition-envelope}），由
Lemma \ref{chow-lemma-envelope-bivariant} 中的唯一性可知，
$c_p(E)$ 的这两个候选者相同。
\end{proof}
...
\item 若 $E$ 的陈类已定义，则应用
Lemma \ref{chow-lemma-defined-by-envelope} 定义
$$
c(E) \in \prod\nolimits_{p \geq 0} A^p(X),\quad
ch(E) \in
\prod\nolimits_{p \geq 0} A^p(X) \otimes \mathbf{Q},\quad
...
Lemma \ref{chow-lemma-chern-classes-defined}。对于定义中一般的
...
\begin{proof}
条件 (1) 正是 Definition \ref{chow-definition-defined-on-perfect} 的第 (1) 部分。
条件 (2) 蕴含 (1)。
...
$X'_i \to X_i$，使得 $X'_i$ 具有充足的可逆模族；见 More on Morphisms,
Proposition \ref{more-morphisms-proposition-envelope-with-resolution-property}。
...
由 Lemma \ref{chow-lemma-base-change-envelope}，基变换
$f : Y \to X$ 是包络。此外，拉回复形
...
$D(\mathcal{O}_Y)$ 中同构于有限局部自由
$\mathcal{O}_Y$-模组成的局部有界复形 $\mathcal{E}^\bullet$。
由 Lemma \ref{chow-lemma-base-change-envelope}，$f$ 的基变换
...
Lemma \ref{chow-lemma-bivariant-zero} 的判据。因此可以假设 $X$ 整；由
Lemma \ref{chow-lemma-chern-classes-computed}，甚至可以假设
...
$$
在 $\CH_*(X')$ 中成立。这立即由
Lemma \ref{chow-lemma-cap-commutative-chern} 以及
...
Lemma \ref{chow-lemma-additivity-chern-classes} 的乘法性推出。
对 $ch$ 或 $P_p$，使用 Lemmas
\ref{chow-lemma-chern-character-additive}。

\medskip\noindent
上一段已经说明，在条件 (1) 成立时引理成立。
由于 (2) 蕴含 (1)，第二种情形也已处理。现假设 (3)。
完全仿照 Lemma \ref{chow-lemma-chern-classes-defined} 的证明，
...
$(S, \delta), X, E$ 如 Definition
\ref{chow-definition-defined-on-perfect}，并且 $E$ 的陈类已定义。
假设要证明双变类 $c_p(E)$、$P_p(E)$ 与 $ch_p(E)$ 之间的某个关系。
为此，可以选择包络 $f : Y \to X$ 以及表示 $E$ 的、由有限局部自由
$\mathcal{O}_X$-模组成的局部有界复形 $\mathcal{E}^\bullet$。
由 Lemma \ref{chow-lemma-defined-by-envelope} 中的唯一性，
...
（Lemma \ref{chow-lemma-splitting-principle}），可以假设每个
...
$$
考察 Example \ref{chow-example-power-sum} 中多项式
...
\begin{proof}
第一种证明：仿照 Lemma \ref{chow-lemma-commutative-chern-perfect}
的证明，把问题约化到 $E$ 由常秩有限局部自由模组成的有界复形表示的情形，
再应用 Lemma \ref{chow-lemma-chern-classes-dual}。
第二种证明：使用 Remark \ref{chow-remark-splitting-principle-perfect}
...
Lemma \ref{chow-lemma-chern-classes-E-tensor-L}。读者可由这个特殊情形
通过形式计算推出本引理。另一种方法是使用
Remark \ref{chow-remark-splitting-principle-perfect} 的分裂原理。
...
由向量丛的相应结论作计算即可得到本结论；见 Lemmas
\ref{chow-lemma-chern-classes-tensor-product} 和
\ref{chow-lemma-chern-character-multiplicative}。
更好的证明也许是像 Remark \ref{chow-remark-splitting-principle-perfect}
...
然后使用 Remark \ref{chow-remark-splitting-principle-perfect}
...
\begin{proof}
下文将直接使用 Section \ref{chow-section-pre-derived} 的内容，不再另行说明。

\medskip\noindent
假设 $E_2|_{X_1 \cap X_2}$ 为零。考虑局部有限型概形态射
$X' \to X$，并记 $i'_j : X'_j \to X'$ 为 $i_j$ 的基变换。
由 Lemma \ref{chow-lemma-exact-sequence-closed-chow}，任意元素
...
Lemma \ref{chow-lemma-silly} 中 $A^p(X_2 \to X)$ 内的双变类
...
在 Lemma \ref{chow-lemma-silly} 中，设 $X' \to X$ 是局部有限型态射。
记 $X' = X'_1 \cup X'_2$ 以及
$E'_2 \in D(\mathcal{O}_{X'_2})$ 为拉回到 $X'$ 的对象。
则使用 $X' = X'_1 \cup X'_2$ 和 $E_2'$，由
Lemma \ref{chow-lemma-silly} 构造的 $A^p(X_2' \to X')$ 中的类
$P'_p(E_2')$，分别地\ $c'_p(E_2')$，是
$A^p(X_2 \to X)$ 中的类 $P'_p(E_2)$，分别地\ $c'_p(E_2)$，
的限制（Remark \ref{chow-remark-restriction-bivariant}）。
\end{lemma}

\begin{proof}
立即由 Lemma \ref{chow-lemma-silly} 对这些类的刻画得出。
\end{proof}

\begin{lemma}
\label{chow-lemma-silly-silly}
在 Lemma \ref{chow-lemma-silly} 中，设 $E_2$ 是某个完美对象
$E \in D(\mathcal{O}_X)$ 的限制，并且 $E|_{X_1}$ 为零，
分别地\ 同构于置于上同调次数 $0$ 的、秩 $< p$ 的有限局部自由
$\mathcal{O}_{X_1}$-模。若 $E$ 的陈类已定义，则
$i_{2, *} \circ P'_p(E_2) = P_p(E)$，
分别地\ $i_{2, *} \circ c'_p(E_2) = c_p(E)$
（其中 $\circ$ 如 Lemma \ref{chow-lemma-push-proper-bivariant}）。
\end{lemma}

\begin{proof}
先假设 $E|_{X_1}$ 为零。采用 Lemma \ref{chow-lemma-silly}
证明中的记号，此情形下本引理由下式推出：
\begin{align*}
P_p(E) \cap \alpha'
& =
...
在 Lemma \ref{chow-lemma-silly} 中，假设存在闭子概形
...
在 Lemma \ref{chow-lemma-silly} 中，设 $f : Y \to X$ 局部有限型，
...
最后应用 Lemma \ref{chow-lemma-commutative-chern-perfect}。
\end{proof}

\begin{lemma}
\label{chow-lemma-silly-compose}
在 Lemma \ref{chow-lemma-silly} 中，假设 $E_2|_{X_1 \cap X_2}$ 为零。则
\begin{align*}
P'_1(E_2) & = c'_1(E_2), \\
...
\end{align*}
等等，其中乘法如 Remark \ref{chow-remark-ring-loc-classes}。
\end{lemma}

\begin{proof}
零层的秩 $< 1$，所以类 $c'_p(E_2)$ 对所有 $p \geq 1$
都有定义，因而该陈述有意义。这些等式立即由
Lemma \ref{chow-lemma-silly} 所构造类的刻画，以及
Remark \ref{chow-remark-splitting-principle-perfect} 给出的
与 $E_2$ 的陈类取帽积的相应结论推出。
\end{proof}
...
\end{enumerate}
采用 Remark \ref{chow-remark-ring-loc-classes} 的记号，令
$$
c^{(p)}(E) = 1 + c_1(E) + \ldots + c_{p - 1}(E) +
c'_p(E|_{X_2}) + c'_{p + 1}(E|_{X_2}) + \ldots \in A^{(p)}(X_2 \to X)
$$
其中 $c'_p(E|_{X_2})$ 如 Lemma \ref{chow-lemma-silly}。
...
\begin{proof}
立即由 Lemma \ref{chow-lemma-silly} 对这些类的刻画以及
Lemma \ref{chow-lemma-additivity-on-perfect} 中的可加性得出。
\end{proof}
...
\end{enumerate}
对 $p \geq 0$，记由 Lemma \ref{chow-lemma-silly} 构造的类为
...
在 Lemma \ref{chow-lemma-silly} 中，假设 $E_2$ 具有常秩 $0$。
设 $\mathcal{L}$ 是可逆 $\mathcal{O}_X$-模。则
$$
...
所述等式立即由 Lemma \ref{chow-lemma-chern-class-perfect-tensor-invertible}
以及 Lemma \ref{chow-lemma-silly} 对 $c'_i$ 的刻画得出。
\end{proof}

\begin{lemma}
\label{chow-lemma-silly-tensor-product}
在 Situation \ref{chow-situation-setup} 中，设 $X$ 在 $S$ 上局部有限型。设
$$
...
\end{enumerate}
为 Lemma \ref{chow-lemma-silly} 所构造的类。则有
$$
...
所述等式由 Lemma \ref{chow-lemma-silly} 对这些类的刻画以及
Lemma \ref{chow-lemma-chern-classes-perfect-tensor-product} 的等式推出。
细节从略。
\end{proof}
...
Lemma \ref{chow-lemma-support-cap-effective-Cartier}，二者都表示类
...
$i_\infty^*$ 都是双变运算；见 Lemmas
\ref{chow-lemma-flat-pullback-bivariant} 和 \ref{chow-lemma-gysin-bivariant}。
...
在 Lemma \ref{chow-lemma-gysin-at-infty} 中，设 $X' \to X$ 是局部有限型态射。
记 $b' : W' \to \mathbf{P}^1_{X'}$ 和
$i'_\infty : W'_\infty \to W'$ 分别为 $b$ 与 $i_\infty$ 的基变换。
则使用 $b'$ 按 Lemma \ref{chow-lemma-gysin-at-infty} 构造的类
...
在 Lemma \ref{chow-lemma-gysin-at-infty} 中，设 $g : W' \to W$ 是真态射，
并且在 $\mathbf{A}^1_X$ 上是同构。设
$C' \in A^0(W'_\infty \to X)$ 和 $C \in A^0(W_\infty \to X)$
是 Lemma \ref{chow-lemma-gysin-at-infty} 所构造的类。则在
...
Lemma \ref{chow-lemma-closed-in-X-gysin}，
...
在 Lemma \ref{chow-lemma-gysin-at-infty} 中有
...
因为可在由 $i_0$ 拉回后检验这一点；见 Lemmas
\ref{chow-lemma-linebundle} 和 \ref{chow-lemma-linebundle-formulae}。
...
在 Lemma \ref{chow-lemma-gysin-at-infty} 中，设 $f : Y \to X$
...
Lemma \ref{chow-lemma-gysin-at-infty} 所构造的双变类。记
$A^p(E \to W_\infty)$ 中的 $P'_p(Q|_E)$，分别地\ $c'_p(Q|_E)$，
为 Lemma \ref{chow-lemma-silly} 所构造的双变类。这是有意义的，
...
$$
这是双变类；见 Lemma \ref{chow-lemma-push-proper-bivariant}。
...
c_p(Q|_{X \times \{0\}})
\end{align*}
第二个等式由 Lemma \ref{chow-lemma-silly-silly} 得出。
第三个等式使用了 $c_p(Q)$ 是双变类。
第四个等式由 Lemma \ref{chow-lemma-gysin-at-infty} 得出。
...
$$
因此本引理由 $C$ 的相应基变换性质
（Lemma \ref{chow-lemma-base-change-gysin-at-infty}），以及
Lemma \ref{chow-lemma-silly} 所构造的类也具有同样基变换性质这一事实推出
（省略一个小细节）。
\end{proof}

\begin{lemma}
\label{chow-lemma-localized-chern-pre-independent}
Lemma \ref{chow-lemma-localized-chern-pre} 中的双变类
...
\begin{proof}
关于 $T$ 的独立性立即由 Lemma \ref{chow-lemma-silly-independent} 得出。
...
在 Lemma \ref{chow-lemma-localized-chern-pre} 中，假设 $Q|_T$
同构于秩 $< p$ 的有限局部自由 $\mathcal{O}_T$-模。
记 $C \in A^0(W_\infty \to X)$ 为
Lemma \ref{chow-lemma-gysin-at-infty} 的类。则
$$
...
\begin{proof}
第一个等式成立，因为由 Lemma \ref{chow-lemma-localized-chern-pre}，
$c_p(Q|_{X \times \{0\}}) =
(Z \to X)_* \circ c'_p(Q)$。第二个等式可逐个循环类证明
（见 Lemma \ref{chow-lemma-bivariant-zero}）。由于引理中的双变类构造
...
Lemma \ref{chow-lemma-silly} 所构造的类。第二个等式只是说
...
第四个等式是 Lemma \ref{chow-lemma-silly-silly}。
第五个等式使用了 $c_p(Q)$ 是双变类，因而与 $i_\infty^*$ 交换。
第六个等式是 Lemma \ref{chow-lemma-homomorphism-pre}。
...
在 Lemma \ref{chow-lemma-localized-chern-pre} 中，设 $Y \to X$
...
其中 $C$ 如 Lemma \ref{chow-lemma-gysin-at-infty}，而
$P'_p(Q|_E)$，分别地\ $c'_p(Q|_E)$，如 Lemma \ref{chow-lemma-silly}。
由 Lemma \ref{chow-lemma-gysin-at-infty-commutes}，$C$ 与 $c$ 交换；
由 Lemma \ref{chow-lemma-silly-commutes}，$P'_p(Q|_E)$，
...
在 Lemma \ref{chow-lemma-localized-chern-pre} 中，假设 $Q|_T$ 为零。
在 $A^*(Z \to X)$ 中有
\begin{align*}
P'_1(Q) & = c'_1(Q), \\
...
都有定义，因而该陈述有意义。在
Lemma \ref{chow-lemma-localized-chern-pre} 的证明中，我们用
Lemma \ref{chow-lemma-gysin-at-infty} 的双变类
$C \in A^0(W_\infty \to X)$，以及 Lemma \ref{chow-lemma-silly}
给出的双变类 $P'_p(Q|_E)$ 和 $c'_p(Q|_E)$，构造了类
$P'_p(Q)$ 与 $c'_p(Q)$；这里 $Q|_E$ 是 $Q$ 在
$W_\infty$ 中 $Z$ 的逆像 $E$ 上的限制。由
Lemma \ref{chow-lemma-silly-compose}，$P'_p(Q|_E)$ 与
...
c'_p(Q) = (E \to Z)_* \circ c'_p(Q|_E) \circ C
$$
为完成证明，只须说明 Remark \ref{chow-remark-ring-loc-classes}
...
$c'_i(Q)$ 如 Lemma \ref{chow-lemma-localized-chern-pre}。
...
见 Lemma \ref{chow-lemma-localized-chern-pre}。因此次数
$< p + p'$ 的等式由 Lemma \ref{chow-lemma-additivity-on-perfect}
的可加性推出。

\medskip\noindent
取 $n \geq p + p'$。仿照 Lemma \ref{chow-lemma-localized-chern-pre}
的证明，记 $E \subset W_\infty$ 为 $Z$ 的逆像。注意，由
Lemma \ref{chow-lemma-silly-sum-c}，在
...
$$
由 Lemma \ref{chow-lemma-silly}，左边等于
$$
...
$$
由 Lemma \ref{chow-lemma-homomorphism}，这立即等于上述左边。
...
Lemma \ref{chow-lemma-homomorphism} 的一部分。
...
它们同与陈类取帽积交换
（Lemma \ref{chow-lemma-cap-commutative-chern}）。因此只须证明
$$
...
此时由 Lemma \ref{chow-lemma-localized-chern-pre} 构造的类
$P'_p(Q), P'_p(Q'), P'_p(Q \oplus Q') \in A^p(Z \to X)$
满足 $P'_p(Q \oplus Q') = P'_p(Q) + P'_p(Q')$。
\end{lemma}

\begin{proof}
立即由这些类的构造以及 Lemma \ref{chow-lemma-silly-sum-P} 得出。
\end{proof}
...
Macpherson 图构造；它在 More on Flatness, Section
\ref{flat-section-blowup-complexes-III} 中完成。
(2) 给定 $W$ 与 $Q$ 后实际类的构造在 Section
\ref{chow-section-preparation-localized-chern-II} 中完成，并依赖于 Sections
\ref{chow-section-gysin-at-infty} 和
...
相关讨论见 Remark \ref{chow-remark-loc-chern}。}成立：
(a) $X$ 拟紧；
(b) $X$ 的不可约分支拟紧；
...
在 Situation \ref{chow-situation-loc-chern} 中，存在典范双变类
$$
...
Q
$$
分别为 More on Flatness, Section
\ref{flat-section-blowup-complexes-III} 和 Lemma
\ref{flat-lemma-graph-construction} 中构造的爆破、
$D(\mathcal{O}_W)$ 中的完美对象 $Q$ 以及闭浸入。
令 $T' \subset T$ 为开闭子概形，使得 $Q|_{T'}$ 为零，
分别地\ 同构于置于上同调次数 $0$ 的、秩 $< p$ 的有限局部自由
$\mathcal{O}_{T'}$-模。由 Situation \ref{chow-situation-loc-chern}
的条件 (2)，态射
$$
...
由 Lemma \ref{chow-lemma-localized-chern-pre} 构造的双变类给出
$$
...
\medskip\noindent
本段证明 $Q$ 的陈类已定义
（Definition \ref{chow-definition-defined-on-perfect}）；建议读者跳过。
若 Situation \ref{chow-situation-loc-chern} 的假设 (3)(a) 或 (3)(b) 成立，
即 $X$ 具有拟紧不可约分支，则 $W$ 也如此（因为
$W \to X$ 是真态射）。所以由
Lemma \ref{chow-lemma-chern-classes-defined}，$D(\mathcal{O}_W)$
中任意完美对象的陈类均已定义。若 (3)(c) 成立，即 $E$
可由有限局部自由模组成的局部有界复形表示，则由 More on Flatness,
Lemma \ref{flat-lemma-graph-construction} 的第 (5) 部分，
...
出发，按 More on Flatness, Section
\ref{flat-section-blowup-complexes-III} 构造的态射与完美对象。
由上述讨论，$Q'$ 的陈类已定义。由于 $b$ 和 $b'$ 是应用
More on Flatness, Lemma
\ref{flat-lemma-complex-and-divisor-blowup-pre} 构造的，
由 More on Flatness, Lemma
\ref{flat-lemma-complex-and-divisor-blowup-base-change}，
存在态射 $W \to W'$，使得 $Q = L(W \to W')^*Q'$。
于是由 Lemma \ref{chow-lemma-chern-classes-defined}，
$Q$ 的陈类已定义。
\end{proof}
...
$i : Z \to X$ 如 Situation \ref{chow-situation-loc-chern}。
\begin{enumerate}
\item 若限制 $E|_{X \setminus Z}$ 为零，则对所有 $p \geq 0$，
由 Lemma \ref{chow-lemma-independent-loc-chern} 的构造定义
$$
P_p(Z \to X, E) \in A^p(Z \to X)
$$
并用公式
...
Lemma \ref{chow-lemma-independent-loc-chern} 的构造定义
...
在 Situation \ref{chow-situation-loc-chern} 中，设
$f : X' \to X$ 是局部有限型概形态射。记
$E' = f^*E$ 且 $Z' = f^{-1}(Z)$。则 Definition
\ref{chow-definition-localized-chern} 中的双变类
$$
P_p(Z' \to X', E') \in A^p(Z' \to X'),
\quad\text{分别}\quad
c_p(Z' \to X', E') \in A^p(Z' \to X')
$$
即使用 $X', Z', E'$ 按 Lemma \ref{chow-lemma-independent-loc-chern}
...
$b' : W' \to \mathbf{P}^1_{X'}$ 是在 More on Flatness, Lemma
\ref{flat-lemma-complex-and-divisor-blowup-pre} 中分别从三元组
...
由 More on Flatness, Lemma
\ref{flat-lemma-complex-and-divisor-blowup-base-change}，得到交换图
$$
\xymatrix{
...
在 Situation \ref{chow-situation-loc-chern} 中，更自然的做法是把假设 (3)
替换为假设“$E$ 的陈类已定义”。事实上，把 Lemmas
\ref{chow-lemma-independent-loc-chern} 和
\ref{chow-lemma-base-change-loc-chern} 与
Lemma \ref{chow-lemma-envelope-bivariant} 结合起来，很容易把定义推广到
这个略微更一般的情形。若以后需要，我们将在此完成该推广。
\end{remark}

\begin{lemma}
\label{chow-lemma-loc-chern-after-pushforward}
在 Situation \ref{chow-situation-loc-chern} 中，对任意
...
由 Lemma \ref{chow-lemma-base-change-loc-chern}，
...
在 Situation \ref{chow-situation-loc-chern} 中，若
...
Lemma \ref{chow-lemma-base-change-loc-chern} 看出该限制为零，
从而计算 $c_p(Z \to X, E) \cap \alpha$。
\end{proof}

\begin{lemma}
\label{chow-lemma-loc-chern-shrink-Z}
在 Situation \ref{chow-situation-loc-chern} 中，假设
...
\begin{proof}
Lemma \ref{chow-lemma-independent-loc-chern} 中
$P_p(Z' \to X, E)$，分别地\ $c_p(Z' \to X, E)$ 的构造，
使用完全相同的态射 $b : W \to \mathbf{P}^1_X$ 与
$D(\mathcal{O}_W)$ 中的完美对象 $Q$。于是应用
Lemma \ref{chow-lemma-silly-shrink} 即得结论。省略一些细节。
\end{proof}
...
分别地\ 同构于置于上同调次数 $0$ 的、秩 $< p$ 的有限局部自由
$\mathcal{O}_{X_1}$-模。则 Lemma \ref{chow-lemma-silly} 的类
$P'_p(E_2)$，分别地\ $c'_p(E_2)$，与 Definition
\ref{chow-definition-localized-chern} 的
$P_p(X_2 \to X, E)$，分别地\ $c_p(X_2 \to X, E)$ 相同，
只要 $E$ 满足 Situation \ref{chow-situation-loc-chern} 的假设 (3)。
\end{lemma}

\begin{proof}
关于 $E$ 的假设蕴含存在包含 $X_1$ 的开集 $U \subset X$，
使得 $E|_U$ 为零，分别地\ 同构于秩 $< p$ 的有限局部自由
$\mathcal{O}_U$-模。见 More on Algebra, Lemma
\ref{more-algebra-lemma-lift-perfect-from-residue-field}。
令 $Z \subset X$ 为 $U$ 在 $X$ 中的补集，并赋予诱导的既约闭子概形结构。
则由 Lemma \ref{chow-lemma-loc-chern-shrink-Z}，
...
满足 Lemma \ref{chow-lemma-silly} 给出的对 $P'_p(E_2)$，分别地\ 
$c'_p(E_2)$ 的刻画。事实上，由刚刚证明的关系
$P_p(X_2 \to X, E) = (Z \to X_2)_* \circ P_p(Z \to X, E)$，
分别地\ $c_p(X_2 \to X, E) = (Z \to X_2)_* \circ c_p(Z \to X, E)$，
以及 $X_1 \cap Z = \emptyset$，由
Lemma \ref{chow-lemma-loc-chern-disjoint}，复合
$P_p(X_2 \to X, E) \circ i_{1, *}$，分别地\ 
$c_p(X_2 \to X, E) \circ i_{1, *}$ 为零。另一方面，由
Lemma \ref{chow-lemma-loc-chern-after-pushforward}，
...
下一个引理是我们在 Lemmas \ref{chow-lemma-localized-chern-pre-compose}
...
满足 Lemma \ref{chow-lemma-localized-chern-pre} 的假设；
\item $Q_i|_{W \setminus W_i}$ 为零，分别地\ 同构于置于上同调次数
$0$ 的、秩 $< p_i$ 的有限局部自由模；
\item $W$ 上的 $Q_i$ 满足 Situation \ref{chow-situation-loc-chern}
的假设 (3)。
\end{enumerate}
...
\circ C
$$
由 Lemma \ref{chow-lemma-base-change-localized-chern-pre}，
...
Z_{n - 1} \cap \ldots \cap Z_1)$ 为 Lemma \ref{chow-lemma-gysin-at-infty}
的类。于是 $c'_{p_n}(Q_n)$ 在
...
C_{n - 1}
\end{align*}
其中等式由 Lemma \ref{chow-lemma-base-change-silly} 得出
（右边的限制记号从略）。因此上式化为
\begin{align*}
(W_{n, \infty} \cap \ldots \cap W_{1, \infty} \to
Z_n \cap \ldots \cap Z_1)_* \circ
...
\circ C
\end{align*}
由 Lemma \ref{chow-lemma-homomorphism-pre}，复合
...
Lemma \ref{chow-lemma-loc-chern-agree} 以及引理陈述中关于 $Q_i$
的假设 (2)、(3)，可以写成
$$
...
Situation \ref{chow-situation-loc-chern} 的假设 (3)。
\end{enumerate}
则 Lemma \ref{chow-lemma-localized-chern-pre} 所构造的
$A^p(Z \to X)$ 中的类 $P'_p(Q)$，分别地\ $c'_p(Q)$，
等于 Definition \ref{chow-definition-localized-chern} 中的
...
依照 Lemma \ref{chow-lemma-gysin-at-infty} 中 $C$ 的构造，
选择 $\beta \in \CH_{k + 1}(W)$，使其在
...
+ 3c_3(Z \to X, E),
\end{align*}
依此类推，其中乘积取自 Remark \ref{chow-remark-ring-loc-classes}
中的代数 $A^{(1)}(Z \to X)$。
\end{lemma}
...
结论本身立即由更一般的 Lemma
\ref{chow-lemma-localized-chern-pre-compose} 得出，因为局部化陈类
是使用 Lemma \ref{chow-lemma-localized-chern-pre} 的步骤、在
Section \ref{chow-section-localized-chern} 中定义的。
\end{proof}
...
\begin{proof}
这由 Lemma \ref{chow-lemma-homomorphism-commute} 得出。
更确切地，令
$$
b : W \to \mathbf{P}^1_X
\quad\text{且}\quad
...
$$
如 Lemma \ref{chow-lemma-independent-loc-chern} 的证明中所取。
按定义，作为双变运算，
$c_p(Z \to X, E) = c'_p(Q)$，其中右边是在
Lemma \ref{chow-lemma-localized-chern-pre} 中使用
$W, b, Q, T'$ 构造的双变类。由
Lemma \ref{chow-lemma-homomorphism-commute}，在
$A^*(Y \times_X Z \to X)$ 中有
...
在 Situation \ref{chow-situation-loc-chern} 中，方便起见定义
$$
c^{(p)}(Z \to X, E) = 1 + c_1(E) + \ldots + c_{p - 1}(E) +
c_p(Z \to X, E) + c_{p + 1}(Z \to X, E) + \ldots
$$
将其视为 Remark \ref{chow-remark-ring-loc-classes} 中所考虑的代数
$A^{(p)}(Z \to X)$ 的元素。
\end{remark}
...
$\mathcal{O}_Y$-模组成的局部有界复形之间的映射表示。
\end{enumerate}
采用 Remark \ref{chow-remark-loc-chern-classes} 中的记号，在
$A^{(p_1 + p_3)}(Z \to X)$ 中有
$$
...
由 Lemma \ref{chow-lemma-envelope-bivariant} 中的唯一性，只须证明
$A^*(Y)$ 和 $A^*(Z \to Y)$ 中对应的关系。又因所涉及各类的构造
与基变换相容（Lemma \ref{chow-lemma-base-change-loc-chern}），
可将 $X$ 替换为 $Y$，并将可区别三角替换为其拉回。

\medskip\noindent
在 Lemma \ref{chow-lemma-additivity-on-perfect} 的证明中已经看到，
...
由局部化陈类与基变换的相容性
（Lemma \ref{chow-lemma-base-change-loc-chern}），得到
$$
...
$i_0^* = i_1^*$（更确切地，这由
Lemma \ref{chow-lemma-linebundle-formulae} 中的公式得出），所以
$$
...
如 Situation \ref{chow-situation-loc-chern}。对 $i = 1, 3$，令
$$
...
$$
其中右边是在 Lemma \ref{chow-lemma-localized-chern-pre} 中使用
...
由 Lemma \ref{chow-lemma-localized-chern-pre-independent}，可以使用
$W$、$b$、$g_i^*\mathcal{Q}_i$ 与
$T'$ 构造 $i = 1, 3$ 时的 $c_p(Z \to X, E_i)$。
此外，由 Lemma \ref{chow-lemma-independent-loc-chern-bQ}
给出的更强独立性，可以使用
$W$、$b$、$g_1^*Q_1 \oplus g_3^*Q_3$ 与 $T'$
计算各类 $c_p(Z \to X, E_2)$。于是所需等式由
Lemma \ref{chow-lemma-localized-chern-pre-sum-c} 得出。
\end{proof}
...
\begin{proof}
证明与 Lemma \ref{chow-lemma-additivity-loc-chern-c} 的证明完全相同，
只是最后使用 Lemma \ref{chow-lemma-localized-chern-pre-sum-P}。
当 $p > 0$ 时，可令 $p_1 = p_3 = 1$，由
Lemma \ref{chow-lemma-additivity-loc-chern-c} 以及
Lemma \ref{chow-lemma-loc-chern-character} 中
...
满足 Situation \ref{chow-situation-loc-chern} 的假设 (3)。记
...
或更一般地如 Lemma \ref{chow-lemma-independent-loc-chern-bQ} 所取。
选取交换图
$$
\xymatrix{
...
由 Lemma \ref{chow-lemma-independent-loc-chern-bQ}，可以使用
...
\end{enumerate}
\end{enumerate}
由 Lemma \ref{chow-lemma-independent-loc-chern-bQ}，对 $i = 1, 2$ 有
$$
...
$$
由 Lemma \ref{chow-lemma-silly-tensor-product}，各类
$c'_p(Q|_{E_1 \cap E_2})$ 与 $P'_p(Q|_{E_1 \cap E_2})$
可用各类 $c'_p(Q_i|_{E_i})$ 与 $P'_p(Q_i|_{E_i})$
按正确方式展开。最后，Lemma \ref{chow-lemma-homomorphism-final}
...
$$
其中水平箭头均为闭浸入（Divisors, Lemma
\ref{divisors-lemma-strict-transform}）。记 $E \subset W$
...
Divisors, Definition \ref{divisors-definition-conormal-sheaf}；
...
的严格变换同构地映到 $\mathbf{P}^1_Z$；见 Divisors, Lemmas
\ref{divisors-lemma-strict-transform} 和
\ref{divisors-lemma-blow-up-effective-Cartier-divisor}。
这给出 (8) 中提到的态射 $\mathbf{P}^1_Z \to W$。
由于 $b$ 分离，该态射是闭浸入；见 Schemes, Lemma
...
\medskip\noindent
由于 $\text{Proj}$ 的形成与基变换交换（Constructions, Lemma
\ref{constructions-lemma-base-change-map-proj}），可见
$$
...
确实如上所示。这证明了 (5)，因为由 Divisors, Lemma
\ref{divisors-lemma-affine-conormal-sheaf}，
...
Divisors, Lemma \ref{divisors-lemma-blowing-up-affine} 和
...
\noindent
设 $(S, \delta)$ 如 Situation \ref{chow-situation-setup}，且
...
\begin{proof}
这由 Morphisms, Lemma
\ref{morphisms-lemma-conormal-functorial-flat} 所证明的映射
...
b : W \longrightarrow \mathbf{P}^1_X
$$
为 Section \ref{chow-section-blowup-Z-first} 中构造的沿
$\infty(Z)$ 的爆破。由 Lemma \ref{chow-lemma-gysin-at-infty}，
存在典范双变类
$$
C \in A^0(W_\infty \to X)
$$
...
$i : C_ZX \to N$。由 Lemma \ref{chow-lemma-vectorbundle}，
...
$A^*(Z \to X)^\wedge$ 见 Remark
\ref{chow-remark-completion-bivariant} 的讨论。若 $X$ 拟紧，则
$A^*(Z \to X)^\wedge = A^*(Z \to X)$。}
$$
c(Z \to X, \mathcal{N}) \in A^*(Z \to X)^\wedge
$$
此外，该构造与 Lemma
\ref{chow-lemma-pullback-virtual-normal-sheaf} 中的基变换相容。
...
（见 Lemmas \ref{chow-lemma-flat-pullback-bivariant} 和
\ref{chow-lemma-push-proper-bivariant}），可以用等式
$$
i_* \circ j^* \circ C  = p^* \circ c(Z \to X, \mathcal{N})
$$
（作适当解释）将 $c(Z \to X, \mathcal{N})$ 定义为双变类。
这是可行的，因为由 Lemma \ref{chow-lemma-vectorbundle}，
$p^*$ 在 Chow 群上总是双射。

\medskip\noindent
设 $X' \to X$、$Z' \to X'$ 与 $\mathcal{N}'$ 如
Lemma \ref{chow-lemma-pullback-virtual-normal-sheaf}。记
$c = c(Z \to X, \mathcal{N})$ 且
$c' = c(Z' \to X', \mathcal{N}')$。引理的第二个陈述是说，
$c'$ 是 Remark \ref{chow-remark-restriction-bivariant} 意义下
...
$$
为得到 $c \cap \alpha'$，使用在 Lemma
\ref{chow-lemma-gysin-at-infty} 中由态射
...
的推前等于 $C \cap \alpha'$。因此，由 Lemma
\ref{chow-lemma-flat-pullback-proper-pushforward}，二者到开集
$$
...
C_ZX \to N' \to N
$$
所需双变类关系立即由 Lemma \ref{chow-lemma-easy-virtual-class} 得出。
\end{proof}

\begin{lemma}
\label{chow-lemma-gysin-excess}
设 $(S, \delta)$ 如 Situation \ref{chow-situation-setup}。考虑
$S$ 上局部有限型概形的笛卡儿图
$$
\xymatrix{
Z' \ar[r] \ar[d]_g & X' \ar[d]^f \\
...
\begin{proof}
由 Lemma \ref{chow-lemma-construction-gysin}，
$res(c(Z \to X, \mathcal{N})) = c(Z' \to X', g^*\mathcal{N})$；
于是等式由 Lemma \ref{chow-lemma-gysin-decompose} 得出。
\end{proof}
...
假设 $Z \times_X Y \to Y$ 是正则闭浸入；见 Divisors, Section
\ref{divisors-section-regular-immersions}。此时余法层
...
在 Remark \ref{chow-remark-top-chern-class} 中定义。
由 Lemma \ref{chow-lemma-construction-gysin}，可将 $X$ 替换为 $Y$。
因此可假设 $Z \to X$ 是余维 $r$ 的正则闭浸入，
$\dim_\delta(X) = n$，并须在 $\CH_*(Z)$ 中证明
$c(Z \to X, \mathcal{N}) \cap [X]_n =
c_{top}(\mathcal{E}) \cap [Z]_{n - r}$。
由 Lemma \ref{chow-lemma-gysin-decompose}，甚至可假设
...
Lemma \ref{chow-lemma-gysin-at-infty} 中的类。注意
...
Lemma \ref{chow-lemma-easy-gysin}。该类在开集
$C_ZX \subset W_\infty$ 上的限制当然正是 $[C_ZX]_n$。
因为 $Z \subset X$ 是正则嵌入，有
$$
\mathcal{C}_{Z/X, *} = \text{Sym}(\mathcal{C}_{Z/X})
$$
作为分次 $\mathcal{O}_Z$-代数；见 Divisors, Lemma
\ref{divisors-lemma-quasi-regular-immersion}。因此
...
这由 Lemma \ref{chow-lemma-gysin-fundamental} 得出，因为超额法层在
$V$ 上为 $0$。
\end{proof}
...
（见 Lemma \ref{chow-lemma-gysin-bivariant}），则它与双变类
...
\begin{proof}
使用 Lemma \ref{chow-lemma-bivariant-zero} 的判据。
因此可假设 $X$ 是整概形，并须证明
$i^*[X]$ 等于 $c \cap [X]$。令
...
Lemma \ref{chow-lemma-gysin-fundamental} 和 Definition
\ref{chow-definition-gysin-homomorphism}。

\medskip\noindent
若 $D \not = X$，则 $D \to X$ 是有效 Cartier 除子，
特别地，是余维 $1$ 的正则闭浸入。再次由
Lemma \ref{chow-lemma-gysin-fundamental} 得到
...
交换（Remark \ref{chow-remark-bivariant-commute}）。
\end{lemma}

\begin{proof}
可用 Lemma \ref{chow-lemma-bivariant-zero} 检验此事。
因此可假设 $X$ 是整概形，并须在 $\CH_*(Z \times_X Y)$ 中证明
$c \cap c(Z \to X, \mathcal{N}) \cap [X] =
c(Z \to X, \mathcal{N}) \cap c \cap [X]$。

\medskip\noindent
若 $Z = X$，则由 Lemma \ref{chow-lemma-gysin-fundamental}，
$c(Z \to X, \mathcal{N}) = c_{top}(\mathcal{N})$；
它与双变类 $c$ 交换，见 Lemma
\ref{chow-lemma-cap-commutative-chern}。

\medskip\noindent
假设 $Z$ 不等于 $X$。由 Lemma \ref{chow-lemma-bivariant-zero}，
...
Gysin 同态（Lemma \ref{chow-lemma-gysin-bivariant}），而
$\mathcal{E}$ 是 $\mathcal{N}^\vee \to \mathcal{C}_{Z/X}$
之核的对偶；见 Lemmas \ref{chow-lemma-gysin-decompose} 和
\ref{chow-lemma-gysin-agrees}。陈类位于双变环的中心
（以 Lemma \ref{chow-lemma-cap-commutative-chern} 所表述的强意义），
...
$C_YX$ 是 $Y$ 上的线丛，得到双变类
$o^* \in A^1(Y \to C_YX)$；见 Lemma
\ref{chow-lemma-gysin-bivariant}。
...
\begin{proof}
记 $W \to \mathbf{P}^1_X$ 为 Section
\ref{chow-section-blowup-Z-first} 中沿 $\infty(Z)$ 的爆破。
...
（Lemma \ref{chow-lemma-gysin-commutes-gysin}），并且
...
$$
其中下方序列来自 More on Morphisms, Lemma
\ref{more-morphisms-lemma-transitivity-conormal}，而上方序列是短正合列。
则在 $A^*(Z \to X)^\wedge$ 中有
$$
c(Z \to X, \mathcal{N}) =
...
故沿任意基变换仍然正合）。因此可用
Lemma \ref{chow-lemma-bivariant-zero} 检验双变类的等式。
于是可假设 $X$ 是整概形，并须证明
...
c(Z \to X, \mathcal{N}) \cap [X] 
\end{align*}
第一个等式由 Lemma \ref{chow-lemma-gysin-decompose} 得出。
第二个等式是因为陈类与双变类交换
（Lemma \ref{chow-lemma-cap-commutative-chern}）。
第三个等式由 Lemma \ref{chow-lemma-gysin-decompose} 得出。

\medskip\noindent
假设 $Y \not = X$。由 Lemma \ref{chow-lemma-bivariant-zero}，
甚至只须在对 $X$ 沿某个非零理想爆破后证明结论。
沿 $Y$ 的理想层与 $Z$ 的理想层之积爆破 $X$。
这化归为 $Y$ 和 $Z$ 均为 $X$ 上有效 Cartier 除子的情形；见
Divisors, Lemmas
\ref{divisors-lemma-blowing-up-gives-effective-Cartier-divisor} 和
...
$\mathcal{N}_1$。由 Lemma \ref{chow-lemma-gysin-decompose}，
$$
...
$$
由于丛的陈类与双变类交换
（Lemma \ref{chow-lemma-cap-commutative-chern}），只须在
$A^*(Z \to X)$ 中证明
$$
c(Z \to X, \mathcal{N}_1) =
...
是笛卡儿图，其中 $o$ 是 $Y$ 上线丛
$C_Y X$ 的零截面。由 Lemma
\ref{chow-lemma-relation-normal-cones}，有
...
\ref{morphisms-definition-conormal-sheaf}）；
...
Section \ref{chow-section-splitting-principle}。注意
$$
c(\wedge^n \mathcal{E}) =
...
Lemma \ref{chow-lemma-compute-koszul} 得出。该引理还给出
$$
c_r(i_*\mathcal{O}_X) = - (r - 1)!
c_r(\pi^*\mathcal{C} \otimes \mathcal{O}_E(-1))
$$
另一方面，由 Lemma \ref{chow-lemma-chern-classes-dual}，
$$
c_r(\pi^*\mathcal{C} \otimes \mathcal{O}_E(-1)) =
...
（Lemma \ref{chow-lemma-vectorbundle}）。因此由
Lemma \ref{chow-lemma-restrict-to-open}，可写
...
Lemma \ref{chow-lemma-top-chern-class}，在情形 (a) 有
...
A^r(Z \to X)
$$
其中 $c_t(Z \to X, i_*\mathcal{O}_Z)$ 是 Definition
\ref{chow-definition-localized-chern} 中的局部化陈类。
\end{lemma}
...
$f_1, \ldots, f_r \in A$ 是正则序列。见 Divisors, Definition
\ref{divisors-definition-regular-immersion} 和 Lemma
\ref{divisors-lemma-Noetherian-scheme-regular-ideal}。
于是由例如 More on Algebra, Lemma
...
Situation \ref{chow-situation-loc-chern} 和 Definition
\ref{chow-definition-localized-chern}。综上，该陈述是有意义的。

\medskip\noindent
记 $b : W \to \mathbf{P}^1_X$ 为 Section
\ref{chow-section-blowup-Z-first} 中沿 $\infty(Z)$ 的爆破。
...
满足 Lemma \ref{chow-lemma-independent-loc-chern-bQ} 的假设。

\medskip\noindent
假设该断言成立。Lemma
\ref{chow-lemma-independent-loc-chern-bQ} 给出：对所有
$p \geq 1$，
$$
...
因此，将 Lemma \ref{chow-lemma-compute-section} 应用于 $E \to Z$，
...
Lemma \ref{chow-lemma-gysin-at-infty} 中的类。）
因此，引理陈述中的显示等式由
Lemma \ref{chow-lemma-compute-section} 中的相应等式得出。

\medskip\noindent
证明该断言。设 $A$ 和 $f_1, \ldots, f_r$ 如上。
考虑 Section \ref{chow-section-blowup-Z-first} 中的仿射开集
$\Spec(A[s]) \subset \mathbf{P}^1_X$。回忆在该开集上，
$s = 0$ 定义 $(\mathbf{P}^1_X)_\infty$。因此在
$\Spec(A[s])$ 上，我们沿正则序列
$s, f_1, \ldots, f_r$ 生成的理想爆破。由 More on Algebra, Lemma
\ref{more-algebra-lemma-blowup-regular-sequence}，这 $r + 1$
...
（以及 Lemma \ref{chow-lemma-localized-chern-pre}）中均立即得到验证。
\end{proof}

\begin{lemma}
\label{chow-lemma-actual-computation}
在 Lemma \ref{chow-lemma-agreement-with-loc-chern} 的情形中，设
$\dim_\delta(X) = n$。则
\begin{enumerate}
...
\begin{proof}
第 (1)、(2) 项立即由 Lemma
\ref{chow-lemma-agreement-with-loc-chern} 与 Lemma
\ref{chow-lemma-gysin-fundamental} 合并得出。
然后利用 Lemma \ref{chow-lemma-loc-chern-character} 中
...
\ref{perfect-section-K-groups} 中，已经构造了与该范畴对应的第零
...
上定义 Adams 算子 $\psi^2$（Derived Categories of Schemes,
Definition \ref{perfect-definition-K-group}）。
...
Lemma \ref{chow-lemma-second-adams-operator} 的证明说明它良定义。
...
由 Lemma \ref{chow-lemma-additivity-chern-classes}，该映射良定义。
\end{remark}
...
$$
方法是将左边的生成元 $[\mathcal{E}]$ 映到
$ch(\mathcal{E})$，再作加法延拓。由 Lemma
\ref{chow-lemma-chern-character-additive}，该映射良定义；由
Lemma \ref{chow-lemma-chern-character-multiplicative}，它是环同态。
\end{remark}

\begin{lemma}
\label{chow-lemma-adams-and-chern}
设 $(S, \delta)$ 如 Situation \ref{chow-situation-setup}，且
$X$ 在 $S$ 上局部有限型。设 $\psi^2$ 如 Lemma
\ref{chow-lemma-second-adams-operator}，而 $c$ 与 $ch$ 如 Remarks
\ref{chow-remark-chern-classes-K} 和 \ref{chow-remark-chern-character-K}。
...
$\mathcal{E}$ 的秩恒为 $r$ 的情形；见 Remark
\ref{chow-remark-extend-to-finite-locally-free}。
此时可选取投影光滑态射 $p : P \to X$，使得限制
$A^*(X) \to A^*(P)$ 是单射，并且 $p^*\mathcal{E}$
有有限滤过，其各分次片是可逆 $\mathcal{O}_P$-模
$\mathcal{L}_j$；见 Lemma \ref{chow-lemma-splitting-principle}。于是
...
$$
（等号由 Lemma \ref{chow-lemma-K-coherent-supported-on-closed} 得出）。
...
设 $X$、$Z$、$D_{Z, perf}(\mathcal{O}_X)$ 如 Remark
\ref{chow-remark-perf-Z-cohomology-K}，并假设 $X$ 正则。
则存在典范映射
$$
...
由 Derived Categories of Schemes, Lemma
\ref{perfect-lemma-perfect-on-regular}，$\mathcal{F}[0]$
...
见 Derived Categories, Section
\ref{derived-section-canonical-delta-functor}。这表明，将
...
Remarks \ref{chow-remark-perf-Z-cohomology-K} 和
...
回忆由 Derived Categories of Schemes, Lemma
...
$D_{Z, perf}(\mathcal{O}_X)$ 如 Remark
\ref{chow-remark-perf-Z-cohomology-K}。若 $X$ 拟紧
...
$$
再作乘法延拓（右边的乘积如 Remark
\ref{chow-remark-ring-loc-classes}）。$X$ 的拟紧条件保证
局部化陈类已有定义（Situation \ref{chow-situation-loc-chern} 和
Definition \ref{chow-definition-localized-chern}），并且这些局部化陈类
...
设 $X$ 在 $S$ 上局部有限型，$Z \subset X$ 是闭子概形，并设
$D_{Z, perf}(\mathcal{O}_X)$ 如 Remark
\ref{chow-remark-perf-Z-cohomology-K}。若 $X$ 的不可约分支拟紧，
则局部化陈特征定义典范的加法且乘法映射
$$
...
（Situation \ref{chow-situation-loc-chern} 和 Definition
\ref{chow-definition-localized-chern}），并且这些局部化陈特征
...
\ref{perfect-remark-perf-Z}），所以由 Lemma
\ref{chow-lemma-loc-chern-tensor-product}，
...
Remarks \ref{chow-remark-localized-chern-classes-K} 和
...
$$
见 Derived Categories of Schemes, Definition
\ref{perfect-definition-K-group}。因此，作为 Remarks
\ref{chow-remark-localized-chern-classes-K} 和
...
$$
当然，也可以直接使用 Definition
\ref{chow-definition-defined-on-perfect}、Lemmas
\ref{chow-lemma-additivity-on-perfect} 和
\ref{chow-lemma-chern-classes-perfect-tensor-product} 构造这些映射
（立即可见这会产生相同的类）。
回忆存在典范映射 $K_0(\textit{Vect}(X)) \to K_0(X)$，
它将有限局部自由模映到作为完美复形的自身
（置于次数 $0$）；见 Derived Categories of Schemes, Section
\ref{perfect-section-K-groups}。于是图
$$
\xymatrix{
...
$$
交换，其中东南方向的箭头是在 Remark
\ref{chow-remark-chern-classes-K} 中构造的。类似地，图
$$
\xymatrix{
...
$$
交换，其中东南方向的箭头是在 Remark
\ref{chow-remark-chern-character-K} 中构造的。
\end{remark}
...
\noindent
本节是 Section \ref{chow-section-chow-and-K} 的继续。
设 $(S, \delta)$ 如 Situation \ref{chow-situation-setup}，且
...
K_0(\textit{Coh}(X)/\textit{Coh}_{\leq k - 1}(X))
$$
其中等号由 Homology, Lemma
\ref{homology-lemma-serre-subcategory-K-groups} 得出。
Remark \ref{chow-remark-good-cases-K-A} 中的讨论表明，存在典范映射
$$
...
Remark \ref{chow-remark-perf-Z-regular} 中的映射
$\mathcal{F} \mapsto \mathcal{F}[0]$，Remark
\ref{chow-remark-localized-chern-character-K} 中的映射
...
\begin{proof}
由于 $Y$ 正则，可以应用 Remark
\ref{chow-remark-perf-Z-regular} 中的构造。由于 $Y$ 拟紧，
可以应用 Remark \ref{chow-remark-localized-chern-character-K} 中的构造。
$Y$ 局部等维（Lemma \ref{chow-lemma-locally-equidimensional}），
因而“基本周” $[Y]$ 已定义为 $\CH_*(Y)$ 的元素；见 Remark
\ref{chow-remark-fundamental-class}。结合上面构造的映射
...
$Z \subset X$，则由 Lemma \ref{chow-lemma-loc-chern-shrink-Z}，
$$
...
可假设 $Z$ 的理想由正则序列生成；见 Divisors, Lemma
\ref{divisors-lemma-Noetherian-scheme-regular-ideal}。
于是结论由 Lemma \ref{chow-lemma-actual-computation} 得出。
\end{proof}
...
Remark \ref{chow-remark-chern-character-K} 中的映射 $ch$
...
\begin{proof}
由 Derived Categories of Schemes, Lemmas
\ref{perfect-lemma-Kprime-K}、
\ref{perfect-lemma-regular-resolution-property} 和
\ref{perfect-lemma-K-is-old-K}，有
$K'_0(X) = K_0(X) = K_0(\textit{Vect}(X))$。
由 Remark \ref{chow-remark-chern-classes-agree}，当 $X = Y$ 时，
给定复合与 Proposition \ref{chow-proposition-K-tensor-Q} 中的映射一致。
故结论由该命题得出。
\end{proof}

\noindent
设 $X, S, \delta$ 如 Lemma \ref{chow-lemma-K-tensor-Q}。
为简单起见，考虑给定余维的周；见 Section
\ref{chow-section-cycles-codimension}。令 $[X]$ 为 $X$ 的基本周；
见 Remark \ref{chow-remark-fundamental-class}。
...
由 Lemma \ref{chow-lemma-adams-and-chern}，
...
（见 Divisors, Lemma \ref{divisors-lemma-quasi-regular-immersion}）。
因此法层
$$
...
（因为同构当然也是满射）。
Section \ref{chow-section-gysin-higher-codimension} 中的构造给出规范双变类
$$
...
\begin{proof}
第一项断言即 Divisors, Lemma
\ref{divisors-lemma-composition-regular-immersion}。
由 Divisors, Lemma \ref{divisors-lemma-transitivity-conormal-quasi-regular}，
存在短正合列
$$
0 \to
i^*(\mathcal{C}_{Y/Z}) \to
...
\mathcal{C}_{X/Y} \to 0
$$
因此结论由更一般的 Lemma \ref{chow-lemma-gysin-composition} 得出。
\end{proof}
...
$p^* \in A^*(P \to X)^\wedge$ 是 Lemma
\ref{chow-lemma-flat-pullback-bivariant} 的双变类。
\end{lemma}
...
结论由 Lemma \ref{chow-lemma-gysin-fundamental} 得出。
\end{proof}

\noindent
设 $(S, \delta)$ 如 Situation \ref{chow-situation-setup}。考虑 $S$ 上局部有限型概形的交换图
$$
\xymatrix{
...
Lemma \ref{chow-lemma-flat-pullback-bivariant} 的双变类
$g^* \in A^*(P \to Y)^\wedge$ 复合，得到
$$
f^! = i^! \circ g^* \in A^*(X \to Y)
$$
请注意，态射 $f$ 是局部完全交态射；见 More on Morphisms,
Definition \ref{more-morphisms-definition-lci}。
...
（Lemma \ref{chow-lemma-compose-flat-pullback}），故只需在
...
（分别由 Divisors, Lemma \ref{divisors-lemma-section-smooth-regular-immersion}
和 Divisors, Lemma \ref{divisors-lemma-flat-base-change-regular-immersion}），
且 $i' = j \circ s$。由 Lemma \ref{chow-lemma-composition-regular-immersion}，
有 $(i')^! = s^! \circ j^!$。
由于该方块是笛卡尔的，双变类 $j^!$ 是 $i^!$ 在 $P'$ 上的限制
（Remark \ref{chow-remark-restriction-bivariant}）；见
Lemma \ref{chow-lemma-construction-gysin}。
由于双变类与平坦拉回可交换，得到
$j^! \circ p^* = \overline{p}^* \circ i^!$。
因此只需证明 $s^! \circ \overline{p}^* = \text{id}$，
这已由 Lemma \ref{chow-lemma-section-smooth} 完成。
\end{proof}
...
基变换 $f'$、$g'$、$i'$ 满足相同假设
（见 Morphisms, Lemmas \ref{morphisms-lemma-base-change-smooth}、
\ref{morphisms-lemma-base-change-syntomic} 以及 More on Morphisms,
Lemma \ref{more-morphisms-lemma-flat-lci}）。
因此，由 Lemma \ref{chow-lemma-bivariant-zero}，归结为当 $Y$ 整时证明
...
（Lemma \ref{chow-lemma-gysin-fundamental}），证明完成。
\end{proof}
...
\ref{more-morphisms-lemma-composition-lci}，$g \circ f$ 是局部完全交态射，
故引理的陈述是有意义的。
...
由于双变类与平坦拉回可交换，因而
$a^! \circ q^* = p^* \circ b^!$。
将此事实与 Lemmas \ref{chow-lemma-composition-regular-immersion} 和
...
（Remark \ref{chow-remark-restriction-bivariant}）。
则 $c$ 与 $res(f^!)$ 可交换（Remark \ref{chow-remark-bivariant-commute}）。
\end{lemma}
...
关于 $i^!$ 的情形立即由 Lemma \ref{chow-lemma-gysin-commutes} 得出。
\end{proof}
...
\item $X$、$X'$、$Y$、$Y'$ 满足 Lemma
\ref{chow-lemma-locally-equidimensional} 中的等价条件；
...
\end{enumerate}
则 $f^![Y'] = [X']$，其中 $[Y']$ 和 $[X']$ 如
Remark \ref{chow-remark-fundamental-class} 所定义。
\end{lemma}
...
平坦且具有正则纤维
（见 Algebra, Section \ref{algebra-section-smooth-overview}）；
正则局部环是 Cohen--Macaulay 的
（Algebra, Lemma \ref{algebra-lemma-regular-ring-CM}）；
由假设 (4)，$\mathcal{O}_{Y', f'(\xi)}$ 是 Cohen--Macaulay 的；
于是 Algebra, Lemma \ref{algebra-lemma-CM-goes-up} 给出所需结论。
因此归结为下一段所讨论的情形。
...
Lemma \ref{chow-lemma-gysin-easy} 得出。
\end{proof}
...
例如，若 $X$ 和 $Y$ 满足 Lemma
\ref{chow-lemma-locally-equidimensional} 的等价条件，且 $Y$ 是
Cohen--Macaulay 的，则由 Lemma \ref{chow-lemma-lci-gysin-easy} 有
$f^![Y] = [X]$。在这种情形下还有
...
\begin{proof}
$\mathcal{C}$ 有限局部自由这一事实立即由 More on Algebra, Lemma
\ref{more-algebra-lemma-base-change-lci-bis} 得出。
...
$$
特别地，$f'$ 的 Gysin 映射存在。由 More on Morphisms, Lemmas
\ref{more-morphisms-lemma-get-NL}，有
$$
\NL_{X/Y} = \left( \mathcal{C}_{X/P} \to i^*\Omega_{P/Y} \right)
$$
其中 $\mathcal{C}_{X/P}$ 是嵌入 $i$ 的余法层；带撇版本同理。
由 Morphisms, Lemma \ref{morphisms-lemma-base-change-differentials}，
...
$(i')^!$ 亦然。因此应用 Lemma \ref{chow-lemma-gysin-excess} 得到
$$
...
于是 Lemma \ref{chow-lemma-compare-gysin-base-change} 适用，并给出
$$
res(b^!) = c_{top}(\mathcal{C}^\vee) \circ \pi^!
$$
其中 $\mathcal{C}$ 如该引理的陈述所定义。
当然，由 Lemma \ref{chow-lemma-lci-gysin-flat}，有 $\pi^* = \pi^!$。
...
由 More on Morphisms, Lemmas
\ref{more-morphisms-lemma-get-triangle-NL} 和
...
Section \ref{chow-section-intersection-regular} 所定义。
\end{lemma}

\begin{proof}
第一项断言由 More on Morphisms, Lemma
\ref{more-morphisms-lemma-morphism-regular-schemes-is-lci} 得出。
请注意，由 Lemma \ref{chow-lemma-lci-gysin-easy}，有 $f^![Y] = [X]$。
如 Section \ref{chow-section-intersection-regular}，写成
...
由 Lemma \ref{chow-lemma-lci-gysin-commutes}，$f^!$ 与 $c$、$c'$ 均可交换。因此
\begin{align*}
f^!(\alpha \cdot \beta)
& =
f^!(c \cap c' \cap [Y]) \\
...
\beta \cdot f_*(\alpha)
\end{align*}
第一项等式来自交乘积的构造。由 Lemma
\ref{chow-lemma-lci-gysin-commutes}，$f^!$ 与 $c'$ 可交换。
...
因此有 Section \ref{chow-section-koszul} 中构造的 Gysin 映射
$$
...
并且它是 Lemma \ref{chow-lemma-lci-gysin-composition} 的特殊情形。
\end{proof}
...
（见 Section \ref{chow-section-koszul}）。特别地，有
...
（Remark \ref{chow-remark-restriction-bivariant}）。由于图
$$
\xymatrix{
...
考察该图上方的方块并使用 Lemma \ref{chow-lemma-lci-gysin-commutes}，得到
...
c
\end{align*}
最后一项等式由 Lemma \ref{chow-lemma-diagonal-identity} 得出。
...
由 Lemma \ref{chow-lemma-flat-pullback-bivariant}，沿态射
$p' : X' \to \Spec(k)$ 的平坦拉回是元素
$(p')^* \in A^{-n}(X' \to \Spec(k))$；因而由
Lemma \ref{chow-lemma-push-proper-bivariant}，有
...
为证明这是双变类，如 Definition \ref{chow-definition-cycles} 写成
...
由 Lemma \ref{chow-lemma-flat-pullback-bivariant}，沿 $f$ 的拉回是双变类
$f^* \in A^p(\coprod X_i \to \Spec(k))$。
以 $\nu \in A^0(\coprod X_i)$ 表示如下双变类：
它在第 $i$ 个分支上将周乘以 $n_i$。于是
$\nu \circ f^* \in A^p(\coprod X_i \to X)$。
最后，由 Lemma \ref{chow-lemma-push-proper-bivariant}，有双变类
$$
g_* \circ \nu \circ f^*
$$
...
$\alpha = c \cap [\Spec(k)]$。由 Lemma \ref{chow-lemma-bivariant-zero}，
...
\begin{proof}
在 Lemma \ref{chow-lemma-chow-cohomology-towards-point} 的证明中已经看到，
...
Lemmas \ref{chow-lemma-chow-cohomology-towards-point} 和
...
$c = c_\alpha \in A^*(X \to \Spec(k))$；见 Lemma
\ref{chow-lemma-chow-cohomology-towards-point} 的证明。
\end{remark}
...
\medskip\noindent
设 $X$ 是 $k$ 上光滑概形。Section \ref{chow-section-gysin-for-diagonal} 的双变类
...
\CH_{n + m - *}(Y \times_X Z)
$$
其中第一支箭头是 Section \ref{chow-section-exterior-product} 中构造的外积，
第二支箭头是 Section \ref{chow-section-gysin-for-diagonal} 中研究的对角态射 Gysin 映射。
...
\begin{proof}
由 Lemma \ref{chow-lemma-exterior-product-associative}，在
...
由 Remark \ref{chow-remark-commuting-exterior} 成立；第二项等式同理。
\end{proof}
...
$$
第一支是 Proposition \ref{chow-proposition-compute-bivariant} 的同构
$c \mapsto c \circ g^*$，第二支是 Lemma
\ref{chow-lemma-chow-cohomology-towards-point} 的同构
$c \mapsto c \cap [\Spec(k)]$。
由 Lemma \ref{chow-lemma-chow-cohomology-towards-point} 的证明，
第二支箭头的逆把 $\alpha \in \CH_{d - p}(X)$ 映到双变类
$c_\alpha$；对于 $k$ 上局部有限型的 $Y$ 和
$\beta \in \CH_*(Y)$，该双变类把它映到
$\CH_*(X \times_k Y)$ 中的 $\alpha \times \beta$。
再由 Proposition \ref{chow-proposition-compute-bivariant} 的证明，
...
故 $f$ 的 Gysin 映射存在（Definition \ref{chow-definition-lci-gysin}）。
为证明该公式，可以把 $X$ 和 $Y$ 分解为连通分支；
因此可以假设 $X$ 是 $k$ 上维数为 $d$ 的等维光滑概形，
而 $Y$ 是 $k$ 上维数为 $e$ 的等维光滑概形。
请注意 $f^![Y]_e = [X]_d$（例如见 Lemma \ref{chow-lemma-lci-gysin-easy}）。
写成 $\alpha = c \cap [Y]_e$ 和 $\beta = c' \cap [Y]_e$，
于是 $\alpha \cdot \beta = c \cap c' \cap [Y]_e$；见
Lemma \ref{chow-lemma-identify-chow-for-smooth}。
由 Lemma \ref{chow-lemma-lci-gysin-commutes}，$f^!$ 与 $c$、$c'$ 均可交换。因此
\begin{align*}
f^!(\alpha \cdot \beta)
& =
...
f^!(\alpha) \cdot f^!(\beta)
\end{align*}
这正是所需结论；这里也对 $X$ 使用了 Lemma
\ref{chow-lemma-identify-chow-for-smooth}。

\medskip\noindent
另一种证明方法是先证明
$(f \times f)^!(\alpha \times \beta) = f^!\alpha \times f^!\beta$，
再使用 Lemma \ref{chow-lemma-lci-gysin-composition}。
\end{proof}
...
Lemma \ref{chow-lemma-identify-chow-for-smooth}。于是
\begin{align*}
f_*(\alpha \cdot f^!\beta)
& =
f_*(c \cap f^!(c' \cap [Y]_e)) \\
...
\beta \cdot f_*(\alpha)
\end{align*}
第一项等式来自对 $X$ 应用 Lemma \ref{chow-lemma-identify-chow-for-smooth} 的结果。
由 Lemma \ref{chow-lemma-lci-gysin-commutes}，$f^!$ 与 $c'$ 可交换。
...
是高余维 Gysin 映射（Section \ref{chow-section-gysin-higher-codimension}），且
...
故 Lemma \ref{chow-lemma-gysin-easy} 的条件 (1)、(2)、(3) 成立。
...
由此可见，Lemma \ref{chow-lemma-gysin-easy} 的所有假设都满足，结论随即得出。
\end{proof}
...
Lemma \ref{chow-lemma-lci-gysin-commutes} 给出；第三项等式是因为
$\CH_*(Y)$ 中 $i^![X]_d = [Y]_e$
（Lemma \ref{chow-lemma-lci-gysin-easy}）；第四项等式是因为双变类与固有前推可交换；
最后一项等式由 Lemma \ref{chow-lemma-identify-chow-for-smooth} 给出。
\end{proof}
...
与 Section \ref{chow-section-intersection-regular} 中构造的交乘积一致。
\end{lemma}
...
于是 Section \ref{chow-section-intersection-regular} 中的交乘积给出
$c \cap c' \cap [X]$。由 Lemma \ref{chow-lemma-identify-chow-for-smooth}，
...
则 $(S, \delta)$ 是一般 Situation \ref{chow-situation-setup} 的特殊情形；
见 Example \ref{chow-example-domain-dimension-1}。
请注意，$S$ 正规（Algebra, Lemma \ref{algebra-lemma-characterize-Dedekind}），
因而是正规整概形的不交并
（Properties, Lemma \ref{properties-lemma-normal-locally-Noetherian}）。
...
More on Algebra, Lemma \ref{more-algebra-lemma-dedekind-torsion-free-flat}。
因此，由 Lemma \ref{chow-lemma-flat-pullback-bivariant}，沿 $p'$ 的平坦拉回是元素
...
Lemma \ref{chow-lemma-gysin-bivariant}。态射 $p' : X' \to s$
是相对维数为 $n$ 的平坦态射。因此，由
Lemma \ref{chow-lemma-flat-pullback-bivariant}，沿 $p'$ 的平坦拉回是元素
$(p')^* \in A^{-n}(X' \to S)$。于是
$$
c' = i_* \circ (p')^* \circ c \in A^{-n}(X \to S)
$$
由 Lemma \ref{chow-lemma-push-proper-bivariant}，这给出映射
$$
c' \cap - : \CH_m(Y) \longrightarrow \CH_{m + n - 1}(X \times_S Y)
$$
对于任意 $\delta$-维数为 $m$ 的整闭子概形 $Y' \subset Y$，
...
$$
最后，由于 $g$ 固有，Lemma \ref{chow-lemma-push-proper-bivariant} 给出双变类
$$
g_* \circ \xi \in A^p(X \to S)
$$
读者容易验证 $c_\alpha$ 等于该双变类
（请与 Lemma \ref{chow-lemma-exterior-product-well-defined-dim-1} 的证明比较），
因而它本身也是双变类。
...
由 Lemma \ref{chow-lemma-bivariant-zero}，只需在 $X'$ 是 $S$ 上局部有限型整概形时证明
$$
c \cap [X'] = c_\alpha \cap [X']
$$
...
\begin{proof}
在 Lemma \ref{chow-lemma-chow-cohomology-towards-base-dim-1} 的证明中已经看到，
...
Lemmas \ref{chow-lemma-chow-cohomology-towards-base-dim-1} 和
...
$c = c_\alpha \in A^*(X \to S)$；见 Lemma
\ref{chow-lemma-chow-cohomology-towards-base-dim-1} 的证明。
\end{remark}
...
对闭点 $s \in S$ 置 $\delta(s) = 0$，否则置 $\delta(s) = 1$。
则 $(S, \delta)$ 是一般 Situation \ref{chow-situation-setup} 的特殊情形；
见 Example \ref{chow-example-domain-dimension-1} 以及
Section \ref{chow-section-exterior-product-dim-1} 中的讨论。

\medskip\noindent
设 $X$ 是 $S$ 上光滑概形。Section \ref{chow-section-gysin-for-diagonal} 的双变类
$\Delta^!$ 使我们能够在 $X$ 上局部有限型概形的 Chow 群上定义一种交乘积。
具体地，设 $Y \to X$ 和 $Z \to X$ 是局部有限型概形之间的态射。注意到
$$
...
\CH_{n + m - *}(Y \times_X Z)
$$
其中第一支箭头是 Section \ref{chow-section-exterior-product-dim-1} 中构造的外积，
第二支箭头是 Section \ref{chow-section-gysin-for-diagonal} 中研究的对角态射 Gysin 映射。
...
\begin{proof}
由 Lemma \ref{chow-lemma-exterior-product-associative-dim-1}，在
...
第一支是 Proposition \ref{chow-proposition-compute-bivariant} 的同构
$c \mapsto c \circ g^*$，第二支是 Lemma
\ref{chow-lemma-chow-cohomology-towards-base-dim-1} 的同构
$c \mapsto c \cap [S]_1$。
由 Lemma \ref{chow-lemma-chow-cohomology-towards-base-dim-1} 的证明，
第二支箭头的逆把 $\alpha \in \CH_{d - p}(X)$ 映到双变类
$c_\alpha$；对于 $k$ 上局部有限型的 $Y$ 和
$\beta \in \CH_*(Y)$，该双变类把它映到
$\CH_*(X \times_k Y)$ 中的 $\alpha \times \beta$。
...
这里 $(S, \delta)$ 和 $(S', \delta')$ 如 Situation \ref{chow-situation-setup}。
...
在 Situation \ref{chow-situation-setup-base-change} 中，设 $X \to S$ 局部有限型，
...
（因为由 Morphisms, Lemma \ref{morphisms-lemma-generalizations-lift-flat}，
...
由 Morphisms, Lemma \ref{morphisms-lemma-dimension-fibre-after-base-change}，有
$\dim_x(X_s) = \dim_{x'}(X_{s'})$。
由于 $x$ 是不可约分支 $X_s$ 的一般点
（这是不可约概形，不过我们不需要这一点），而 $x'$ 是 $X'_{s'}$ 某个不可约分支的一般点，
故由 Morphisms, Lemma \ref{morphisms-lemma-dimension-fibre-at-a-point}，得到
...
$$
由 Definition \ref{chow-definition-delta-dimension}，这就证明了所需结论。
\end{proof}
...
映到 $[Z \times_S S']_{k + c}$。由 Lemma
\ref{chow-lemma-dimension-base-change}，这是有意义的。
...
\begin{proof}
该证明与 Lemma \ref{chow-lemma-pullback-coherent} 的证明完全相同，
建议读者略过。

\medskip\noindent
关于维数的断言由 Lemma \ref{chow-lemma-dimension-base-change} 得出。
由 Lemma \ref{chow-lemma-cycle-closed-coherent}，以及凝聚模
$\mathcal{O}_Z$ 的基变换是 $\mathcal{O}_{Z'}$ 这一事实，
(1) 由 (2) 得出。

\medskip\noindent
证明 (2)。由于 $X$、$X'$ 局部 Noether，可应用
Cohomology of Schemes, Lemma \ref{coherent-lemma-coherent-Noetherian} 得知
$\mathcal{F}$ 有限型；所以 $\mathcal{F}'$ 有限型
（Modules, Lemma \ref{modules-lemma-pullback-finite-type}），
进而 $\mathcal{F}'$ 凝聚
（再次使用 Cohomology of Schemes, Lemma
\ref{coherent-lemma-coherent-Noetherian}）。故引理的陈述有意义。
...
见 Lemma \ref{chow-lemma-length-finite}。）有
...
m = \text{length}_A(M) \text{length}_B(B/\mathfrak m_AB)
$$
所需等式遂由 Algebra, Lemma \ref{algebra-lemma-pullback-module} 得出。
\end{proof}
...
由 Lemma \ref{chow-lemma-rational-equivalence-family}，存在一个整闭子概形的局部有限族
...
Lemma \ref{chow-lemma-closed-subscheme-cross-p1} 的假设，其中以 $k + c$ 代替 $k$；以及
\item $g^*[W_0]_k = [(W')_0]_{k + c}$ 且
$g^*[W_\infty]_k = [(W')_\infty]_{k + c}$。
\end{enumerate}
(2) 立即由 Lemma \ref{chow-lemma-pullback-coherent-base-change} 以及
$(W')_0$ 是 $W_0$ 的基变换这一事实得出（利用纤维积的结合律）。
对于 (1)，维数断言先由 Lemma \ref{chow-lemma-dimension-base-change} 得出。
...
在 Situation \ref{chow-situation-setup-base-change} 中，设
...
由构造，有 $g^*[Z] = [Z']_{k + c}$。由 Lemma
\ref{chow-lemma-pullback-coherent}，有
$(f')^*g^*[Z] = [Z' \times_{X'} Y']_{k + c + r}$。
反过来，由 Definition \ref{chow-definition-flat-pullback}，有
$f^*[Z] = [Z \times_X Y]_{k + r}$；由
Lemma \ref{chow-lemma-pullback-coherent-base-change}，有
...
该同构由平坦基变换得出；见 Cohomology of Schemes, Lemma
\ref{coherent-lemma-flat-base-change-cohomology}。
\end{proof}
...
Lemmas \ref{chow-lemma-linebundle} 和 \ref{chow-lemma-linebundle-formulae}。
拉回后仍有相同的刻画。因此引理由 Lemmas
\ref{chow-lemma-pullback-base-change-pullback} 和
...
$c_1$ 作杯积都与基变换可交换（Lemmas
\ref{chow-lemma-pullback-base-change-pullback} 和
\ref{chow-lemma-pullback-base-change-c1}）。因此，该引理由
Lemma \ref{chow-lemma-determine-intersections} 中对与
...
Situation \ref{chow-situation-setup}。设 $g : S' \to S$ 和 $g' : S'' \to S'$
...
\item Lemma \ref{chow-lemma-pullback-base-change} 的映射
$g^* : \CH_k(X) \to \CH_{k + c}(X')$ 与
$(g')^* : \CH_{k + c}(X') \to \CH_{k + c + c'}(X'')$ 的复合，
给出 Lemma \ref{chow-lemma-pullback-base-change} 的映射
...
\begin{proof}
由 Lemma \ref{chow-lemma-compose-base-change}，得到周群系 $Z_k(X_i)$、
...
设 $Z' \subset X'$ 是 $\delta'$-维数为 $k$ 的整闭子概形。
由 Limits, Lemma \ref{limits-lemma-descend-finite-presentation}，
...
由 Lemma \ref{chow-lemma-dimension-base-change}，得到
...
由 Lemma \ref{chow-lemma-dimension-base-change}，$Z'_j$ 的每个不可约分支都具有
...
故对所有 $j$ 都有 $Z'_j = \emptyset$。然而，由 Limits, Lemma
\ref{limits-lemma-limit-nonempty}，这意味着存在某个过渡态射
...
由 Limits, Lemma \ref{limits-lemma-descend-opens}，这些开子概形可以下降；
再增大 $i$ 后，由 Limits, Lemma \ref{limits-lemma-descend-section}，
...
$$
由 Algebra, Lemma \ref{algebra-lemma-pullback-module} 以及
Algebra, Section \ref{algebra-section-orders-of-vanishing} 中
$\text{ord}$ 的构造得出。
\end{proof}
